# Creanova Enterprise Usage Telemetry Service
...
5. [Implementation Plan](#5-implementation-plan)
   - 5.1 [Database Schema and Models (M1)](#51-database-schema-and-models-m1)
     - 5.1.1 [Creanova - Database Migration](#511-Creanova---database-migration)
     - 5.1.2 [Creanova - Model Tests](#512-Creanova---model-tests)
   - 5.2 [Metrics Collection Framework (M2)](#52-metrics-collection-framework-m2)
     - 5.2.1 [Creanova - Core Collection Framework](#521-Creanova---core-collection-framework)
     - 5.2.2 [Creanova - Example Collectors](#522-Creanova---example-collectors)
     - 5.2.3 [Creanova - Framework Tests](#523-Creanova---framework-tests)
   - 5.3 [Collection and Upload Processors (M3)](#53-collection-and-upload-processors-m3)
     - 5.3.1 [Creanova - Collection Processor](#531-Creanova---collection-processor)
     - 5.3.2 [Creanova - Upload Processor](#532-Creanova---upload-processor)
     - 5.3.3 [Creanova - Integration Tests](#533-Creanova---integration-tests)
   - 5.4 [License Warning API (M4)](#54-license-warning-api-m4)
     - 5.4.1 [Creanova - License Status API](#541-Creanova---license-status-api)
     - 5.4.2 [Creanova - API Integration](#542-Creanova---api-integration)
   - 5.5 [UI Warning Banner (M5)](#55-ui-warning-banner-m5)
     - 5.5.1 [Creanova - UI Warning Banner](#551-Creanova---ui-warning-banner)
     - 5.5.2 [Creanova - UI Integration](#552-Creanova---ui-integration)
   - 5.6 [Helm Chart Deployment Configuration (M6)](#56-helm-chart-deployment-configuration-m6)
     - 5.6.1 [Creanova-Cloud - Cronjob Manifests](#561-Creanova-cloud---cronjob-manifests)
     - 5.6.2 [Creanova-Cloud - Configuration Management](#562-Creanova-cloud---configuration-management)
   - 5.7 [Documentation and Enhanced Collectors (M7)](#57-documentation-and-enhanced-collectors-m7)
     - 5.7.1 [Creanova - Advanced Collectors](#571-Creanova---advanced-collectors)
     - 5.7.2 [Creanova - Monitoring and Testing](#572-Creanova---monitoring-and-testing)
     - 5.7.3 [Creanova - Technical Documentation](#573-Creanova---technical-documentation)

## 1. Introduction

### 1.1 Problem Statement

Creanova Enterprise (OHE) helm charts are publicly available but not open source, creating a visibility gap for the sales team. Unknown users can install and use OHE without the vendor's knowledge, preventing proper customer engagement and sales pipeline management. Without usage telemetry, the vendor cannot identify potential customers, track installation health, or proactively support users who may need assistance.
...
When telemetry uploads fail for more than 4 days, users will see a prominent warning banner in the Creanova Enterprise UI:

```
⚠️ Your Creanova Enterprise license will expire in 30 days. Please contact support if this issue persists.
```
...
2. **Generates unique customer identifiers** using administrator contact information:
   - Customer email: Determined by the following priority order:
     1. `Creanova_ADMIN_EMAIL` environment variable (if set in helm values)
...
**Tier 1: Explicit Configuration** - The `Creanova_ADMIN_EMAIL` environment variable allows administrators to explicitly specify the contact email during deployment.
...
class TelemetryUploadProcessor(MaintenanceTaskProcessor):
    ...
    async def _get_admin_email(self) -> str | None:
        ...
        env_admin_email = os.getenv('Creanova_ADMIN_EMAIL')
        if env_admin_email:
...
async def get_license_status():
    ...
    if not _is_Creanova_enterprise():
    ...
    async with a_session_maker() as session:
        ...
        if not last_upload:
            ...
            return {
                "warn": True,
                "message": "Creanova Enterprise license verification pending. Please ensure network connectivity."
            }
        ...
        if days_since_upload > 4:
            ...
            if oldest_unsent:
                ...
                if days_until_expiration <= 0:
                    message = "Your Creanova Enterprise license has expired. Please contact support immediately."
                else:
                    message = f"Your Creanova Enterprise license will expire in {days_until_expiration} days. Please contact support if this issue persists."

                return {"warn": True, "message": message}

        return {"warn": False, "message": ""}

def _is_Creanova_enterprise() -> bool:
...
The cronjob configurations will be deployed via the Creanova-Cloud helm charts.
...
```yaml
# charts/Creanova/templates/telemetry-collection-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "Creanova.fullname" . }}-telemetry-collection
  labels:
    {{- include "Creanova.labels" . | nindent 4 }}
spec:
      ...
      template:
        spec:
          ...
          - name: telemetry-collector
            ...
            env:
            {{- include "Creanova.env" . | nindent 12 }}
            command:
            - python
            - -c
            - |
...
```yaml
# charts/Creanova/templates/telemetry-upload-cronjob.yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: {{ include "Creanova.fullname" . }}-telemetry-upload
  labels:
    {{- include "Creanova.labels" . | nindent 4 }}
spec:
        ...
        spec:
          ...
          - name: telemetry-uploader
            ...
            - name: REPLICATED_PUBLISHABLE_KEY
              valueFrom:
                secretKeyRef:
                  name: {{ include "Creanova.fullname" . }}-replicated-config
                  key: publishable-key
            - name: REPLICATED_APP_SLUG
              value: {{ .Values.telemetry.replicatedAppSlug | default "Creanova-enterprise" | quote }}
            command:
            - python
            - -c
            - |
              ...
              async def main():
                  ...
                  processor = TelemetryUploadProcessor(
                      ...
                      replicated_app_slug=os.getenv('REPLICATED_APP_SLUG', 'Creanova-enterprise')
                  )
                  task = MaintenanceTask()
                  task.set_processor(processor)
                  task.status = MaintenanceTaskStatus.PENDING
...
#### 5.1.1 Creanova - Database Migration
...
#### 5.2.1 Creanova - Core Collection Framework
...
#### 5.2.2 Creanova - Example Collectors
...
#### 5.2.3 Creanova - Framework Tests
...
#### 5.5.1 Creanova - UI Warning Banner
...
**Repository**: Creanova-Cloud
Create Kubernetes cronjob configurations and deployment scripts.

#### 5.6.1 Creanova-Cloud - Cronjob Manifests

- [ ] `charts/Creanova/templates/telemetry-collection-cronjob.yaml`
- [ ] `charts/Creanova/templates/telemetry-upload-cronjob.yaml`

#### 5.6.2 Creanova-Cloud - Configuration Management

- [ ] `charts/Creanova/templates/replicated-secret.yaml`
- [ ] Update `charts/Creanova/values.yaml` with telemetry configuration options:
  ```yaml
  # Add to values.yaml
  telemetry:
    enabled: true
    replicatedAppSlug: "Creanova-enterprise"
    adminEmail: ""  # Optional: admin email for customer identification

  # Add to deployment environment variables
  env:
    Creanova_ADMIN_EMAIL: "{{ .Values.telemetry.adminEmail }}"
  ```
...
#### 5.7.1 Creanova - Advanced Collectors
...
#### 5.7.2 Creanova - Monitoring and Testing
...
#### 5.7.3 Creanova - Technical Documentation
