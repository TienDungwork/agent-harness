...
Creanova uses Keycloak for identity management in the SaaS deployment. The authentication flow involves multiple services:
...
| **UserAuth** | Abstract auth interface | `Creanova/server/user_auth/user_auth.py` |
| **SaasUserAuth** | Keycloak implementation | `enterprise/server/auth/saas_user_auth.py` |
| **JWT Service** | Token signing/verification | `Creanova/app_server/services/jwt_service.py` |
