...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('Creanova_helped_author', sa.Boolean(), nullable=True),
        sa.Column('num_Creanova_commits', sa.Integer(), nullable=True),
        sa.Column('num_Creanova_review_comments', sa.Integer(), nullable=True),
        sa.Column('num_Creanova_general_comments', sa.Integer(), nullable=True),
    )

    # Create indexes for efficient querying
    op.create_index(
        'ix_Creanova_prs_repo_id', 'Creanova_prs', ['repo_id'], unique=False
    )
    op.create_index(
        'ix_Creanova_prs_pr_number', 'Creanova_prs', ['pr_number'], unique=False
    )
    op.create_index(
        'ix_Creanova_prs_status', 'Creanova_prs', ['status'], unique=False
    )

    # Create unique constraint on repo_id + pr_number combination
    op.create_index(
        'ix_Creanova_prs_repo_pr_unique',
        'Creanova_prs',
        ['repo_id', 'pr_number'],
        unique=True,
    )
...
def downgrade() -> None:
    op.drop_index('ix_Creanova_prs_repo_id', table_name='Creanova_prs')
    op.drop_index('ix_Creanova_prs_pr_number', table_name='Creanova_prs')
    op.drop_index('ix_Creanova_prs_status', table_name='Creanova_prs')
    op.drop_index('ix_Creanova_prs_repo_pr_unique', table_name='Creanova_prs')
    op.drop_table('Creanova_prs')
