...
from Creanova.app_server.settings.settings_models import Settings as DataSettings
...
async def test_ensure_api_key_keeps_valid_key():
    ...
    item = _make_settings(model='Creanova/gpt-4', api_key=existing_key)
    ...
    with patch(
        ...
        await store._ensure_api_key(item, 'org-123', Creanova_type=True)
...
async def test_ensure_api_key_generates_new_key_when_verification_fails():
    ...
    switching to/from an Creanova/* model never orphans a key."""
    from storage.lite_llm_manager import get_Creanova_cloud_key_alias

    store = SaasSettingsStore('test-user-id-123')
    new_key = 'sk-new-key'
    item = _make_settings(model='Creanova/gpt-4', api_key='sk-invalid-key')
    expected_alias = get_Creanova_cloud_key_alias('test-user-id-123', 'org-123')
    ...
    with (
        ...
        # The Creanova branch now deletes the prior key under the shared alias
        ...
        mock_generate.assert_awaited_once_with(
            'test-user-id-123', 'org-123', expected_alias, {'type': 'Creanova'}
        )
...
async def test_load_canonicalizes_legacy_litellm_proxy_active_llm(
    ...
    assert loaded.agent_settings.llm.model == 'Creanova/claude-opus-4-8'
...
async def test_load_canonicalizes_legacy_litellm_proxy_llm_profiles(
    ...
    assert legacy.model == 'Creanova/claude-opus-4-8'
...
@pytest.mark.asyncio
async def test_store_keeps_Creanova_managed_keys_member_specific(
    session_maker, async_session_maker, org_with_multiple_members_fixture
):
    """Managed Creanova keys should not be copied from one member to everyone else."""
    from sqlalchemy import select
    from storage.org import Org
    from storage.org_member import OrgMember
    ...
    with session_maker() as session:
        org = session.execute(select(Org).where(Org.id == org_id)).scalars().first()
        assert org is not None
        # Settings keeps the public Creanova/ provider prefix in persisted data
        assert (
            org.agent_settings['llm']['model'] == 'Creanova/claude-opus-4-5-20251101'
        )
        ...
        for member in members.values():
            assert (
                member.agent_settings_diff['llm']['model']
                == 'Creanova/claude-opus-4-5-20251101'
            )
...
@pytest.mark.asyncio
async def test_store_skips_ensure_api_key_for_non_Creanova_model_without_base_url(
    session_maker, async_session_maker, org_with_multiple_members_fixture
):
    """When saving a non-Creanova model with no base URL (basic view BYOR),
...
@pytest.mark.asyncio
async def test_store_calls_ensure_api_key_for_Creanova_model_without_base_url(
    session_maker, async_session_maker, org_with_multiple_members_fixture
):
    """Creanova models still require proxy-key verification without a base URL."""
    fixture = org_with_multiple_members_fixture
    admin_user_id = str(fixture['admin_user_id'])
    store = SaasSettingsStore(admin_user_id)
