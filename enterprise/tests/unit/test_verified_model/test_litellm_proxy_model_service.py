...
from Creanova.app_server.utils.llm import ModelsResponse
    ...
    'data': [
        {
            ...
            'model_info': {'Creanova_default': True},
        },
        {
            'model_name': 'hidden-model',
            'litellm_params': {'model': 'openai/gpt-5'},
            'model_info': {
                'Creanova_hidden': True,
                'Creanova_canonical': 'claude-sonnet-4-5',
            },
        },
        {
...
# guard; an ``Creanova/``-prefixed entry must be dropped from the union.
...
class TestHappyPath:
    async def test_models_response_shape(self, monkeypatch):
        ...
        # Creanova/ prefix.
        assert response.models == ['Creanova/claude-sonnet-4-5', 'Creanova/gpt-5']
        assert response.verified_models == ['claude-sonnet-4-5', 'gpt-5']
        assert response.verified_providers == ['Creanova']
        # Hidden alias routes are still reported so saved settings that
        # reference them count as available.
        assert response.hidden_models == ['Creanova/hidden-model']
        # The alias carries its canonical mapping (canonical is visible).
        assert response.hidden_model_canonicals == {
            'Creanova/hidden-model': 'Creanova/claude-sonnet-4-5'
        }
        # litellm_proxy/ prefix translated to Creanova/.
        assert response.default_model == 'Creanova/claude-sonnet-4-5'
    ...
    async def test_search_llm_models_uses_proxy_models(self, monkeypatch):
        ...
        assert names == [
            ('Creanova', 'claude-sonnet-4-5', False, None),
            ('Creanova', 'gpt-5', False, None),
            ...
            ('Creanova', 'hidden-model', True, 'claude-sonnet-4-5'),
        ]
    ...
    async def test_hidden_only_when_all_duplicate_entries_hidden(self, monkeypatch):
            ...
            'data': [
                {
                    ...
                    'model_info': {'Creanova_hidden': True},
                },
        ...
        assert response.models == ['Creanova/mixed-model']
        assert response.hidden_models == ['Creanova/legacy-alias']
        # No Creanova_canonical tags anywhere -> no mappings.
        assert response.hidden_model_canonicals == {}
    ...
    async def test_canonical_skipped_when_not_visible(self, monkeypatch):
        ...
        payload = {
            'data': [
                {'model_name': 'real-model', 'model_info': {}},
                {
                    ...
                    'model_info': {
                        'Creanova_hidden': True,
                        'Creanova_canonical': 'removed-model',
                    },
                },
                {
                    ...
                    'model_info': {
                        'Creanova_hidden': True,
                        'Creanova_canonical': 'other-alias',
                    },
                },
                {
        ...
        assert response.models == ['Creanova/real-model']
        assert response.hidden_models == [
            'Creanova/alias-to-missing',
            'Creanova/alias-to-hidden',
            'Creanova/other-alias',
        ]
        assert response.hidden_model_canonicals == {}
    ...
    async def test_canonical_mapped_for_visible_target(self, monkeypatch):
        ...
        assert response.hidden_model_canonicals == {
            'Creanova/legacy-alias': 'Creanova/real-model'
        }

    async def test_search_providers_only_Creanova(self, monkeypatch):
        install_client(monkeypatch, response=FakeResponse(HAPPY_PAYLOAD))
        service = LiteLLMProxyModelService()

        page = await service.search_providers()

        assert [(p.name, p.verified) for p in page.items] == [('Creanova', True)]
...
class TestDefaultModelTranslation:
    def test_litellm_proxy_prefix_translated(self, monkeypatch):
        ...
        assert _derive_default_model() == 'Creanova/gpt-5'

    def test_Creanova_prefix_preserved(self, monkeypatch):
        monkeypatch.setattr(
            router_module, 'get_default_litellm_model', lambda: 'Creanova/gpt-5'
        )
        assert _derive_default_model() == 'Creanova/gpt-5'
...
class TestFailurePaths:
    ...
    async def test_stale_cache_served_on_failure(self, monkeypatch, fake_clock, caplog):
        ...
        assert first.models == ['Creanova/claude-sonnet-4-5', 'Creanova/gpt-5']
...
class TestCacheTtl:
    ...
    async def test_after_ttl_refetches(self, monkeypatch, fake_clock):
        ...
        assert len(calls) == 1
        assert len(calls2) == 1
        assert response.models == ['Creanova/new-model']
...
class TestByokUnion:
    ...
    async def test_byok_on_unions_catalogue(self, monkeypatch):
        ...
        # the Creanova/-prefixed catalogue entry is dropped.
        assert response.models == [
            'Creanova/claude-sonnet-4-5',
            'Creanova/gpt-5',
            'anthropic/claude-sonnet-4-5',
            'openai/gpt-5',
            'openai/gpt-4o',
    ...
    async def test_byok_on_only_managed_models_verified(self, monkeypatch):
        ...
        assert verified == {('Creanova', 'claude-sonnet-4-5'), ('Creanova', 'gpt-5')}
    ...
    async def test_byok_on_exposes_catalogue_providers(self, monkeypatch):
        ...
        # Creanova stays the verified managed provider, listed first; catalogue
