...
def test_normalize_agent_settings_masks_api_key_in_json_on_empty_and_real_keys():
    ...
    empty_key = OrgUpdate.model_validate(
        {
            'agent_settings_diff': {
                'llm': {'model': 'Creanova/x', 'api_key': '', 'base_url': None},
            },
        }
    )
...
def test_normalize_agent_settings_fills_base_url_for_all_providers():
    ...
    Creanova_null = OrgUpdate.model_validate(
        {
            'agent_settings_diff': {
                'llm': {'model': 'Creanova/claude-3', 'base_url': None},
            },
        }
    )
    Creanova_missing = OrgUpdate.model_validate(
        {'agent_settings_diff': {'llm': {'model': 'Creanova/claude-3'}}}
    )
    ...
    Creanova_null_diff = Creanova_null.agent_settings_diff
    assert Creanova_null_diff is not None
    assert Creanova_null_diff['llm']['model'] == 'Creanova/claude-3'
    assert Creanova_null_diff['llm']['base_url'].rstrip('/') == (
        LITE_LLM_API_URL.rstrip('/')
    )

    Creanova_missing_diff = Creanova_missing.agent_settings_diff
    assert Creanova_missing_diff is not None
    assert Creanova_missing_diff['llm']['model'] == 'Creanova/claude-3'
    assert Creanova_missing_diff['llm']['base_url'].rstrip('/') == (
        LITE_LLM_API_URL.rstrip('/')
    )
...
def test_from_org_validates_persisted_Creanova_agent_kind():
    """GIVEN: An org row whose persisted ``agent_settings`` carry the
        canonical ``agent_kind: 'Creanova'`` discriminator (the exact shape
    ...
    org.agent_settings = {
        ...
        'agent': 'CodeActAgent',
        'agent_kind': 'Creanova',
        'llm': {'model': 'Creanova/claude', 'base_url': LITE_LLM_API_URL},
    }
    ...
    # Assert
    assert response.agent_settings.agent_kind == 'Creanova'
    assert response.agent_settings.llm.model == 'Creanova/claude'
...
def test_from_org_preserves_acp_agent_settings_without_500():
    ...
    THEN: It returns the ``ACPAgentSettings`` variant instead of force-casting
        to ``CreanovaAgentSettings`` (which 500'd on the non-nullable
        ``agent_context``).
    """
    org = MagicMock(spec=Org)
    org.agent_settings = {
...
def test_from_org_keeps_Creanova_prefix_and_hides_managed_base_url():
    """Managed Creanova models should return the public prefix in basic mode."""
    org = MagicMock(spec=Org)
    org.agent_settings = {
        'schema_version': 1,
        'agent': 'CodeActAgent',
        'llm': {
            'model': 'Creanova/minimax-m2.5',
    ...
    assert response.agent_settings.llm.model == 'Creanova/minimax-m2.5'
...
def test_from_org_returns_provider_default_base_url_as_stored_for_non_managed_models():
    ...
    from Creanova.app_server.utils.llm import get_provider_api_base as _provider_base
