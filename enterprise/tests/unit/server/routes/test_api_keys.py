...
class TestRefreshManagedLlmApiKey:
    ...
    with org-default precedence), the Creanova metadata attachment, and the
    ...
    async def _seed(
        async_session_maker,
        *,
        llm_model='Creanova/claude-sonnet-4',
        llm_base_url=None,
        member_custom=False,
    ...
    def test_managed_config_detects_managed_base_url(self):
        with patch(
            ...
            ) == ManagedLlmKeyConfig(Creanova_type=False)

    def test_managed_config_detects_Creanova_model_without_base_url(self):
        with patch(
            'storage.saas_settings_store.LITE_LLM_API_URL',
            'https://litellm.example.com',
        ):
            assert managed_llm_key_config_from_model(
                'Creanova/claude-sonnet-4', None
            ) == ManagedLlmKeyConfig(Creanova_type=True)

    def test_managed_config_detects_Creanova_model_with_all_hands_base_url(self):
        with patch(
            'storage.saas_settings_store.LITE_LLM_API_URL',
            'https://litellm.example.com',
        ):
            assert managed_llm_key_config_from_model(
                'Creanova/claude-sonnet-4',
                'https://llm-proxy.staging.all-hands.dev/v1',
            ) == ManagedLlmKeyConfig(Creanova_type=True)

    def test_managed_config_rejects_Creanova_model_custom_base_url(self):
        with patch(
            'storage.saas_settings_store.LITE_LLM_API_URL',
            'https://litellm.example.com',
        ):
            assert (
                managed_llm_key_config_from_model(
                    'Creanova/claude-sonnet-4', 'https://custom-llm.example.com'
                )
                is None
            )
    ...
    @pytest.mark.asyncio
    async def test_rotate_Creanova_model_attaches_Creanova_metadata(
        self, async_session_maker, managed_env
    ):
        """An Creanova/* effective config rotates with {'type': 'Creanova'}."""
        ...
        assert rotation.Creanova_type is True
        assert rotation.old_key == 'sk-old-managed-key'
        assert rotation.new_key == 'sk-new-managed-key'

        expected_alias = get_Creanova_cloud_key_alias(user_id, str(org_id))
        ...
        mock_generate.assert_awaited_once_with(
            user_id, str(org_id), expected_alias, {'type': 'Creanova'}
        )
    ...
    @pytest.mark.asyncio
    async def test_rotate_managed_base_url_attaches_no_Creanova_metadata(
        self, async_session_maker, managed_env
    ):
        """A non-Creanova model on the managed base_url rotates without the
        Creanova metadata marker.
        """
        ...
        assert rotation.status == ManagedLlmKeyStatus.ROTATED
        assert rotation.Creanova_type is False
        expected_alias = get_Creanova_cloud_key_alias(user_id, str(org_id))
        mock_delete_alias.assert_awaited_once_with(key_alias=expected_alias)
        mock_generate.assert_awaited_once_with(
        ...
        async with async_session_maker() as session:
            session.add(
                Org(
                    ...
                    org_version=ORG_SETTINGS_VERSION,
                    agent_settings={'llm': {'model': 'Creanova/claude-sonnet-4'}},
                )
            )
            await session.commit()
