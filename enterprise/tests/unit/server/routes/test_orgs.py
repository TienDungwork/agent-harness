...
@pytest.mark.parametrize(
    ...
    ids=['agent_kind_Creanova', 'agent_kind_llm_legacy', 'no_agent_kind'],
)
async def test_list_user_orgs_handles_persisted_agent_kind_variants(
    ...
    """GIVEN: An org row whose persisted ``agent_settings`` carries any of the
        three discriminator shapes seen in production (current ``'Creanova'``,
...
async def test_get_org_defaults_settings_success():
    ...
    mock_org.agent_settings = CreanovaAgentSettings(
        agent='CodeActAgent',
        llm={'model': 'Creanova/claude-3', 'base_url': 'https://proxy.example'},
    )
    ...
    assert response.agent_settings.llm.model == 'Creanova/claude-3'
...
async def test_update_org_defaults_settings_forwards_through_org_service():
    ...
    updated_org.agent_settings = CreanovaAgentSettings(
        llm={
            'model': 'Creanova/claude-3.5-sonnet',
    ...
    update_data = OrgUpdate(
        agent_settings_diff={
            'llm': {'model': 'Creanova/claude-3.5-sonnet'},
        },
    ...
    assert response.agent_settings.llm.model == 'Creanova/claude-3.5-sonnet'
