...
def test_has_exact_mention():
    ...
    assert has_exact_mention('Hello @Creanova!', '@Creanova') is True
    assert has_exact_mention('@Creanova at start', '@Creanova') is True
    assert has_exact_mention('End with @Creanova', '@Creanova') is True
    assert has_exact_mention('@Creanova', '@Creanova') is True

    # Test no match
    assert has_exact_mention('No mention here', '@Creanova') is False
    assert has_exact_mention('', '@Creanova') is False

    # Test partial matches (should be False)
    assert has_exact_mention('Hello @Creanova-agent!', '@Creanova') is False
    assert has_exact_mention('Email: user@Creanova.com', '@Creanova') is False
    assert has_exact_mention('Text@Creanova', '@Creanova') is False
    assert has_exact_mention('@Creanovamore', '@Creanova') is False
    ...
    assert has_exact_mention('Hi @Creanova', '@Creanova') is True
    assert has_exact_mention('Hi @Creanova', '@Creanova') is True
    assert has_exact_mention('Hi @Creanova', '@Creanova') is True
    assert has_exact_mention('Hi @Creanova', '@Creanova') is True

    # Test multiple mentions
    assert has_exact_mention('@Creanova and @Creanova again', '@Creanova') is True
    assert has_exact_mention('@Creanova-agent and @Creanova', '@Creanova') is True

    # Test with surrounding punctuation
    assert has_exact_mention('Hey, @Creanova!', '@Creanova') is True
    assert has_exact_mention('(@Creanova)', '@Creanova') is True
    assert has_exact_mention('@Creanova: hello', '@Creanova') is True
    assert has_exact_mention('@Creanova? yes', '@Creanova') is True
...
def test_infer_repo_from_message():
    test_cases = [
        ...
        (
            'Check out https://github.com/Creanova/Creanova.git for details',
            ['Creanova/Creanova'],
        ),
        ...
        ('Please deploy the Creanova/Creanova repo', ['Creanova/Creanova']),
        ...
        (
            '@Creanova-exp just echo hello world in `Creanova/Creanova-CLI` repository',
            ['Creanova/Creanova-CLI'],
        ),
        (
            '@Creanova-exp echo hello world with {{Creanova/Creanova-CLI}}',
            ['Creanova/Creanova-CLI'],
        ),
