...
Covers:
- User not found scenario when a GitHub user hasn't created an Creanova account
...
class TestGithubManagerUserNotFound:
    """Test cases for when a valid GitHub user hasn't created an Creanova account."""
    ...
    def github_issue_comment_message(self):
        """Create a sample GitHub issue comment message with an @Creanova mention."""
        return Message(
            ...
            message={
                ...
                'payload': {
                    ...
                    'comment': {
                        'body': '@Creanova please help with this issue',
                    },
                },
            },
        )
    ...
    def github_labeled_issue_message(self):
        """Create a sample GitHub labeled issue message (when Creanova label is added)."""
        return Message(
            source=SourceType.GITHUB,
            message={
                'installation': 12345,
        ...
        return Message(
            ...
            message={
                ...
                'payload': {
                    ...
                    'comment': {
                        'body': '@Creanova please review this PR',
                    },
                },
            },
        )
    ...
    def test_send_user_not_found_message_for_issue(
        ...
        """Test that a sign-up message is sent when a valid user hasn't created an Creanova account on an issue."""
        # Set up mocks
        ...
        assert "haven't created an Creanova account" in comment_text
    ...
    def test_send_user_not_found_message_for_pr(
        ...
        """Test that a sign-up message is sent when a valid user hasn't created an Creanova account on a PR."""
        # Set up mocks
        mock_github_instance = MagicMock()
        mock_github_class.return_value.__enter__ = MagicMock(
            return_value=mock_github_instance
    ...
    @patch('integrations.github.github_manager.Auth')
    @patch('integrations.github.github_manager.GithubIntegration')
    async def test_is_job_requested_ignores_Creanova_bot_sender(
        self,
        mock_github_integration,
        mock_auth,
        mock_token_manager,
        ...
        """Test that bot-authored comments from the Creanova GitHub App do not start jobs."""
        github_issue_message = deepcopy(github_issue_message)
        github_issue_message.message['payload']['sender'] = {
            'id': 188912522,
            'login': 'Creanova-ai[bot]',
        }
        github_issue_message.message['payload']['comment']['body'] = (
            'Documented upstream changes in @Creanova/extensions.'
        )
    ...
    @patch('integrations.github.github_manager.Auth')
    @patch('integrations.github.github_manager.GithubIntegration')
    async def test_receive_message_ignores_Creanova_bot_sender_before_processing(
        self,
        mock_github_integration,
        mock_auth,
        mock_token_manager,
...
class TestReceiveMessagePayloadProcessingError:
    ...
    async def test_payload_processing_failure_logs_error(
        ...
        # The failure is logged at exception level so the stack trace is preserved automatically.
        exception_calls = [
            call
...
class TestGetUserNotFoundMessageIntegration:
    ...
    def test_message_mentions_Creanova_cloud(self):
        """Test that the message directs users to Creanova Cloud."""
        message = get_user_not_found_message('testuser')
        assert 'Creanova Cloud' in message
    ...
    def test_message_is_friendly_and_informative(self):
        ...
        assert "haven't created an Creanova account" in message.lower()
