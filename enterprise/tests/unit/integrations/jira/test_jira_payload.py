...
@pytest.fixture
def parser():
    """Create a JiraPayloadParser with standard Creanova labels."""
    return JiraPayloadParser(oh_label='Creanova', inline_oh_label='@Creanova')


@pytest.fixture
def valid_label_payload():
    """Create a valid jira:issue_updated payload with Creanova label."""
    return {
...
def valid_comment_payload():
    """Create a valid comment_created payload with Creanova mention."""
    return {
        ...
        'comment': {
            'body': '@Creanova please fix this bug',
            'author': {
                'displayName': 'Test User',
                'accountId': 'account-123',
                'emailAddress': 'test@example.com',
...
class TestLabelFiltering:
    """Tests for Creanova label filtering."""

    def test_label_event_without_Creanova_label_skipped(
        self, parser, valid_label_payload
    ):
        """Verify label events without Creanova label are skipped."""
        ...
        # Assert
        assert isinstance(result, JiraPayloadSkipped)
        assert 'Creanova' in result.skip_reason


class TestCommentFiltering:
    """Tests for Creanova comment mention filtering."""

    def test_comment_without_mention_skipped(self, parser, valid_comment_payload):
        """Verify comments without Creanova mention are skipped."""
