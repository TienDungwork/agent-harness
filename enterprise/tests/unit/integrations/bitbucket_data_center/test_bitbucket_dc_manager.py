...
def _comment_message(*, body: str = 'Hey @Creanova fix') -> Message:
    return Message(
...
@pytest.mark.asyncio
@pytest.mark.parametrize('actor_slug', ['Creanova', 'Creanova'])
async def test_receive_message_skips_event_authored_by_bot(actor_slug):
    ...
    The agent's reply is posted via the bot PAT and can contain "@Creanova";
...
def test_posting_service_raises_when_bot_token_unset():
    ...
    reply with "@Creanova" would re-fire the webhook), so building the posting
