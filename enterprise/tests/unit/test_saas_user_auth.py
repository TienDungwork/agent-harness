...
from Creanova.app_server.user_auth.user_auth import AuthType
...
def mock_config():
    ...
    from Creanova.app_server.utils.encryption_key import EncryptionKey
...
# =============================================================================
# Tests for Creanova_API_KEY injection
# =============================================================================


class TestCreanovaApiKey:
    """Tests for Creanova_API_KEY system secret generation and injection."""

    @pytest.mark.asyncio
    async def test_get_Creanova_api_key_creates_system_key(self):
        """Test that _get_Creanova_api_key creates a system key via ApiKeyStore."""
        user_id = 'test_user_id'
        org_id = uuid.uuid4()
        ...
        with (
            ...
            # Act
            result = await user_auth._get_Creanova_api_key()
            ...
            mock_api_key_store.get_or_create_system_api_key.assert_called_once_with(
                user_id=user_id,
                org_id=org_id,
                name='Creanova_API_KEY',
            )

    @pytest.mark.asyncio
    async def test_get_Creanova_api_key_raises_for_missing_user(self):
        """Test that _get_Creanova_api_key raises ValueError if user not found.
        ...
        with patch('server.auth.saas_user_auth.UserStore') as mock_user_store:
            ...
            with pytest.raises(
                ...
                await user_auth._get_Creanova_api_key()

    @pytest.mark.asyncio
    async def test_get_Creanova_api_key_raises_for_user_without_org(self):
        """Test that _get_Creanova_api_key raises ValueError if user has no org."""
        user_id = 'test_user_id'
    ...
    @pytest.mark.asyncio
    async def test_get_secrets_includes_Creanova_api_key(self):
        """Test that get_secrets injects Creanova_API_KEY into custom_secrets."""
        user_id = 'test_user_id'
        org_id = uuid.uuid4()
        expected_api_key = 'sk-oh-test-key-12345'
        ...
        # Create mock secrets from store (without Creanova_API_KEY)
        ...
        with (
            ...
            assert 'Creanova_API_KEY' in result.custom_secrets
            assert (
                result.custom_secrets['Creanova_API_KEY'].secret.get_secret_value()
                == expected_api_key
            )
            assert (
                'system-managed'
                in result.custom_secrets['Creanova_API_KEY'].description
            )
    ...
    async def test_get_secrets_handles_empty_stored_secrets(self):
        ...
        with (
            ...
            # Assert - should have only Creanova_API_KEY
            assert result is not None
            assert 'Creanova_API_KEY' in result.custom_secrets
