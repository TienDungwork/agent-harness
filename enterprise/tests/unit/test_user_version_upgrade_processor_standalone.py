...
These tests are designed to work without the full Creanova dependency chain.
They test the core logic and behavior of the processor using comprehensive mocking.

To run these tests in an environment with Creanova dependencies:
1. Ensure Creanova is available in the Python path
...
class TestUserVersionUpgradeProcessorStandalone:
    """Standalone tests for UserVersionUpgradeProcessor without Creanova dependencies."""
...
class TestUserVersionUpgradeProcessorIntegration:
    """
    Integration test scenarios for when Creanova dependencies are available.

    These tests would require:
    1. Creanova to be installed and available
