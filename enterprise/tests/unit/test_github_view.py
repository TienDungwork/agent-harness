...
class TestGithubLabels(TestCase):
    def test_labels_with_staging(self):
        ...
        self.assertEqual(oh_label, 'Creanova-exp')
        self.assertEqual(inline_oh_label, '@Creanova-exp')
    ...
    def test_labels_with_prod(self):
        ...
        self.assertEqual(oh_label, 'Creanova')
        self.assertEqual(inline_oh_label, '@Creanova')
    ...
    @mock.patch.dict('os.environ', {'OH_RESOLVER_LABEL': 'Creanova-dev'})
    def test_labels_with_override(self):
        """An explicit OH_RESOLVER_LABEL overrides host inference."""
        oh_label, inline_oh_label = get_oh_labels('app.all-hands.dev')
        self.assertEqual(oh_label, 'Creanova-dev')
        self.assertEqual(inline_oh_label, '@Creanova-dev')
...
class TestGithubCommentCaseInsensitivity(TestCase):
    @mock.patch('integrations.github.github_view.INLINE_OH_LABEL', '@Creanova')
    def test_issue_comment_case_insensitivity(self):
        ...
        message_lower = Message(
            ...
            message={
                'payload': {
                    'action': 'created',
                    'comment': {'body': 'hello @Creanova please help'},
                    'issue': {'number': 1},
                }
            },
        )
