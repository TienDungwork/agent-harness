...
async def test_update_org(async_session_maker, mock_litellm_api):
    ...
    async with async_session_maker() as session:
        ...
        org = Org(
            name='test-org',
            agent_settings=CreanovaAgentSettings(agent='CodeActAgent'),
        )
        session.add(org)
        await session.commit()
        await session.refresh(org)
    ...
    with (
        ...
        assert agent_settings.llm.model == 'Creanova/claude-3'
...
def test_get_org_settings_from_org_use_persisted_loaders():
    ...
    loaded_agent_settings = CreanovaAgentSettings(agent='MigratedAgent')
...
def test_get_agent_settings_from_org_preserves_acp_variant():
    ...
    coerced into ``CreanovaAgentSettings`` — that coercion 500'd on the
...
def test_merge_and_validate_settings_switches_variant_without_mongrel():
    ...
    merged = OrgStore._merge_and_validate_settings(
        {'agent_kind': 'Creanova', 'llm': {'model': 'gpt'}},
...
async def test_update_org_defaults_async_with_llm_api_key():
    ...
    mock_org = Org(
        id=org_id,
        name='Test Organization',
        agent_settings=CreanovaAgentSettings(llm={'model': 'old-model'}),
    )
...
async def test_update_org_defaults_async_propagates_managed_key_reset():
    ...
    mock_org = Org(
        id=org_id,
        name='Test Organization',
        agent_settings=CreanovaAgentSettings(llm={'model': 'Creanova/claude-3'}),
    )
    update_data = OrgUpdate(
        agent_settings_diff={'llm': {'model': 'Creanova/claude-3'}}
    )
