...
def get_Creanova_cloud_key_alias(keycloak_user_id: str, org_id: str) -> str:
    """Generate the key alias for Creanova Cloud managed keys."""
    return f'Creanova Cloud - user {keycloak_user_id} - org {org_id}'
...
class LiteLlmManager:
    ...
    async def create_entries(
        ...
        if not local_deploy:
            ...
            async with httpx.AsyncClient(
                ...
                if add_user_to_team:
                    if create_user:
                        # create_user is True only when no Creanova User row exists,
                    ...
                    key_alias = get_Creanova_cloud_key_alias(keycloak_user_id, org_id)
                    try:
                ...
                if db_key:
                    ...
                    if not key_valid:
                        ...
                        new_key = await LiteLlmManager._generate_key(
                            ...
                            keycloak_user_id,
                            org_id,
                            get_Creanova_cloud_key_alias(keycloak_user_id, org_id),
                            None,
                        )
                        logger.info(
    ...
    async def _verify_existing_key(
        ...
        the given user and organization. For Creanova_type=True, looks for keys
        with metadata type='Creanova' and matching team_id. For Creanova_type=False,
        ...
        for key_info in keys:
            ...
            if (
                Creanova_type
                and metadata.get('type') == 'Creanova'
                and team_id == org_id
            ):
                # Found an existing Creanova key for this org
