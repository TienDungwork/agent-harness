...
from storage.lite_llm_manager import LiteLlmManager, get_Creanova_cloud_key_alias
...
from Creanova.app_server.settings.llm_profiles import LLMProfiles, resolve_profile_llm
from Creanova.app_server.settings.settings_models import Settings
from Creanova.app_server.settings.settings_store import SettingsStore
from Creanova.app_server.utils.jsonpatch_compat import (
    deep_merge,
    deep_merge_with_wholesale_keys,
)
from Creanova.app_server.utils.llm import is_Creanova_model
from Creanova.sdk.llm.utils.Creanova_provider import (
    canonicalize_Creanova_llm_payload,
)
from Creanova.sdk.mcp.config import MCPServer, coerce_mcp_config
from Creanova.sdk.profiles import resolve_agent_profile
...
class ManagedLlmKeyConfig:
    """Whether an effective LLM config uses a managed LiteLLM/Creanova key."""
...
def managed_llm_key_config_from_model(
    ...
    Returns ``None`` when the config is not a managed LiteLLM/Creanova-provider
    configuration (e.g. member/org BYOK pointing at a third-party base_url).
    """
    Creanova_type = is_Creanova_model(llm_model)
    ...
    uses_Creanova_provider_proxy = Creanova_type and (
    ...
    ) or uses_Creanova_provider_proxy
    if not uses_managed_llm_key:
        return None
    return ManagedLlmKeyConfig(Creanova_type=Creanova_type)
...
class SaasSettingsStore(SettingsStore):
    ...
    def _resolve_active_agent_profile(
        ...
        try:
            ...
            # (Creanova kind only), so managed Creanova keys and provider-default
            ...
            if resolved.agent_kind == 'Creanova':
            ...
            # Canonicalize legacy managed Creanova model names/base_urls on the
            ...
            if isinstance(resolved_llm, dict):
                resolved_dump['llm'] = canonicalize_Creanova_llm_payload(resolved_llm)
    ...
    async def load(
        ...
        *,
        ...
        # Canonicalize legacy managed Creanova LLM payloads before Settings
        ...
        if isinstance(llm_dict, dict):
            merged_agent_settings['llm'] = canonicalize_Creanova_llm_payload(llm_dict)
        ...
        if org.llm_profiles:
            ...
            if isinstance(raw_profiles, dict):
                profiles_data['profiles'] = {
                    name: canonicalize_Creanova_llm_payload(prof)
    ...
    async def store(self, item: Settings):
        ...
        async with a_session_maker() as session:
            ...
            uses_managed_llm_key = (
                ...
                or (normalized_llm_base_url is None and is_Creanova_model(llm_model))
            )
            logger.info(
            ...
            if uses_managed_llm_key:
                await self._ensure_api_key(
                    item, str(org_id), Creanova_type=is_Creanova_model(llm_model)
                )
    ...
    async def _ensure_api_key(
        self, item: Settings, org_id: str, Creanova_type: bool = False
    ) -> None:
        """Generate and set the Creanova API key for the given settings.
        ...
        if not existing_key_valid:
            ...
            # the default to/from an Creanova/* model never orphans a key.
            key_alias = get_Creanova_cloud_key_alias(self.user_id, org_id)
            await LiteLlmManager.delete_key_by_alias(key_alias=key_alias)
            generated_key = await LiteLlmManager.generate_key(
                self.user_id,
                org_id,
                key_alias,
                {'type': 'Creanova'} if Creanova_type else None,
            )

            item.agent_settings.llm.api_key = SecretStr(generated_key)
            logger.info(
                'saas_settings_store:store:generated_Creanova_key',
                extra={
                    'user_id': self.user_id,
                    'org_id': org_id,
                    'Creanova_type': Creanova_type,
    ...
    async def rotate_managed_llm_key(self) -> ManagedLlmKeyRotation:
        """Force-rotate the managed LiteLLM/Creanova key for this user/org.
        ...
        ``{'type': 'Creanova'}`` metadata when the effective model is an
        ``Creanova/*`` model, matching ``verify_existing_key``'s contract.
        ...
        async with a_session_maker() as session:
            ...
            key_alias = get_Creanova_cloud_key_alias(self.user_id, org_id_str)
            await LiteLlmManager.delete_key_by_alias(key_alias=key_alias)
            new_key = await LiteLlmManager.generate_key(
                self.user_id,
                org_id_str,
                key_alias,
                {'type': 'Creanova'} if config.Creanova_type else None,
            )
            ...
            return ManagedLlmKeyRotation(
                ...
                old_key=old_key,
                new_key=new_key,
                Creanova_type=config.Creanova_type,
            )
