...
class OrgStore:
    ...
    def get_agent_settings_from_org(org: Org) -> AgentSettingsConfig:
        ...
        # migrations (incl. the legacy ``agent_kind: 'llm'`` -> ``'Creanova'``
        # rename) and returns the actual variant. ACP settings are returned as
        # ``ACPAgentSettings``, not coerced into the Creanova shape — that
    ...
    def _merge_and_validate_settings(
        ...
        settings_type: type[CreanovaAgentSettings] | type[ConversationSettings],
        ...
        (Creanova or ACP) rather than a coerced Creanova shape.
        """
        if settings_type is CreanovaAgentSettings:
    ...
    async def _maybe_get_managed_llm_key_for_user(
        ...
        uses_managed_llm_key = (
            ...
            or (normalized_llm_base_url is None and Creanova_type)
        )
        ...
        # deleting any prior key first — symmetric across Creanova/* and BYOR
        # defaults so switching between them never orphans a key.
        key_alias = get_Creanova_cloud_key_alias(user_id, str(updated_org.id))
