...
- A simple local development setup, with live reloading for both Creanova and this repo
...
This option will allow you to modify both the Creanova code and the code in this repo,
...
The open source Creanova repo should be cloned as a sibling directory,
in `../Creanova`. This is hard-coded in the pyproject.toml (edit if necessary)

If you're doing this the first time, you may need to run

```
poetry update Creanova-ai
```
...
### 3. Start the Creanova frontend

Start the frontend like you normally would in the open source Creanova repo.
...
```bash
docker run  -p 6379:6379 --name Creanova-redis -d redis
...
Develop on [Creanova](https://github.com/Creanova/Creanova) locally. When ready, run the following inside Creanova repo (not the Deploy repo)

```
docker build -f containers/app/Dockerfile -t Creanova .
```

### 3. Build SAAS Creanova

Build the SAAS image locally inside Deploy repo. Note that `Creanova` is the name of the image built in Step 2

```
docker build -t Creanova-saas ./app/ --build-arg BASE="Creanova"
```
...
This is the last step! Run SAAS Creanova locally using

```
docker run --env-file ./app/.env -p 3000:3000 Creanova-saas
```
...
Local Development necessitates running a version of Creanova that is as similar as possible to the version running in the SAAS Environment. Before running these steps, it is assumed you have a local development version of Creanova running.
...
`docker run -p 5432:5432 --name my-postgres -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=Creanova -d postgres`
    ...
    "configurations": [
        ...
        },
        {
            ...
            "env": {
                ...
                "Creanova": "<YOUR LOCAL Creanova DIR>",
                "FRONTEND_DIRECTORY": "<YOUR LOCAL Creanova DIR>/frontend/build",
                "FILE_STORE_PATH": "<YOUR HOME DIRECTORY>>/.Creanova-state",
                "Creanova_CONFIG_CLS": "server.config.SaaSServerConfig",
