...
This service syncs users from the Creanova database to a Resend.com audience. It runs as a Kubernetes CronJob that periodically queries the Creanova database and adds any new users to the specified Resend audience.

## Features

- Syncs Creanova users with email addresses to a Resend.com audience
...
**Failure modes:** If the Creanova database is unavailable, the sync job exits with status code 1 and logs the failure. No contacts are synced until the database connection is restored. Monitor the CronJob logs for database connection errors.

## Deployment

The service is deployed as part of the Creanova Helm chart. To enable it, set the following in your values.yaml:
...
- Kubernetes cluster with the Creanova chart deployed
