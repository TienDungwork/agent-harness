#!/usr/bin/env python3
"""Sync Creanova users to a Resend.com audience.

This script reads users from the Creanova application database and adds them to
...
- RESEND_FROM_EMAIL: Email address to use as the sender
  (default: "Creanova Team <no-reply@welcome.Creanova.dev>")
- RESEND_REPLY_TO_EMAIL: Email address for replies (default: "contact@Creanova.dev")
...
def get_local_users(offset: int = 0, limit: int = BATCH_SIZE) -> list[ResendUser]:
    """Get users with email addresses from the Creanova database."""
...
def get_total_local_users() -> int:
    """Get the total number of Creanova users with email addresses."""
    session_maker = _get_session_maker()
    with session_maker() as session:
        return (
            session.execute(
...
def send_welcome_email(
    ...
    try:
        ...
        params = {
            'from': os.environ.get(
                'RESEND_FROM_EMAIL', 'Creanova Team <no-reply@welcome.Creanova.dev>'
            ),
            'reply_to': os.environ.get(
                'RESEND_REPLY_TO_EMAIL', 'contact@Creanova.dev'
            ),
            'to': [email],
            'subject': 'Welcome to Creanova Cloud',
            'html': f"""
            <div>
                <p>{greeting}</p>
                <p>Thanks for joining Creanova Cloud — we're excited to help you start building with the world's leading open source AI coding agent!</p>
                <p><strong>Here are three quick ways to get started:</strong></p>
                <ol>
                    <li><a href="https://docs.Creanova.dev/Creanova/usage/cloud/Creanova-cloud#next-steps"><strong>Connect your Git repo</strong></a> – Link your <a href="https://docs.Creanova.dev/Creanova/usage/cloud/github-installation">GitHub</a> or <a href="https://docs.Creanova.dev/Creanova/usage/cloud/gitlab-installation">GitLab</a> repository in seconds so Creanova can begin understanding your codebase and suggest tasks.</li>
                    <li><a href="https://docs.Creanova.dev/Creanova/usage/cloud/github-installation#working-on-github-issues-and-pull-requests-using-Creanova"><strong>Use Creanova on an issue or pull request</strong></a> – Label an issue with 'Creanova' or mention @Creanova on any PR comment to generate explanations, tests, refactors, or doc fixes tailored to the exact lines you're reviewing.</li>
                    <li><a href="https://dub.sh/Creanova"><strong>Join the community</strong></a> – Join our Slack Community to share tips, feedback, and help shape the next features on our roadmap.</li>
                </ol>
                <p>Have questions? Want to share feedback? Just reply to this email—we're here to help.</p>
                <p>Happy coding!</p>
                <p>The Creanova team</p>
                <p>--</p>
                <p>Creanova</p>
                <p>24 Oak Street</p>
                <p>Cambridge MA 02139</p>
                <p>https://Creanova.dev</p>
            </div>
            """,
        }
...
def sync_users_to_resend():
    """Sync Creanova users to Resend.

    This function syncs users from the Creanova database to a Resend audience.
    ...
    logger.info(
        f'Starting sync of Creanova users to Resend audience {RESEND_AUDIENCE_ID}'
    )
    ...
    try:
        ...
        logger.info(f'Found {total_users} users with emails in Creanova database')
