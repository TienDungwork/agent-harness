...
class GithubManager(Manager[GithubViewType]):
    def __init__(
        ...
        self.jinja_env = Environment(
            loader=FileSystemLoader(Creanova_RESOLVER_TEMPLATES_DIR + 'github')
        )
    ...
    def _send_user_not_found_message(self, message: Message, username: str):
        """Send a message to the user informing them they need to create an Creanova account.

        This method handles all supported trigger types:
        - Labeled issues (action='labeled' with Creanova label)
        - Issue comments (comment containing @Creanova)
        - PR comments (comment containing @Creanova on a PR)
        - Inline PR review comments (comment containing @Creanova)
    ...
    async def is_job_requested(self, message: Message) -> bool:
        ...
        # Suggestions contain `@Creanova` macro; avoid kicking off jobs for system recommendations
    ...
    async def receive_message(self, message: Message):
        ...
        if await self.is_job_requested(message):
            ...
            # Check if the user has an Creanova account
            if not keycloak_user_id:
                logger.warning(
                    f'[GitHub] User {username} (id={user_id}) not found in Keycloak. '
                    f'User must create an Creanova account first.'
                )
    ...
    async def start_job(self, github_view: GithubViewType) -> None:
        """Kick off a job with Creanova agent using V1 app conversation system.
        ...
        try:
            ...
            try:
                ...
                msg_info = f'@{user_info.username} please re-login into [Creanova Cloud]({HOST_URL}) before starting a job.'
                ...
                msg_info = f'@{user_info.username} please set a valid LLM API key in [Creanova Cloud]({HOST_URL}) before starting a job.'
