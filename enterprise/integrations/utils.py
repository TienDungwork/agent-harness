...
from Creanova.app_server.integrations.service_types import Repository
...
def get_session_expired_message(username: str | None = None) -> str:
    ...
    """
    if username:
        return f'@{username} your session has expired. Please login again at [Creanova Cloud]({HOST_URL}) and try again.'
    return f'Your session has expired. Please login again at [Creanova Cloud]({HOST_URL}) and try again.'


def get_user_not_found_message(username: str | None = None) -> str:
    """Get a user-friendly message when a user hasn't created an Creanova account.

    Used by integrations to notify users when they try to use Creanova features
    but haven't logged into Creanova Cloud yet (no Keycloak account exists).
    ...
    """
    if username:
        return f"@{username} it looks like you haven't created an Creanova account yet. Please sign up at [Creanova Cloud]({HOST_URL}) and try again."
    return f"It looks like you haven't created an Creanova account yet. Please sign up at [Creanova Cloud]({HOST_URL}) and try again."


def get_account_not_linked_message(username: str | None = None) -> str:
    """Get a message when a user has an Creanova account but hasn't linked it.
    ...
    """
    if username:
        return f"@{username} you have an Creanova account but haven't linked it to this workspace yet. Please link it at [Creanova Cloud]({HOST_URL}) under Settings > Integrations and try again."
    return f"You have an Creanova account but haven't linked it to this workspace yet. Please link it at [Creanova Cloud]({HOST_URL}) under Settings > Integrations and try again."
...
def get_jira_dc_relink_message(display_name: str | None = None) -> str:
    ...
    return (
        ...
        f'Please re-link in [Creanova Cloud|{HOST_URL}] under '
        f'Settings → Integrations to continue using Creanova from Jira.'
    )


Creanova_RESOLVER_TEMPLATES_DIR = (
    os.getenv('Creanova_RESOLVER_TEMPLATES_DIR')
    or 'Creanova/app_server/integrations/templates/resolver/'
)
_jinja_env = Environment(loader=FileSystemLoader(Creanova_RESOLVER_TEMPLATES_DIR))


def get_oh_labels(web_host: str) -> tuple[str, str]:
    """Get the Creanova labels based on the web host.
    ...
    Returns:
        ...
        - oh_label is OH_RESOLVER_LABEL when set; otherwise 'Creanova-exp' for
          staging/local hosts and 'Creanova' for everything else
    ...
    oh_label = 'Creanova-exp' if is_staging_or_local else 'Creanova'
    inline_oh_label = '@Creanova-exp' if is_staging_or_local else '@Creanova'
...
def has_exact_mention(text: str, mention: str) -> bool:
    ...
    Args:
        ...
        mention: The mention to look for (e.g. "@Creanova")

    Returns:
        bool: True if the exact mention is found, False otherwise

    Example:
        >>> has_exact_mention("Hello @Creanova!", "@Creanova")  # True
        >>> has_exact_mention("Hello @Creanova-agent!", "@Creanova")  # False
        >>> has_exact_mention("(@Creanova)", "@Creanova")  # True
        >>> has_exact_mention("user@Creanova.com", "@Creanova")  # False
        >>> has_exact_mention("Hello @Creanova!", "@Creanova")  # True (case-insensitive)
    """
