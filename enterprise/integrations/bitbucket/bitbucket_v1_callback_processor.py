...
from Creanova.agent_server.models import AskAgentRequest, AskAgentResponse
from Creanova.app_server.event_callback.event_callback_models import (
    EventCallback,
...
from Creanova.app_server.event_callback.util import (
    ensure_conversation_found,
    ensure_running_sandbox,
    get_agent_server_url_from_sandbox,
)
from Creanova.sdk import Event
from Creanova.sdk.event import ConversationStateUpdateEvent

_logger = logging.getLogger(__name__)


class BitbucketV1CallbackProcessor(EventCallbackProcessor):
    ...
    async def __call__(
        ...
        _logger.info('[Bitbucket V1] Callback agent state was %s', event)
        ...
        try:
            ...
            await self._post_summary_to_bitbucket(summary)
            ...
            can_post_error = bool(self.bitbucket_view_data.get('keycloak_user_id'))
            await handle_callback_error(
                ...
                post_error_func=self._post_summary_to_bitbucket,
            )
            return EventCallbackResult(
    ...
    async def _post_summary_to_bitbucket(self, summary: str) -> None:
        from integrations.bitbucket.bitbucket_service import SaaSBitBucketService

        keycloak_user_id = self.bitbucket_view_data.get('keycloak_user_id')
        if not keycloak_user_id:
            raise RuntimeError('Missing keycloak user ID for Bitbucket')

        bitbucket_service = SaaSBitBucketService(external_auth_id=keycloak_user_id)
        await bitbucket_service.reply_to_pr_comment(
            workspace=self.bitbucket_view_data['workspace'],
            repo_slug=self.bitbucket_view_data['repo_slug'],
            pr_id=self.bitbucket_view_data['pr_id'],
            body=summary,
            parent_comment_id=self.bitbucket_view_data.get('parent_comment_id'),
        )
