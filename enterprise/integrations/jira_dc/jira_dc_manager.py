...
from Creanova.app_server.integrations.provider import ProviderHandler
from Creanova.app_server.integrations.service_types import Comment, Repository
from Creanova.app_server.types import (
    LLMAuthenticationError,
    MissingSettingsError,
    SessionExpiredError,
)
from Creanova.app_server.user_auth.user_auth import UserAuth
from Creanova.app_server.utils.http_session import httpx_verify_option
...
# Unicode codepoint of the emoji reaction posted to acknowledge an @Creanova
...
# Events the Creanova webhook subscribes to, used when auto-enrolling the
...
def _comment_addresses_Creanova(comment: str, bot_ids: set[str] | None) -> bool:
    # Literal @Creanova always triggers (works even when the bot identity can't
    ...
    if not comment:
        return False
    # Boundary-aware + case-insensitive: matches @Creanova but not an email like
    # someone@Creanova.dev (incl. the service account's own address).
...
class JiraDcManager(Manager[JiraDcViewInterface]):
    def __init__(self, token_manager: TokenManager):
        ...
        self.jinja_env = Environment(
            loader=FileSystemLoader(Creanova_RESOLVER_TEMPLATES_DIR + 'jira_dc')
        )
    ...
    async def authenticate_user(
        ...
        """Authenticate Jira DC user and get their Creanova user auth."""
        ...
        # email to their Creanova email instead. In OAuth mode we resolve
        ...
        if not JIRA_DC_ENABLE_OAUTH or not jira_dc_user_id or jira_dc_user_id == 'none':
            ...
            # matches an Creanova account is auto-enrolled, no manual link step.
    ...
    def parse_webhook(
        ...
        if event_type == 'comment_created':
            ...
            if not _comment_addresses_Creanova(comment, bot_mentions):
                return None
    ...
    async def receive_message(self, message: Message):
        ...
        if not job_context:
            ...
            # bot itself (name 'Creanova' or a resolved bot id) -- the FDE-84
        ...
        if not jira_dc_user or not saas_user_auth:
            ...
            # Distinguish "no Creanova account" from "account exists but not linked
    ...
    async def start_job(self, jira_dc_view: JiraDcViewInterface) -> None:
        ...
        try:
            ...
            msg_info = f'Please re-login into [Creanova Cloud]({HOST_URL}) before starting a job.'

        except LLMAuthenticationError as e:
            logger.warning(f'[Jira DC] LLM authentication error: {str(e)}')
            msg_info = f'Please set a valid LLM API key in [Creanova Cloud]({HOST_URL}) before starting a job.'
    ...
    async def _resolve_service_account_mentions(self, payload: Dict) -> set[str] | None:
        ...
        Returns the ids a [~...] token can carry; _comment_addresses_Creanova
        matches a token's inner id against these. Gated so only wiki-markup
        mentions pay a lookup; literal @Creanova and non-comment payloads
    ...
    async def get_issue_comments(
        ...
        ``Comment.system`` so the prompt can label them as Creanova' own prior
    ...
    async def _add_acknowledgement_reaction(
        ...
        """Acknowledge the @Creanova mention with a best-effort reaction.
    ...
    async def register_webhook(
        ...
        """Create or update the Creanova webhook in Jira DC via the admin API.
        ...
        Args:
            ...
            events_url: The Creanova endpoint Jira should POST events to.
    ...
    async def delete_webhook(
        ...
        """Delete the Creanova webhook from Jira DC, if present.
        ...
        Args:
            ...
            admin_api_key: A Jira admin PAT. Used only for this call; never
                stored.
            events_url: The Creanova endpoint whose webhook should be removed.
        ...
        try:
            ...
            if not mentioned_repos:
                ...
                comment_msg = (
                    ...
                    'linked to your Creanova user can access it.'
                )
