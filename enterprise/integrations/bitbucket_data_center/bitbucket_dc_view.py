...
# editing an existing @Creanova comment must not start a second job.
...
class BitbucketDCFactory:
    ...
    contains a case-insensitive mention of the configured ``@Creanova``
    ...
    async def create_bitbucket_dc_view_from_payload(
        ...
        ``keycloak_user_id`` is the Creanova user the resolver runs the
