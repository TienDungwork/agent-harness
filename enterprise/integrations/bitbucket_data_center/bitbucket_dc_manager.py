...
class BitbucketDCManager(Manager[BitbucketDCViewType]):
    ...
    def __init__(
        ...
        self.jinja_env = Environment(
            loader=FileSystemLoader(Creanova_RESOLVER_TEMPLATES_DIR + 'bitbucket')
        )
    ...
    def _posting_service(self):
        ...
        App's ``Creanova[bot]`` identity. ``receive_message`` gates on the bot
    ...
    async def receive_message(self, message: Message) -> None:
        ...
        # we'd risk re-firing the webhook on our own "@Creanova" comment, so skip
        ...
        # the bot PAT and can contain "@Creanova"; without this guard it would
