...
class JiraManager(Manager[JiraViewInterface]):
    ...
    def __init__(self, token_manager: TokenManager):
        ...
        self.jinja_env = Environment(
            loader=FileSystemLoader(Creanova_RESOLVER_TEMPLATES_DIR + 'jira')
        )
    ...
    async def _authenticate_user(
        ...
        """Authenticate the Jira user and get Creanova auth."""
        ...
        if not saas_user_auth:
            logger.warning(
                '[Jira] Failed to get Creanova auth',
                extra={
            ...
            await self._send_error_from_payload(
                payload,
                workspace,
                f'User {payload.user_email} is not authenticated with Creanova.',
            )
            return None, None
