...
import httpx
from integrations.azure_devops.azure_devops_view import mark_Creanova_comment
...
class AzureDevOpsV1CallbackProcessor(EventCallbackProcessor):
    ...
    async def __call__(
        ...
        try:
            ...
            can_post_error = bool(self.azure_devops_view_data.get('keycloak_user_id'))
    ...
    async def _post_summary_to_azure_devops(self, summary: str) -> None:
        ...
        keycloak_user_id = self.azure_devops_view_data.get('keycloak_user_id')
        ...
        summary = mark_Creanova_comment(summary)
        ...
        if self.azure_devops_view_data.get('is_pr'):
            thread_id = self.azure_devops_view_data.get('thread_id')
            if thread_id:
                ...
                await azure_service.add_pr_thread(repository, issue_number, summary)
        else:
