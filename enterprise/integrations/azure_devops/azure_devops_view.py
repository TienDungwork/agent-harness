...
from Creanova.app_server.integrations.service_types import Comment
...
from Creanova.sdk import TextContent

OH_LABEL, INLINE_OH_LABEL = get_oh_labels(HOST)
Creanova_COMMENT_MARKER = '<!-- Creanova-azure-devops-resolver -->'


def mark_Creanova_comment(comment: str) -> str:
    if Creanova_COMMENT_MARKER in comment:
        return comment
    return f'{Creanova_COMMENT_MARKER}\n{comment}'


def _is_Creanova_comment(comment: str) -> bool:
    return Creanova_COMMENT_MARKER in comment
...
class AzureDevOpsFactory:
    ...
    def is_pr_comment(message: Message) -> bool:
        ...
        if _is_Creanova_comment(comment):
            return False
