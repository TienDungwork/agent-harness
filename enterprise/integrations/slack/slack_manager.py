...
class SlackManager(Manager[SlackViewInterface]):
    def __init__(self, token_manager):
        ...
        self.login_link = (
            'User has not yet authenticated: [Click here to Login to Creanova]({}).'
        )

        self.jinja_env = Environment(
            loader=FileSystemLoader(Creanova_RESOLVER_TEMPLATES_DIR + 'slack')
        )
    ...
    async def authenticate_user(
        ...
        # We get the user and correlate them back to a user in Creanova - if we can
        slack_user = None
        async with a_session_maker() as session:
            ...
            # slack_view.slack_to_Creanova_user = slack_user # attach user auth info to view
        ...
        if slack_user:
            ...
            # slack_view.saas_user_auth = await self._get_user_auth(slack_view.slack_to_Creanova_user.keycloak_user_id)
    ...
    async def _try_verify_inferred_repo(
        ...
        """
        user = slack_view.slack_to_Creanova_user
    ...
    async def start_job(self, slack_view: SlackViewInterface) -> None:
        ...
        try:
            msg_info = None
            user_info = slack_view.slack_to_Creanova_user
            try:
                logger.info(
                ...
                msg_info = f'{user_info.slack_display_name} please re-login into [Creanova Cloud]({HOST_URL}) before starting a job.'
                ...
                msg_info = f'@{user_info.slack_display_name} please set a valid LLM API key in [Creanova Cloud]({HOST_URL}) before starting a job.'
