...
from Creanova.app_server.config import get_app_conversation_service
from Creanova.app_server.integrations.provider import ProviderHandler
from Creanova.app_server.sandbox.sandbox_models import SandboxStatus
...
from Creanova.app_server.utils.async_utils import GENERAL_TIMEOUT
from Creanova.app_server.utils.logger import Creanova_logger as logger
from Creanova.sdk import ImageContent, TextContent
...
class SlackNewConversationView(SlackViewInterface):
    ...
    user_msg: str
    slack_user_id: str
    slack_to_Creanova_user: SlackUser
    ...
    async def _get_instructions(self, jinja_env: Environment) -> tuple[str, str]:
        ...
        user_info: SlackUser = self.slack_to_Creanova_user
    ...
    async def save_slack_convo(self):
        if self.slack_to_Creanova_user:
            user_info: SlackUser = self.slack_to_Creanova_user
    ...
    async def create_or_update_conversation(self, jinja: Environment) -> str:
        ...
        if self._resolved_git_provider and self.selected_repo:
            self.resolved_org_id = await resolve_org_for_repo(
                ...
                keycloak_user_id=self.slack_to_Creanova_user.keycloak_user_id,
            )
    ...
    async def _create_v1_conversation(self, jinja: Environment) -> None:
        ...
        start_request = AppConversationStartRequest(
            ...
            title=f'Slack conversation from {self.slack_to_Creanova_user.slack_display_name}',
...
class SlackUpdateExistingConversationView(SlackNewConversationView):
    ...
    async def send_message_to_v1_conversation(self, jinja: Environment):
        ...
        from Creanova.agent_server.models import SendMessageRequest
...
class SlackFactory:
    ...
    async def create_slack_view_from_payload(
        ...
        # Determine if this is a known slack user by Creanova
        ...
        if conversation:
            ...
            return SlackUpdateExistingConversationView(
                ...
                user_msg=user_msg,
                slack_user_id=slack_user_id,
                slack_to_Creanova_user=slack_user,
