...
from Creanova.app_server.user_auth import get_user_auth, get_user_id
...
async def get_admin_user_id(
    ...
    """
    Dependency that validates user has @Creanova.dev email domain.
    ...
    by checking for @Creanova.dev email domain.
    ...
    Raises:
        HTTPException: 403 if email domain is not @Creanova.dev
    ...
    if not user_email.endswith('@Creanova.dev'):
        logger.warning(
        ...
        raise HTTPException(
            ...
            detail='Access restricted to @Creanova.dev users',
        )
...
async def get_org_creator_user_id(
    ...
    - When **disabled** (the default), this falls back to ``get_admin_user_id``
      so org creation remains restricted to ``@Creanova.dev`` admin users.
