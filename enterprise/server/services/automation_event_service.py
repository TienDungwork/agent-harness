...
1. Resolves Git org → Creanova org_id (via cached OrgGitClaim lookup)
...
class AutomationEventService:
    ...
    async def forward_event(
        ...
        try:
            ...
            logger.exception(
                ...
                f'{provider.value} event (org_id={org_id})',
    ...
    async def forward_jira_dc_event(
        ...
        Jira DC workspaces are configured directly in Creanova, so the route
        resolves the Creanova org from the workspace instead of using the
    ...
    async def _resolve_default_org_fallback(
        ...
        Applies only when the default org is enabled (Creanova_DEFAULT_ORG_*)
    ...
    def _build_event_payload(
        ...
        return {
            'organization': {
                ...
                'Creanova_org_id': str(org_context.org_id),
            },
            'payload': payload,
        }
    ...
    async def _resolve_git_org(
        ...
        """
        Resolve a Git organization name to an Creanova org_id.
    ...
    async def _resolve_personal_org(
        ...
        """
        Resolve a provider user to their personal Creanova org.

        For personal repos (owner type is 'User'), the Creanova org_id
    ...
    async def _send_source_to_automation_service(
        ...
        automation service can verify it came from the Creanova server.

        Args:
            source: The automation event source
            org_id: The Creanova organization ID
