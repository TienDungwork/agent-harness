...
# or {some-text}.staging.Creanova.dev (the domain the staging/feature stack is
...
# staging.Creanova.dev) does not count as a feature environment.
STAGING_BASE_HOSTS = ('staging.all-hands.dev', 'staging.Creanova.dev')
IS_STAGING_ENV = bool(
    re.match(r'^.+\.staging\.(?:all-hands|Creanova)\.dev$', HOST)
...
def _is_all_hands_managed_domain(host: str) -> bool:
    ...
    return (
        ...
        or host == 'app.Creanova.ai'
        or host.endswith('.all-hands.dev')
        or host.endswith('.Creanova.ai')
        or host.endswith('.Creanova.dev')
    )
...
Creanova_LLM_PROVIDER_ROUTE = os.getenv('Creanova_LLM_PROVIDER_ROUTE')
Creanova_DEFAULT_LLM_MODEL = os.getenv('Creanova_DEFAULT_LLM_MODEL') or os.getenv(
    'LLM_MODEL'
)
Creanova_DEFAULT_LLM_BASE_URL = os.getenv(
    'Creanova_DEFAULT_LLM_BASE_URL'
) or os.getenv('LLM_BASE_URL')
Creanova_DEFAULT_LLM_API_KEY = os.getenv('Creanova_DEFAULT_LLM_API_KEY') or os.getenv(
    'LLM_API_KEY'
)
...
# (the default), org creation remains restricted to @Creanova.dev admin users.
# Accepts both ``'true'`` and ``'1'`` because older Helm chart versions emit
# the latter and would otherwise silently keep the feature disabled.
...
def should_use_direct_llm_defaults() -> bool:
    ...
    return (
        Creanova_LLM_PROVIDER_ROUTE == 'direct'
        and bool(Creanova_DEFAULT_LLM_MODEL)
        and bool(Creanova_DEFAULT_LLM_BASE_URL)
    )


def get_default_llm_model() -> str:
    """Return the deployment default LLM model."""
    if should_use_direct_llm_defaults() and Creanova_DEFAULT_LLM_MODEL:
        return Creanova_DEFAULT_LLM_MODEL
    return get_default_litellm_model()


def get_default_llm_base_url() -> str:
    """Return the deployment default LLM base URL."""
    if should_use_direct_llm_defaults() and Creanova_DEFAULT_LLM_BASE_URL:
        return Creanova_DEFAULT_LLM_BASE_URL
    return LITE_LLM_API_URL


def get_default_llm_api_key() -> str | None:
    """Return the optional shared deployment default LLM API key."""
    if not should_use_direct_llm_defaults() or not Creanova_DEFAULT_LLM_API_KEY:
        return None
    key = Creanova_DEFAULT_LLM_API_KEY.strip()
    return key or None
