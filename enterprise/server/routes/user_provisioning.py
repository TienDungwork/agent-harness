...
4. Create the Creanova user record (mirroring the ``keycloak_callback``
...
* ``Creanova_user_id`` — set once ``UserStore.create_user`` returns,
...
(user-first) would leak the *entire* Creanova cascade instead of a
...
Creanova ``OrgMember`` row does not propagate to LiteLLM, so on
...
async def provision_user(
    ...
    # have not mutated any Creanova state yet, so a clean error is
    # safe to return.
    try:
    ...
    # Anything after this point may need to roll back partial Creanova
    # state (DB rows, LiteLLM team, OrgMember rows) **and** the Keycloak
    # user, to avoid leaving an Creanova ``User``/personal-``Org``
    ...
    Creanova_user_id: UUID | None = None
    ...
    try:
        ...
        # 3. Create the Creanova user. Mirrors ``keycloak_callback``'s
        ...
        if target_org_id != Creanova_user_id:
            ...
            # Track separately from ``Creanova_user_id`` so the unwind
        ...
        await _rollback_partial_provision(
            ...
            Creanova_user_id=Creanova_user_id,
...
async def _rollback_partial_provision(
    *,
    ...
    3. **Keycloak user.** Last, so the local Creanova identity is
    ...
    if Creanova_user_id is not None and target_membership_added:
        try:
            await OrgMemberStore.remove_user_from_org(target_org_id, Creanova_user_id)
        except Exception:
            logger.exception(
                ...
                extra={
                    'kc_user_id': kc_user_id,
                    'target_org_id': str(target_org_id),
                    'Creanova_user_id': str(Creanova_user_id),
                },
                stack_info=True,
            )
    ...
    if Creanova_user_id is not None:
        try:
            await OrgStore.delete_org_cascade(
                Creanova_user_id, requester_user_id=kc_user_id
            )
        except Exception:
            logger.exception(
    ...
    # 3. Keycloak user. Always last; runs even when no Creanova DB
