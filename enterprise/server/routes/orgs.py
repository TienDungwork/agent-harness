...
async def get_org_members_financial(
    ...
    - Organization Admins
    - Organization Owners
    - Creanova members (users with @Creanova.dev emails)
    ...
    Raises:
        HTTPException: 401 if user is not authenticated
        HTTPException: 403 if user lacks access (not admin/owner and not @Creanova.dev)
...
async def switch_org(
    ...
    try:
        ...
        if analytics:
            try:
                from Creanova.analytics import resolve_analytics_context
...
async def get_git_claims(
    ...
    """Get all Git organization claims for an Creanova organization.

    Only admin and owner roles can view Git organization claims.

    Args:
        org_id: Creanova organization UUID
...
async def claim_git_organization(
    ...
    """Claim a Git organization for an Creanova organization.

    Only admin and owner roles can claim Git organizations.
    A Git organization can only be claimed by one Creanova organization at a time.
...
async def disconnect_git_organization(
    ...
    """Remove a Git organization claim from an Creanova organization.
