...
class Permission(str, Enum):
    ...
    # User provisioning (create new Keycloak/Creanova users directly in an org)
...
async def require_financial_data_access(
    ...
    2. User has @Creanova.dev email domain
    ...
    # Check if user has @Creanova.dev email
    user_auth = await get_user_auth(request)
    user_email = await user_auth.get_user_email()

    if user_email and user_email.endswith('@Creanova.dev'):
        logger.debug(
            'Financial data access granted via @Creanova.dev email',
    ...
    if user_role.name not in (RoleName.OWNER.value, RoleName.ADMIN.value):
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail='Access restricted to organization admins, owners, or Creanova members',
        )
