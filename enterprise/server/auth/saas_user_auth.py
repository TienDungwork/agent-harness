...
from Creanova.app_server.user_auth.user_auth import AuthType, UserAuth
...
class SaasUserAuth(UserAuth):
    ...
    async def get_secrets(self):
        ...
        # Inject Creanova_API_KEY (system-level, lazily generated)
        Creanova_api_key = await self._get_Creanova_api_key()
        if Creanova_api_key:
            custom_secrets = dict(user_secrets.custom_secrets) if user_secrets else {}
            custom_secrets['Creanova_API_KEY'] = CustomSecret(
                secret=SecretStr(Creanova_api_key),
                description='Creanova Cloud API Key for automations and integrations (system-managed)',
            )
    ...
    async def _get_Creanova_api_key(self) -> str:
        """Get or create the user's Creanova_API_KEY (system-level, non-deletable).
        ...
        api_key_store = ApiKeyStore.get_instance()
        Creanova_api_key = await api_key_store.get_or_create_system_api_key(
            user_id=self.user_id,
            org_id=effective_org_id,
            name='Creanova_API_KEY',
        )
        return Creanova_api_key
