...
from Creanova.app_server.utils.llm import ModelsResponse, get_supported_llm_models
...
# ``Creanova/`` -> ``litellm_proxy/`` at the transport boundary.
_Creanova_PREFIX = 'Creanova/'
...
def _derive_default_model() -> str:
    ...
    the app stores and displays models as ``Creanova/<name>``. An already-
    ``Creanova/``-prefixed default is kept as-is; a bare name is prefixed.
    """
    default = get_default_litellm_model()
    if default.startswith(_LITELLM_PROXY_PREFIX):
        return _Creanova_PREFIX + default[len(_LITELLM_PROXY_PREFIX) :]
    if default.startswith(_Creanova_PREFIX):
        return default
    return _Creanova_PREFIX + default
...
class LiteLLMProxyModelService(DefaultLLMModelService):
    ...
    async def _fetch_proxy_model_names(
        ...
        Entries flagged ``model_info.Creanova_hidden`` (legacy alias routes
        ...
        ``model_info.Creanova_canonical`` naming the visible route it
        ...
        for entry in payload.get('data') or []:
            ...
            hidden = isinstance(model_info, dict) and bool(
                model_info.get('Creanova_hidden')
            )
            if hidden and name not in canonical_by_name:
                canonical = model_info.get('Creanova_canonical')
    ...
    def _build_response(
        ...
        return ModelsResponse(
            models=[_Creanova_PREFIX + name for name in model_names],
            verified_models=list(model_names),
            verified_providers=['Creanova'],
            default_model=_derive_default_model(),
            hidden_models=[
                _Creanova_PREFIX + name for name in hidden_model_names or []
            ],
            hidden_model_canonicals={
                _Creanova_PREFIX + alias: _Creanova_PREFIX + canonical
    ...
    async def _union_with_catalogue(
        ...
        extra = [
            ...
            if not m.startswith(_Creanova_PREFIX) and m not in proxy_models
        ]
        return ModelsResponse(
    ...
    def _is_model_verified(
        ...
        # is not. Require the Creanova/ prefix so a catalogue model whose bare
        # name collides with a proxy one (openai/gpt-5 vs proxy gpt-5) isn't
        # falsely verified.
        return model_name.startswith(_Creanova_PREFIX) and name in set(
