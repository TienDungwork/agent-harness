...
export const useCreateConversation = () => {
  ...
  return useMutation({
    ...
    mutationFn: async (
      ...
      // /api/agent-profiles in Creanova #15060, #3730). Await the list from
      ...
      // Creanova profile whose `llm_profile_ref` can point at an LLM profile
      ...
      if (
        ...
        // The seeded Creanova `default` profile is the enriched baseline, not a
        ...
        //
        // Scoped to Creanova: an ACP `default` must keep the profile path.
        // Activation is pointer-only, so global agent_settings is stale (often
        // still Creanova) when an ACP profile is active — launching it via
      ...
      // A launch from a named Creanova profile runs that profile's
      ...
      // Creanova cloud pattern: when the start task isn't immediately
