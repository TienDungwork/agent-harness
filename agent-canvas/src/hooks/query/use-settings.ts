...
import { fromApiAgentKind } from "#/api/agent-kind";
...
const normalizeSettingsResponse = (settings: Partial<Settings>): Settings => {
  ...
  const normalizedAgentSettings = {
    ...(settings.agent_settings ?? DEFAULT_SETTINGS.agent_settings),
    ...agentSettings,
    agent_kind: fromApiAgentKind(
      (agentSettings.agent_kind as string | undefined) ??
        (settings.agent_settings as { agent_kind?: string } | undefined)
          ?.agent_kind,
    ),
  };
  ...
  return {
    ...
    agent_settings: normalizedAgentSettings as Settings["agent_settings"],
