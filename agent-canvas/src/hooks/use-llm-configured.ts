...
interface LlmConfiguredResult {
   ...
   * - Creanova agents are ready only once an LLM API key has been saved.
 ...
 * which persists no settings — leaving an Creanova agent without an API key.
 */
export function useLlmConfigured(): LlmConfiguredResult {
  ...
  const referencedLlmProfileName =
    activeAgentProfile?.agent_kind === "Creanova"
  ...
  // `useSettings` maps it to DEFAULT_SETTINGS (no key, Creanova agent) while
  ...
  // an unconfigured Creanova agent and flashes the "LLM not set up" banner.
