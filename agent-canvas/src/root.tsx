...
function FirstRunOnboardingScreen({ onClose }: { onClose: () => void }) {
  ...
  // matches the UX expectation for canvas.Creanova.dev where Cloud is the
  // only backend option.
  if (isLockedToCloud) {
    return (
      <main
...
export default function App() {
  const location = useLocation();
  const localAuthOn = isLocalAuthEnabled();
  const { data: localIsAuthed, isLoading: localAuthLoading } = useIsAuthed();
  const onLocalLoginRoute = location.pathname === "/login";
  ...
  // In locked-to-Cloud mode the `Creanova-onboarded` localStorage flag is
  ...
  // Local Keycloak session gate (Milestone 1). Login route is exempt.
  if (localAuthOn) {
    if (onLocalLoginRoute) {
      return <Outlet />;
    }
    if (localAuthLoading) {
