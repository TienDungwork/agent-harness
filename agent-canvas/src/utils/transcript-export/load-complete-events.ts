...
export const loadCompleteTranscriptEvents = async (
  loadedEvents: CreanovaEvent[],
  searchEvents: SearchTranscriptEvents,
  expectedEventCount?: number,
): Promise<CreanovaEvent[]> => {
  const persistedDescending: CreanovaEvent[] = [];
  ...
  const eventsById = new Map<string, CreanovaEvent>();
  persistedDescending
    .slice()
    .reverse()
    .forEach((event) => {
