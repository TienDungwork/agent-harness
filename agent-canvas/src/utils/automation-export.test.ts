...
const cronAutomation: Automation = {
  ...
  enabled: true,
  repository: "Creanova/agent-canvas",
  branch: "main",
  model: "fast",
  prompt: "Review open pull requests.",
  plugins: ["github:Creanova/extensions"],
  ...
  it("round-trips event trigger fields", () => {
    const exported = serializeAutomation({
      ...
      trigger: {
        ...
        filter: "repository.full_name == 'Creanova/agent-canvas'",
      },
      timezone: undefined,
    });
  ...
  it("reports every malformed field with its path", () => {
    const malformed = {
      ...
      spec: {
        ...
        plugins: ["github:Creanova/extensions", 42],
        model: false,
        timeout: -5,
      },
    };
