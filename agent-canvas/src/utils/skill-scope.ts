...
function isPublicSource(source: string): boolean {
  ...
  return (
    norm.includes("public-skills") || norm.includes("/.Creanova/cache/skills/")
  );
}

function isUserHomeSkillPath(source: string): boolean {
  const norm = normalizePath(source);
  if (/^\/Users\/[^/]+\/\.(agents|Creanova)\//.test(norm)) {
    return true;
  }
  if (/^\/home\/[^/]+\/\.(agents|Creanova)\//.test(norm)) {
    return true;
  }
