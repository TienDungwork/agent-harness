 ...
 *
 * Mirrors Creanova' cloud frontend (parameterized by `app_mode`):
 ...
 *
 * @example
 * constructMicroagentUrl("github", "owner/repo", ".Creanova/microagents/tell-me-a-joke.md")
 * // "https://github.com/owner/repo/blob/main/.Creanova/microagents/tell-me-a-joke.md"
 ...
 * constructMicroagentUrl("bitbucket", "owner/repo", ".Creanova/microagents/docker-helper.md")
 * // "https://bitbucket.org/owner/repo/src/main/.Creanova/microagents/docker-helper.md"
 */
...
export const getRepoMdCreatePrompt = (
  ...
  return `Please explore this repository. Create the file .Creanova/microagents/repo.md with:
            ${
              query
                ? `- ${query}`
...
/**
 * Get the Creanova query string based on the provider
 * @param provider The git provider
 * @returns The query string for searching Creanova repositories
 */
export const getCreanovaQuery = (provider: Provider | null): string => {
  const providerRepositorySuffix: Record<string, string> = {
    gitlab: "Creanova-config",
    azure_devops: "Creanova-config",
    default: ".Creanova",
  } as const;
...
/**
 * Check if a repository has the Creanova suffix based on the provider
 * @param repo The Git repository to check
 * @param provider The git provider
 * @returns True if the repository has the Creanova suffix
 */
export const hasCreanovaSuffix = (
  repo: GitRepository,
  provider: Provider | null,
): boolean => repo.full_name.endsWith(`/${getCreanovaQuery(provider)}`);
