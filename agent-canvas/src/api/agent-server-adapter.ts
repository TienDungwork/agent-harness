...
export function buildRuntimeServicesSystemSuffix(): string | undefined {
  ...
  if (automation?.url_from_agent) {
    lines.push(
      ...
      `    ${automation.description ?? "Creanova Automations service."}`,
    );
    ...
    if (automation.auth_env_var) {
      ...
      // and automation backend (see Creanova-automation auth.py).
      lines.push(
...
export function toAppConversation(
  ...
  // payload accidentally carries an ``acpserver`` tag on an Creanova
...
function buildConfiguredAcpAgentSettings(
  ...
  // ``@Creanova/typescript-client`` (1.24.3) doesn't surface it on
...
function buildConfiguredCreanovaAgentSettings(
  settings: Settings,
): AgentSettingsPayload {
  const agentSettings = toRecord(settings.agent_settings);
...
function buildConfiguredAgentSettings(
  ...
  return isAcpAgent(settings)
    ...
    : buildConfiguredCreanovaAgentSettings(settings);
}
