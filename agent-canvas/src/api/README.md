...
Use an object literal with named export. Use object destructuring for parameters to make calls self-documenting. Prefer typed `@Creanova/typescript-client` classes over generic HTTP calls. If a needed endpoint is missing, add it to `@Creanova/typescript-client` first.

```typescript
// feature-service/feature-service.api.ts
import { FeatureClient } from "@Creanova/typescript-client/clients";
