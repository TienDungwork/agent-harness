import { ConversationClient } from "@Creanova/typescript-client/clients";
import { RemoteEventsList } from "@Creanova/typescript-client/events/remote-events-list";
...
class EventService {
  ...
  static async searchEvents(
  ...
  ): Promise<EventSearchPage<CreanovaEvent>> {
    const active = getActiveBackend().backend;
    ...
    if (active.kind === "cloud") {
      ...
      // require the server-side fix from Creanova/Creanova#14399. If
      ...
      const doCloudSearch = (searchParams: URLSearchParams) =>
        callCloudProxy<EventSearchPage<CreanovaEvent>>({
          backend: active,
          method: "GET",
      ...
      try {
        ...
        console.warn(
          "[EventService] Cloud backend doesn't support pagination filters. " +
            ...
            "Server needs Creanova/Creanova#14399.",
        );
    ...
    return {
      items: (page?.items ?? []) as CreanovaEvent[],
