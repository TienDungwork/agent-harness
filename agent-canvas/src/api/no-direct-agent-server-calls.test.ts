...
describe("agent-server API access", () => {
  it("uses typed @Creanova/typescript-client access instead of ad-hoc HTTP", () => {
    const violations = collectSourceFiles(SRC_ROOT).flatMap((relPath) => {
      ...
      if (
        /from\s+["']@Creanova\/typescript-client\/client\/http-client["']/.test(
          source,
        )
      ) {
