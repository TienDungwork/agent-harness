 ...
 *
 * Mirrors Creanova' cloud flow: POST /api/v1/app-conversations with the
 * `AppConversationStartRequest` payload, returning a
 * `AppConversationStartTask`. The task is initially WORKING; the caller
 * polls `getCloudAppConversationStartTask` (3s cadence per Creanova)
 ...
 * Creanova' `AgentServerConversationService.updateConversationPublicFlag`:
 ...
 * Creanova' `SandboxService.pauseSandbox`:
...
/**
 * Resume a paused cloud sandbox. Mirrors Creanova' `SandboxService.resumeSandbox`
 ...
 * Creanova' `AgentServerConversationService.readConversationFile` — hits
...
/**
 * Fetch a single v1 app-conversation start task. Mirrors Creanova'
