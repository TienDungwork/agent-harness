...
class AgentServerConversationService {
  ...
  static async createConversation(
    ...
    // cloud app-server (Creanova #15060): local threads it through the
    // encrypted-settings builder; cloud sends it as a flat request field.
    agentProfileId?: string,
    agentProfileKind?: AgentKind,
  ): Promise<AppConversationStartTask> {
    if (getActiveBackend().backend.kind === "cloud") {
      // Cloud path mirrors Creanova' frontend: build a flat
  ...
  static async readConversationFile(
    ...
    if (getActiveBackend().backend.kind === "cloud") {
      ...
      // a known absolute path. Mirrors Creanova' readConversationFile.
