...
/**
 * Lifecycle state of a cloud sandbox. Mirrors Creanova' V1SandboxStatus.
...
export interface AppConversationStartRequest {
  ...
  // prompt (Creanova#14640). Omit/null to create a fresh conversation.
  ...
  // Accepted by the cloud app-server (Creanova #15060) on
...
export interface AppConversation {
  ...
  pr_number: number[];
  /**
   * High-level kind of the conversation's agent — ``"Creanova"`` for an LLM-
   ...
   * ``acpserver`` form. ``null`` for Creanova conversations and for ACP
  ...
  /**
   * Cloud-only sandbox lifecycle status. Mirrors Creanova' V1SandboxStatus.
