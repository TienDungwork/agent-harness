import { SettingsClient } from "@Creanova/typescript-client/clients";
 ...
 * SDK `Creanova.sdk.settings.AppPreferences`). Mirrored here as a flat
...
class SettingsService {
  ...
  static async saveSettings(
    ...
    if (agentSettingsDiff && Object.keys(agentSettingsDiff).length > 0) {
      const mappedDiff = { ...agentSettingsDiff };
      if ("agent_kind" in mappedDiff) {
        mappedDiff.agent_kind = toApiAgentKind(
          mappedDiff.agent_kind as string,
        ) as SettingsValue;
      }
      payload.agent_settings_diff = mappedDiff;
    }
    ...
    // ``Creanova.agent_server.persistence.models._deep_merge``). That works
