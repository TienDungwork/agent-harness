 ...
 * Creanova branch omits `llm_profile_ref` — the profile editor supplies it.
 */
 ...
 * shell string. Creanova: reuses the schema-driven ``tool_concurrency_limit``
...
export function AgentSettingsScreen({
  ...
  // --- Parallel tool calls (Creanova path) ---
  ...
  // --- ACP path ---
  const [agentType, setAgentType] = useState<AgentType>("Creanova");
  ...
  // Dirty tracking: for Creanova path, also check sub-agents toggle and the
  // parallel-tool-calls input.
  const isCreanovaDirty =
    !isAcp &&
    (subAgentsEnabled !== initialSubAgentsEnabled ||
      toolConcurrency !== initialToolConcurrency);
  const settingsDirty = isDirty || isCreanovaDirty;
  ...
  const handleSave = async () => {
    ...
    if (isAcp) {
      ...
      // Creanova path: save agent_kind + sub-agents toggle + parallel tool calls
  ...
  // Sub-agents field metadata for Creanova section
    ...
    >
      ...
      <SettingsDropdownInput
        ...
        items={[
          {
            key: "Creanova",
            label: t(I18nKey.SETTINGS$AGENT_TYPE_Creanova),
          },
