...
interface MessagesProps {
  messages: CreanovaEvent[]; // UI events (actions replaced by observations)
  allEvents: CreanovaEvent[]; // Full event history (for action lookup)
}

const getLastEventId = (events: CreanovaEvent[]) => events.at(-1)?.id;
const getLastEvent = (events: CreanovaEvent[]) => events.at(-1);
