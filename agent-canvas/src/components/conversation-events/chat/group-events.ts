import { ActionEvent, CreanovaEvent } from "#/types/agent-server/core";
import {
  isActionEvent,
  isObservationEvent,
 ...
 */
export const isGroupableEvent = (event: CreanovaEvent): boolean => {
...
export type RenderedItem =
  | { kind: "single"; event: CreanovaEvent; index: number }
  | { kind: "thought"; action: ActionEvent; index: number }
  | { kind: "group"; events: CreanovaEvent[]; startIndex: number };
...
export const groupEvents = (
  events: CreanovaEvent[],
  minSize: number = EVENT_GROUP_MIN_SIZE,
  allEvents: CreanovaEvent[] = events,
): RenderedItem[] => {
  if (minSize < 1) {
  ...
  let run: { events: CreanovaEvent[]; startIndex: number } | null = null;
