 ...
 * settings form (embedded) plus a profile name and — for Creanova profiles —
 * an LLM-profile picker + optional API key that writes to the selected LLM
 * profile. Available on local backends only.
 */
export function AgentProfilesLocalView() {
  ...
  const activateLlmProfile = useActivateLlmProfile();
  ...
  // Typed key for the selected LLM profile. Empty = leave that profile's key
  // unchanged (same "no change" UX as the LLM profile editor).
  ...
  const selectedLlmProfile = llmProfiles.find((p) => p.name === llmProfileRef);
  const selectedLlmApiKeySet = selectedLlmProfile?.api_key_set === true;
  ...
  const handleAddProfile = useCallback(() => {
    ...
    setOverride({ agent_kind: "Creanova" });
  ...
  const handleLlmProfileChange = useCallback((key: Key | null) => {
    if (!key) return;
    setLlmProfileRef(String(key));
    setLlmApiKey("");
  }, []);
  ...
  const handleSave = useCallback(async () => {
    ...
    const typedApiKey = llmApiKey.trim();
    ...
    try {
      ...
      // Optional: write a newly typed API key onto the referenced LLM profile
      // (blank field = preserve existing key). Fail before saving the agent
      // profile so we don't leave a half-applied launch setup.
      if (typedApiKey && llmProfileRef && saveControl.agentType !== "acp") {
        ...
        // Keep global agent_settings.llm in sync when the referenced profile
        // is the active LLM profile (same policy as the LLM profile editor).
        if (llmProfilesData?.active_profile === llmProfileRef) {
          await activateLlmProfile.mutateAsync(llmProfileRef);
        }
      }
    ...
    llmProfileRef,
    llmApiKey,
    llmProfilesData?.active_profile,
    profileName,
    viewMode,
    editingProfile,
    saveProfile,
  ...
  const isCreanova = saveControl?.agentType !== "acp";
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Creanova profiles reference an LLM profile (required). */}
      {isCreanova &&
        (llmProfiles.length > 0 ? (
          <div className="flex flex-col gap-6">
            <SettingsDropdownInput
              ...
              onSelectionChange={handleLlmProfileChange}
            />

            {llmProfileRef ? (
              <>
                <SettingsInput
                  testId="agent-profile-llm-api-key-input"
                  ...
                  placeholder={selectedLlmApiKeySet ? "<hidden>" : ""}
                  onChange={setLlmApiKey}
                  isDisabled={isSaving}
                  startContent={
                    selectedLlmApiKeySet ? (
                      <KeyStatusIcon isSet={selectedLlmApiKeySet} />
                    ) : undefined
                  }
                />
                <HelpLink
                  testId="agent-profile-llm-api-key-help"
                  text={t(I18nKey.SETTINGS$DONT_KNOW_API_KEY)}
                  linkText={t(I18nKey.SETTINGS$CLICK_FOR_INSTRUCTIONS)}
                  href="https://docs.Creanova.dev/usage/local-setup#getting-an-api-key"
                />
              </>
            ) : null}
          </div>
