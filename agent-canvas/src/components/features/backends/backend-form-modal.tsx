...
import { ServerClient } from "@Creanova/typescript-client/clients";
...
import { isCreanovaCloudHost } from "#/api/device-flow-client";
import {
 ...
 * matching (via {@link isCreanovaCloudHost}) rather than a substring test, so
 * a look-alike host such as `all-hands-testing.dev` isn't misread as cloud.
 *
 * This is only a *default*: a self-hosted Creanova Cloud/Enterprise instance
...
function inferKindFromHost(host: string): BackendKind {
  return isCreanovaCloudHost(host) ? "cloud" : "local";
}
...
const DEFAULT_Creanova_CLOUD_HOST = "https://app.all-hands.dev";
    ...
    >
      ...
      >
        ...
        <SettingsInput
          ...
          placeholder={DEFAULT_Creanova_CLOUD_HOST}
          className="w-full"
          showRequiredTag
          error={hostError}
...
/**
 * Manual agent-server connection plus Creanova Cloud OAuth login.
 ...
 * and self-hosted Creanova Cloud with API key auth.
 */
...
/**
 * One-click OAuth login with Creanova Cloud. Includes an "Advanced"
 * disclosure for users who self-host Creanova Cloud and need to override the
 * host.
 */
function CloudLoginColumn({
  ...
  const effectiveHost =
    lockedHost ?? (customHost.trim() || DEFAULT_Creanova_CLOUD_HOST);
