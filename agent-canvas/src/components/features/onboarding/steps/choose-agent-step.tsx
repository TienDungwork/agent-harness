...
function getAgentOptionIcon(id: string): AgentBrandIconKind {
  if (id === "Creanova") return "Creanova";
...
export function AgentOptionIcon({ id, muted }: { id: string; muted: boolean }) {
  ...
  // The Creanova wordmark is wider than the square brand marks (24×16 vs
  ...
  if (icon === "Creanova") {
    return (
      <AgentBrandIcon
        ...
        data-testid="onboarding-agent-icon-Creanova"
      />
    );
  }
...
// ``acp-providers.ts``. The Creanova tile is the only synthetic
  ...
  return [
    {
      id: "Creanova",
      label: "Creanova",
      descriptionKey: I18nKey.ONBOARDING$AGENT_Creanova_DESCRIPTION,
    },
