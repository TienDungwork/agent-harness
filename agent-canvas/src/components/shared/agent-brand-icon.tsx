 ...
 * additionally includes the native Creanova harness.
 */
export type AgentBrandIconKind = "Creanova" | ACPProviderIcon;

/** Creanova mark aspect (~227×190). */
const CREANOVA_MARK_ASPECT_RATIO = 227 / 190;
...
export function AgentBrandIcon({
  ...
  if (kind === "Creanova") {
    return (
      <CreanovaLogo
        width={Math.round(size * CREANOVA_MARK_ASPECT_RATIO)}
        height={size}
        className={cn("shrink-0", className)}
        data-testid={testId ?? "agent-brand-icon-Creanova"}
        aria-hidden
      />
    );
  }
