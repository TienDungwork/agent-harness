import { getAcpProvider as getClientAcpProvider } from "@Creanova/typescript-client";
 ...
 * are sourced at module load from ``@Creanova/typescript-client``'s ACP
 * registry — the generated mirror of the Python source of truth
 * ``Creanova.sdk.settings.acp_providers``. This config only adds the
...
// registry record (``@Creanova/typescript-client`` → Python SDK) with the
 ...
 * client registry once the pinned ``@Creanova/typescript-client`` mirrors that
 ...
 * ``@Creanova/typescript-client``) so they can't drift as providers are added
 ...
 *
 * Returns ``[]`` for Creanova, the ``"custom"`` preset, any unknown key, and a
...
export function buildAcpAgentSettingsDiff(
  ...
  if (providerKey === "Creanova") {
    // Switching back to Creanova. The agent-server's ``Settings.update``
