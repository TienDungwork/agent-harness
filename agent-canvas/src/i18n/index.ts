...
// Re-export so library consumers (`@Creanova/agent-canvas/i18n`) keep working
...
export const Creanova_I18N_NAMESPACE = "Creanova";
  ...
  if (!initializationPromises.has(instance)) {
    const initPromise = instance
      ...
      .init({
        ...
        ns: [Creanova_I18N_NAMESPACE],
        defaultNS: Creanova_I18N_NAMESPACE,
        fallbackNS: Creanova_I18N_NAMESPACE,
        backend: {
...
const withNamespace = (options?: unknown) => {
  if (!options) {
    return { ns: Creanova_I18N_NAMESPACE };
  }
