...
describe("AgentSettingsScreen", () => {
  ...
  it("renders the agent type selector defaulting to Creanova with sub-agents toggle", async () => {
    ...
    // Sub-agents toggle visible on the Creanova branch.
    expect(
      screen.getByTestId("agent-settings-enable-sub-agents"),
    ).toBeInTheDocument();
    // ACP-only fields stay hidden on the Creanova branch.
  ...
  it("labels the save button 'Save Changes' for consistency with other settings pages", async () => {
    ...
    // public/locales/en/Creanova.json; BUTTON$SAVE = "Save" (the bug).
    expect(screen.getByTestId("agent-save-button")).toHaveTextContent(
      "SETTINGS$SAVE_CHANGES",
    );
  });

  it("saves enable_sub_agents when toggling on the Creanova path", async () => {
  ...
  it("saves tool_concurrency_limit when changed on the Creanova path", async () => {
    const user = userEvent.setup();
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettings({
        agent_settings: {
  ...
  it("clears ACP fields when switching back to Creanova", async () => {
    const user = userEvent.setup();
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettings({
        agent_settings: {
    ...
    await user.click(
      await screen.findByRole("option", {
        name: "SETTINGS$AGENT_TYPE_Creanova",
      }),
    );
    await user.click(screen.getByTestId("agent-save-button"));
