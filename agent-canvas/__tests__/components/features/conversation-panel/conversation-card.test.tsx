...
describe("ConversationCard", () => {
  ...
  describe("Agent chip", () => {
    ...
    // toggle (``showLlmProfiles``) — one control for both ACP and Creanova
    ...
    it("hides the chip when showLlmProfiles is omitted for Creanova", () => {
      renderWithProviders(
        <ConversationCard
          title="Conversation 1"
          selectedRepository={null}
    ...
    it("renders the Creanova logo + model name for native conversations", () => {
      // With the chip toggle on, Creanova native conversations show the
      // Creanova logo + the raw ``agent.llm.model`` string. A stray
      // ``acp_server`` value on an Creanova card must not flip the icon to
      ...
      expect(
        within(chip).getByTestId("agent-brand-icon-Creanova"),
      ).toBeInTheDocument();
      expect(
    ...
    it("hides the chip for Creanova conversations with no model", () => {
