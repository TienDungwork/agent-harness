...
describe("BackendSelector", () => {
  ...
  it("expands a cloud backend into one row per org and records the org locally without calling cloud /switch", async () => {
    ...
    await waitFor(() => {
      const stored = JSON.parse(
        window.localStorage.getItem("Creanova-active-backend") ?? "null",
      );
  ...
  it("shows every organization as a separate row for cookie-auth cloud backends", async () => {
    ...
    await waitFor(() => {
      const wrapper = screen.getByTestId("backend-selector");
      const input = wrapper.querySelector("input") as HTMLInputElement;
      expect(input.value).toBe("Creanova Cloud – Acme Inc");
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Creanova Cloud – Personal"),
      ).toBeInTheDocument();
    });
    expect(screen.getByText("Creanova Cloud – Acme Inc")).toBeInTheDocument();
  ...
  it("opens the manage backends modal and removes a backend", async () => {
    ...
    const remaining = JSON.parse(
      window.localStorage.getItem("Creanova-backends") ?? "[]",
    );
