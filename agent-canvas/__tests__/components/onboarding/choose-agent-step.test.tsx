...
function renderStep(initial: OnboardingAgentId = "Creanova") {
...
describe("ChooseAgentStep", () => {
  ...
  it("renders all four agent options with Creanova marked selected by default", () => {
    renderStep();

    const Creanova = screen.getByTestId("onboarding-agent-option-Creanova");
    ...
    expect(Creanova).toHaveAttribute("aria-checked", "true");
    // All four options are clickable — ACP is no longer "coming soon".
    expect(Creanova).not.toBeDisabled();
  ...
  it("propagates click selections through onSelect for every option", async () => {
    ...
    await user.click(screen.getByTestId("onboarding-agent-option-Creanova"));
    expect(onSelect).toHaveBeenLastCalledWith("Creanova");
  });

  it("persists agent_kind:'Creanova' and advances on Next when Creanova is selected", async () => {
    const save = vi.spyOn(SettingsService, "saveSettings");
    const { onNext } = renderStep("Creanova");
    const user = userEvent.setup();
    ...
    expect(call.agent_settings_diff).toEqual({ agent_kind: "Creanova" });
  });
