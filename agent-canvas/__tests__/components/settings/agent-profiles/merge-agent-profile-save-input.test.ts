...
describe("mergeAgentProfileSaveInput", () => {
  it("preserves unmodeled Creanova fields under the edited ones", () => {
    ...
    const merged = mergeAgentProfileSaveInput(storedCreanova, edited);
  ...
  it("strips server-managed identity from the merge", () => {
    const merged = mergeAgentProfileSaveInput(storedCreanova, {
      agent_kind: "Creanova",
      enable_sub_agents: true,
      llm_profile_ref: "new-llm",
    });
  ...
  it("sends a clean variant payload on an agent_kind switch", () => {
    ...
    // Creanova → acp: carrying stored Creanova fields over would produce a
    // mongrel payload the server's extra="forbid" union rejects.
    expect(mergeAgentProfileSaveInput(storedCreanova, edited)).toEqual(edited);
  });
