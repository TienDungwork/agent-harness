...
const CreanovaSettings = {
  agent_settings: { agent_kind: "Creanova" },
};
...
describe("useSettingsNavItems", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    useSettingsMock.mockReturnValue({ data: CreanovaSettings });
  ...
  it("keeps the generic LLM settings item on cloud backends", () => {
    ...
    // Agent profiles are available on cloud too (Creanova #15060), so every
  ...
  it("derives the ACP-disable from the active agent profile, not global settings", () => {
    useConfigMock.mockReturnValue({ data: createConfig() });
    // Global settings still say Creanova, but the active profile is ACP —
  ...
  it("leaves all items enabled when agent_kind is Creanova", () => {
    useConfigMock.mockReturnValue({ data: createConfig() });
