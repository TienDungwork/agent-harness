...
describe("useCreateConversation", () => {
  ...
  it("keeps the profile path for an ACP `default` profile (agent_settings can't carry ACP config)", async () => {
    // The default→agent_settings shortcut is Creanova-only: activation is
    // pointer-only, so global agent_settings is stale (still Creanova) for an
  ...
  it("stamps the launched Creanova profile's llm_profile_ref into conversation metadata (#1082)", async () => {
