...
function makeEvent(id: string, timestamp: string): CreanovaEvent {
  return { id, timestamp } as unknown as CreanovaEvent;
}

function makePage(
  items: CreanovaEvent[],
  nextPageId: string | null = null,
): EventSearchPage<CreanovaEvent> {
...
describe("useLoadOlderEvents", () => {
  ...
  it("keeps paginating while the server keeps returning full pages", async () => {
    ...
    const fullPage: CreanovaEvent[] = Array.from(
  ...
  it("coalesces repeated loadOlder calls while a page request is already in flight", async () => {
    ...
    let resolvePage!: (page: EventSearchPage<CreanovaEvent>) => void;
    const pendingPage = new Promise<EventSearchPage<CreanovaEvent>>(
      (resolve) => {
        resolvePage = resolve;
      },
    );
  ...
  it("throws a descriptive error when searchEvents returns malformed items", async () => {
    ...
    vi.spyOn(EventService, "searchEvents").mockResolvedValue({
      items: { bad: true },
      next_page_id: null,
    } as unknown as EventSearchPage<CreanovaEvent>);
  ...
  it("stops paginating silently when the oldest loaded event is missing a timestamp", async () => {
    act(() => {
      useEventStore
        .getState()
        .addEvent({ id: "evt-missing-ts" } as CreanovaEvent);
    });
