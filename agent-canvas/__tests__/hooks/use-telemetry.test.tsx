...
describe("useTelemetry", () => {
  ...
  it("grants consent and enables telemetry", async () => {
    ...
    expect(result.current.isEnabled).toBe(true);
    expect(result.current.showConsentPrompt).toBe(false);
    expect(localStorage.getItem("Creanova-telemetry-consent")).toBe("granted");
  });
  ...
  it("denies consent and disables telemetry", async () => {
    ...
    expect(result.current.isEnabled).toBe(false);
    expect(result.current.showConsentPrompt).toBe(false);
    expect(localStorage.getItem("Creanova-telemetry-consent")).toBe("denied");
  });
  ...
  it("track function does nothing when consent is not granted", () => {
    localStorage.setItem("Creanova-telemetry-first-use", "true"); // Skip install tracking
    vi.clearAllMocks();
