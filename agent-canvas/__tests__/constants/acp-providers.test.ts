...
describe("ACP provider registry", () => {
  it("sources display_name / default_command / models from the SDK, not a local mirror", () => {
    ...
    // straight from @Creanova/typescript-client's getAcpProvider(), so the
...
describe("getAcpProviderSecrets — containerized credentials", () => {
  ...
  it("returns [] for Creanova / custom / unknown / empty", () => {
    expect(getAcpProviderSecrets("Creanova")).toEqual([]);
...
describe("getAcpPreferredDefaultModel", () => {
  ...
  it("returns null for Creanova / custom / unknown", () => {
    expect(getAcpPreferredDefaultModel("Creanova")).toBeNull();
