...
describe("buildStartConversationRequest", () => {
  ...
  it("derives confirmation and security settings the same way as Creanova", () => {
    const payload = buildStartConversationRequest({
      settings: {
        ...DEFAULT_SETTINGS,
        agent_settings: {
  ...
  it("does not mirror conversation secrets onto agent_context for non-ACP conversations", () => {
    // The Creanova ``Agent`` reads secrets from ``secret_registry``
  ...
  describe("canvas_ui client tool injection", () => {
    ...
    it("sends the client tool for an Creanova profile", () => {
      const payload = buildStartConversationRequest({
        settings: DEFAULT_SETTINGS,
        agentProfileId: "profile-Creanova",
        agentProfileKind: "Creanova",
      });
  ...
  describe("llm.model fallback — frontend always sends its chosen default", () => {
    ...
    it.each([
      ...
      [
        "absent (no llm block)",
        { schema_version: 1, agent_kind: "Creanova", agent: "CodeActAgent" },
      ],
...
describe("toAppConversation", () => {
  ...
  it("hydrates active_profile from stored metadata so the switcher shows the exact profile (#1082)", () => {
    ...
    try {
      const result = toAppConversation({
        ...
        agent: {
          kind: "Agent",
          llm: { model: "Creanova/claude-sonnet-4-6" },
        },
      });
  ...
  it("marks Creanova conversations and surfaces the agent.llm.model", () => {
    const result = toAppConversation({
      ...baseInfo,
      agent: { kind: "Agent", llm: { model: "claude-sonnet-4-6" } },
    });
    expect(result.agent_kind).toBe("Creanova");
  ...
  it("ignores tags.acpserver on Creanova conversations to prevent stray-tag bleed", () => {
    ...
    // turning the sidebar of an Creanova conversation into "Claude
...
describe("buildRuntimeServicesSystemSuffix", () => {
  ...
  it("renders a <RUNTIME_SERVICES> block when an automation entry is present", () => {
    ...
    expect(suffix).toContain(
      "X-Session-API-Key: $Creanova_AUTOMATION_API_KEY",
    );
    expect(suffix).not.toContain("X-API-Key: $Creanova_AUTOMATION_API_KEY");
    expect(suffix).toContain("</RUNTIME_SERVICES>");
...
describe("buildStartConversationRequest — ACP discriminator", () => {
  ...
  it("does not include ACP-only fields in Creanova agent settings", () => {
    const payload = buildStartConversationRequest({
      settings: {
        ...DEFAULT_SETTINGS,
        agent_settings: {
  ...
  it("ACP → Creanova → ACP round trip leaves no field leakage", () => {
