...
describe("isSameCloudHost", () => {
  it.each([
    ["https://staging.Creanova.dev", "https://staging.all-hands.dev"],
    [
      "https://pr-254.staging.Creanova.dev",
      "https://pr-254.staging.all-hands.dev",
    ],
    ["https://Creanova.dev", "https://app.all-hands.dev"],
...
describe("getLockedCloudAuthMode", () => {
  ...
  it("uses cookie auth for equivalent Creanova.dev and all-hands.dev preview hosts", () => {
    mockWindowLocation("https://pr-254.staging.Creanova.dev/canvas");
  ...
  it("uses cookie auth when production moves from app.all-hands.dev to Creanova.dev", () => {
    mockWindowLocation("https://Creanova.dev/canvas");
    setInjectedCloudHost("https://app.all-hands.dev");

    expect(getLockedCloudAuthMode()).toBe("cookie");
    expect(getCookieAuthCloudHost()).toBe("https://Creanova.dev");
  });
