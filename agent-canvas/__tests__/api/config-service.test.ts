...
describe("ConfigService", () => {
  ...
  it("includes verified providers absent from /api/llm/providers and keeps them within the limit", async () => {
    ...
    // /api/llm/providers comes from litellm (no "Creanova"),
    // but /api/llm/models/verified has "Creanova" as a key.
    ...
    server.use(
      ...
      http.get("/api/llm/models/verified", () =>
        HttpResponse.json({
          models: {
            Creanova: ["claude-opus-4-7", "gpt-5.5"],
    ...
    // Assert
    const Creanova = page.items.find((p) => p.name === "Creanova");
    expect(Creanova).toEqual({ name: "Creanova", verified: true });
  });
});
