...
describe("device-flow-client", () => {
  ...
  describe("isCreanovaCloudHost", () => {
    it("returns true for all-hands.dev domains", () => {
      expect(isCreanovaCloudHost("https://app.all-hands.dev")).toBe(true);
      expect(isCreanovaCloudHost("https://staging.all-hands.dev")).toBe(true);
      expect(isCreanovaCloudHost("app.all-hands.dev")).toBe(true);
      expect(isCreanovaCloudHost("ALL-HANDS.DEV")).toBe(true);
      expect(isCreanovaCloudHost("all-hands.dev")).toBe(true);
    });

    it("returns true for Creanova.dev domains", () => {
      expect(isCreanovaCloudHost("https://app.Creanova.dev")).toBe(true);
      expect(isCreanovaCloudHost("Creanova.dev")).toBe(true);
    });

    it("returns false for other domains", () => {
      expect(isCreanovaCloudHost("https://localhost:8000")).toBe(false);
      expect(isCreanovaCloudHost("http://127.0.0.1")).toBe(false);
      expect(isCreanovaCloudHost("https://example.com")).toBe(false);
      expect(isCreanovaCloudHost("https://my-Creanova-server.com")).toBe(
        false,
      );
    });
    ...
    it("prevents substring matching attacks", () => {
      ...
      expect(isCreanovaCloudHost("https://all-hands.dev.evil.com")).toBe(
        false,
      );
      expect(isCreanovaCloudHost("https://malicious-all-hands.dev")).toBe(
        false,
      );
      expect(isCreanovaCloudHost("https://evil.com/all-hands.dev")).toBe(
        false,
      );
    });

    it("returns false for invalid URLs", () => {
      expect(isCreanovaCloudHost("")).toBe(false);
      expect(isCreanovaCloudHost("not-a-url")).toBe(false);
    });
  });
