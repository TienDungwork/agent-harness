...
import {
  Creanova_CLOUD_INTEGRATIONS_URL,
...
describe("resolveResponderDeploymentOption", () => {
  it("resolves each supported target to its action and rejects unsupported targets", () => {
    ...
    expect(resolveResponderDeploymentOption("Creanova-cloud").action).toEqual({
      kind: "open-url",
      url: Creanova_CLOUD_INTEGRATIONS_URL,
    });
    expect(Creanova_CLOUD_INTEGRATIONS_URL).toBe(
