...
describe("check-sdk-version-sync helpers", () => {
  ...
  describe("parseSdkVersionsFromRequiresDist", () => {
    it("parses standard PEP 508 format with >=", () => {
      const deps = [
        "Creanova-sdk>=1.22.0,<2.0.0",
        "Creanova-workspace>=1.22.0",
      ];
      const versions = parseSdkVersionsFromRequiresDist(deps);

      expect(versions["Creanova-sdk"]).toBe("1.22.0");
      expect(versions["Creanova-workspace"]).toBe("1.22.0");
    });

    it("parses exact version pins with ==", () => {
      const deps = ["Creanova-tools==1.21.1"];
      const versions = parseSdkVersionsFromRequiresDist(deps);

      expect(versions["Creanova-tools"]).toBe("1.21.1");
    });

    it("parses parenthesized format", () => {
      const deps = ["Creanova-sdk (>=1.22.0)"];
      const versions = parseSdkVersionsFromRequiresDist(deps);
    ...
    it("handles mixed dependencies", () => {
      const deps = [
        ...
        "Creanova-sdk>=1.22.0",
        "flask>=1.0.0",
        "Creanova-workspace (>=1.21.0)",
      ];
      const versions = parseSdkVersionsFromRequiresDist(deps);

      expect(versions["Creanova-sdk"]).toBe("1.22.0");
      expect(versions["Creanova-workspace"]).toBe("1.21.0");
    ...
    it("handles version with extras", () => {
      ...
      const deps = ["Creanova-sdk[all]>=1.22.0"];
      const versions = parseSdkVersionsFromRequiresDist(deps);
  ...
  describe("SDK_PACKAGES", () => {
    it("contains the expected SDK packages", () => {
      expect(SDK_PACKAGES).toContain("Creanova-sdk");
      expect(SDK_PACKAGES).toContain("Creanova-tools");
      expect(SDK_PACKAGES).toContain("Creanova-workspace");
      expect(SDK_PACKAGES).toContain("Creanova-agent-server");
