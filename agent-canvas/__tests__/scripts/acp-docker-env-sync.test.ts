...
describe("upsertEnvLine", () => {
  const line =
    "AGENT_SERVER_IMAGE=ghcr.io/Creanova/agent-server:1.28.1-python";
  ...
  it("replaces an existing assignment in place, preserving other lines", () => {
    const existing =
      "SESSION_API_KEY=abc\n" +
      "AGENT_SERVER_IMAGE=ghcr.io/Creanova/agent-server:1.25.0-python\n" +
  ...
  it("treats a commented template line as documentation and appends the real value", () => {
    ...
    const existing =
      "# AGENT_SERVER_IMAGE=ghcr.io/Creanova/agent-server:latest-python\n";
    expect(upsertEnvLine(existing, line)).toBe(
      `${existing.trimEnd()}\n${line}\n`,
    );
  });
