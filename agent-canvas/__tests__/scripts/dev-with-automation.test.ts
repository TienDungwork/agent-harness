...
describe("buildAutomationCommand", () => {
  it("uses released PyPI version by default", () => {
    ...
    expect(cmd.args).toContain("Creanova.automation.app:app");
...
describe("buildAgentServerAutomationEnv", () => {
  it("exposes the session API key as Creanova_AUTOMATION_API_KEY for agent curl commands", () => {
    expect(
      buildAgentServerAutomationEnv({ sessionApiKey: "shared-session-key" }),
    ).toEqual({
      Creanova_AUTOMATION_API_KEY: "shared-session-key",
    });
  });
});
...
describe("buildConfig", () => {
   ...
   * so tests don't write to the user's real ~/.Creanova/agent-canvas files.
   *
...
describe("default constants", () => {
  it("has expected default automation repo", () => {
    expect(DEFAULT_AUTOMATION_REPO).toBe(
      "https://github.com/Creanova/automation",
    );
  });

  it("has expected default automation package", () => {
    expect(DEFAULT_AUTOMATION_PACKAGE).toBe("Creanova-automation");
  });
...
describe("dev-with-automation CLI", () => {
  it("shows help with --help flag", async () => {
    ...
    expect(output).toContain("Creanova_AUTOMATION_API_KEY");
  ...
  it("fails fast with a clear error when OH_AGENT_SERVER_LOCAL_PATH is invalid", async () => {
    ...
    // requires the four workspace subdirs (Creanova-agent-server, Creanova-sdk,
    // Creanova-tools, Creanova-workspace) and must reject this.
