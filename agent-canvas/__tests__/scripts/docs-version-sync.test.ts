...
describe("docs/example references stay in sync with config/defaults.json", () => {
  ...
  it("every README Docker image reference uses the pinned tag", () => {
    const imageRefPattern = /ghcr\.io\/Creanova\/agent-canvas:[^\s`"]+/g;
  ...
  it("scripts/check-sdk-version-sync.mjs examples match the current default", () => {
    ...
    expect(src).toContain(`"Creanova-sdk>=${agentServerVersion},<2.0.0"`);
    expect(src).toContain(`"Creanova-tools==${agentServerVersion}"`);
    expect(src).toContain(`"Creanova-workspace (>=${agentServerVersion})"`);
