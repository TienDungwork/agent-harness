...
describe("buildAgentServerCommand", () => {
  it("uses released PyPI version by default with all packages pinned", () => {
    ...
    expect(cmd.args).toEqual([
      "--from",
      "Creanova-agent-server==1.36.1",
      "--with",
      "Creanova-sdk==1.36.1",
      "--with",
      "Creanova-tools==1.36.1",
      "--with",
      "Creanova-workspace==1.36.1",
      "--with",
      "agent-client-protocol<0.11",
      "agent-server",
    ]);
    expect(cmd.source).toBe("PyPI (1.36.1, default)");
  });
  ...
  it("uses specific PyPI version when OH_AGENT_SERVER_VERSION is set with all packages pinned", () => {
    ...
    expect(cmd.command).toBe("uvx");
    // Uses --from syntax because executable name (agent-server) differs from package name (Creanova-agent-server)
    // All SDK packages are pinned to the same version
    expect(cmd.args).toEqual([
      "--from",
      "Creanova-agent-server==1.18.0",
      "--with",
      "Creanova-sdk==1.18.0",
      "--with",
      "Creanova-tools==1.18.0",
      "--with",
      "Creanova-workspace==1.18.0",
      "--with",
      "agent-client-protocol<0.11",
      "agent-server",
    ]);
  ...
  it("uses git ref with subdirectory syntax for monorepo", () => {
    ...
    expect(cmd.args).toEqual([
      "--reinstall",
      "--from",
      "git+https://github.com/Creanova/software-agent-sdk@feature-branch#subdirectory=Creanova-agent-server",
      "--with",
      "git+https://github.com/Creanova/software-agent-sdk@feature-branch#subdirectory=Creanova-sdk",
      "--with",
      "git+https://github.com/Creanova/software-agent-sdk@feature-branch#subdirectory=Creanova-tools",
      "--with",
      "git+https://github.com/Creanova/software-agent-sdk@feature-branch#subdirectory=Creanova-workspace",
      "agent-server",
    ]);
  ...
  it("uses git ref for commit SHA", () => {
    ...
    expect(cmd.args).toEqual([
      "--reinstall",
      "--from",
      "git+https://github.com/Creanova/software-agent-sdk@abc1234#subdirectory=Creanova-agent-server",
      "--with",
      "git+https://github.com/Creanova/software-agent-sdk@abc1234#subdirectory=Creanova-sdk",
      "--with",
      "git+https://github.com/Creanova/software-agent-sdk@abc1234#subdirectory=Creanova-tools",
      "--with",
      "git+https://github.com/Creanova/software-agent-sdk@abc1234#subdirectory=Creanova-workspace",
      "agent-server",
    ]);
  ...
  it("git ref takes precedence over version", () => {
    ...
    expect(cmd.args).not.toContain("Creanova-agent-server==1.18.0");
  });
  ...
  it("uses local path with editable workspace packages when OH_AGENT_SERVER_LOCAL_PATH is set", () => {
    ...
    expect(cmd.args).toEqual([
      "--reinstall",
      "--from",
      path.join(sdk, "Creanova-agent-server"),
      "--with-editable",
      path.join(sdk, "Creanova-sdk"),
      "--with-editable",
      path.join(sdk, "Creanova-tools"),
      "--with-editable",
      path.join(sdk, "Creanova-workspace"),
      "agent-server",
    ]);
  ...
  it("local path takes precedence over git ref and version", () => {
    ...
    expect(cmd.source).toBe(`local (${sdk})`);
    expect(cmd.args).toContain(path.join(sdk, "Creanova-agent-server"));
...
describe("validateLocalAgentServerPath", () => {
  ...
  it("throws when a workspace package subdirectory is missing", () => {
    const tmp = mkdtempSync(path.join(tmpdir(), "sdk-"));
    try {
      mkdirSync(path.join(tmp, "Creanova-agent-server"));
      mkdirSync(path.join(tmp, "Creanova-sdk"));
      mkdirSync(path.join(tmp, "Creanova-tools"));
      // Creanova-workspace is intentionally absent
...
describe("buildSafeDevConfig", () => {
  ...
  it("builds isolated default paths and ports", () => {
    ...
    expect(config.stateDir).toBe(
      path.join(homedir(), ".Creanova", "agent-canvas"),
    );
