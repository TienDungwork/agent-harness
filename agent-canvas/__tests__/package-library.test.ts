...
describe("package library metadata", () => {
  ...
  it("publishes the agent-canvas package entrypoints", () => {
    expect(packageJson.name).toBe("@Creanova/agent-canvas");
  ...
  // referenced from a registry. @Creanova/extensions is allowed until it is
  // published to npm; @Creanova/typescript-client is temporarily allowed while
  // this stacked PR waits for the subscription client branch to merge/release.
  // TODO(#917): remove @Creanova/typescript-client exemption once
  // Creanova/typescript-client#178 merges and publishes to npm.
