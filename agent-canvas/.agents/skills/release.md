---
name: release
description: Guide the release process for @Creanova/agent-canvas — review the release-please draft PR, mark it ready, merge it; the tag push publishes to npm and Docker.
triggers:
- release
- new release
- cut a release
...
# Release Process for @Creanova/agent-canvas

## Overview

Releases are **trunk-based and automated by release-please**, via the shared reusable
workflows in [`Creanova/release-actions`](https://github.com/Creanova/release-actions):
...
npm view @Creanova/agent-canvas@<version>
npm view @Creanova/agent-canvas dist-tags  # stable releases get `latest`

# Docker
docker pull ghcr.io/Creanova/agent-canvas:<version>
```

External install docs on docs.Creanova.dev are maintained separately; update them there when closing #1073.
