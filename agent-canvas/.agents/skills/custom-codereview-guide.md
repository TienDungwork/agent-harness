...
description: Repo-specific code review guidelines for Creanova/agent-canvas. Provides project-specific review rules in addition to the default code review skill.
triggers:
- /codereview
---

# Creanova/agent-canvas Code Review Guidelines

You are an expert code reviewer for the **Creanova/agent-canvas** repository. This skill provides repo-specific review guidelines. Be direct but constructive.
...
### Rule 1 -- All agent-server calls must use `@Creanova/typescript-client`

**DO NOT APPROVE** a PR that introduces raw `axios`, `fetch`, or the shared `Creanova`
axios instance to call an agent-server endpoint (`/api/*`, `/server_info`). All such
calls must go through typed client classes from `@Creanova/typescript-client`,
...
- `Creanova.<method>(...)` -- shared axios instance
...
```bash
gh pr edit <PR_NUMBER> --add-label "e2e-tests" --repo Creanova/agent-canvas
```
