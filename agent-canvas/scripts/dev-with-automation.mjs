 ...
 *
 * Extends agent-canvas's dev-safe.mjs to additionally run the Creanova Automation
 ...
 *   as Creanova_AUTOMATION_API_KEY, making it available to agents in conversations.
...
import { pypiPackage } from "./upstream-registry.mjs";
import {
...
function buildAutomationCommand(env = process.env) {
  ...
  if (gitRef) {
    ...
    uvxArgs.push(
      "--from",
      `${pypiPackage(DEFAULT_AUTOMATION_PACKAGE)}==${version}`,
      "uvicorn",
      "openhands.automation.app:app",
    );
    source = `PyPI (${version})`;
    ...
    uvxArgs.push(
      "--from",
      `${pypiPackage(DEFAULT_AUTOMATION_PACKAGE)}==${DEFAULT_AUTOMATION_VERSION}`,
      "uvicorn",
      "openhands.automation.app:app",
    );
...
async function buildConfig(args, env = process.env) {
  ...
  return {
    ...
    // Local auth gateway (Milestone 1+). Default backend=local → no Docker.
    localAuthEnabled: isLocalAuthEnabled(env),
    localAuthBackend: (env.OH_LOCAL_AUTH_BACKEND || env.AUTH_BACKEND || "local")
      .toString()
      .toLowerCase(),
    localGatewayPort: parseInt(env.OH_LOCAL_GATEWAY_PORT || "18110", 10),
...
/** When local auth is on, HTTP /api goes through the gateway; WS stays direct. */
const AGENT_SERVER_ROUTE_PREFIXES_WITH_GATEWAY = [
  "/sockets",
  "/server_info",
  "/health",
  "/ready",
...
function getLocalServiceRoutes(config) {
  ...
  if (config.localAuthEnabled && config.localGatewayPort) {
    routes.push(["/api", `http://127.0.0.1:${config.localGatewayPort}`]);
    routes.push(["/healthz", `http://127.0.0.1:${config.localGatewayPort}`]);
  }

  if (config.launchAgentServer) {
    const prefixes = config.localAuthEnabled
      ? AGENT_SERVER_ROUTE_PREFIXES_WITH_GATEWAY
      : AGENT_SERVER_ROUTE_PREFIXES;
...
function buildAgentServerAutomationEnv(config) {
  return {
    ...
    // agent-server as Creanova_AUTOMATION_API_KEY. The launcher also seeds
    // this into Settings > Secrets, but agents commonly create automations
    // with a curl command that references `$Creanova_AUTOMATION_API_KEY`;
    // exposing it here keeps that path working even before/without
    // secret-registry env expansion.
    Creanova_AUTOMATION_API_KEY: config.sessionApiKey,
  };
}
...
function startAgentServer(config) {
  ...
  const agentServerEnv = {
    ...
    Creanova_REMOTE_WS_READY_REQUIRED:
      process.env.Creanova_REMOTE_WS_READY_REQUIRED || "false",
...
function ensurePostgres(config) {
  const backend = (process.env.OH_DATA_BACKEND || "sqlite").toLowerCase();
  if (backend !== "postgres") {
    logService(
      "postgres",
      `Skipped (OH_DATA_BACKEND=${backend})`,
      c.dim,
    );
    return;
  }
  ...
  const envFile = resolve(projectRoot, "..", "deploy", "data", ".env");
  if (!existsSync(composeFile)) {
    logService("postgres", `Compose missing: ${composeFile}`, c.yellow);
    return;
  }
  if (!commandExists("docker")) {
    logService("postgres", "docker not found — cannot start Postgres", c.yellow);
    return;
  }
  logService(
    "postgres",
    "Ensuring PostgreSQL is up (127.0.0.1:54288, subnet 10.240.123.0/24)...",
    c.cyan,
  );
  ...
  if (result.status !== 0) {
    logService(
      "postgres",
      "docker compose failed — gateway may fall back or fail",
      c.yellow,
    );
  }
}

function startLocalGateway(config) {
  const gatewayRoot = resolve(projectRoot, "..", "services", "local-gateway");
  if (!existsSync(join(gatewayRoot, "main.py"))) {
    logService(
      "gateway",
      `Local gateway not found at ${gatewayRoot} — skip`,
      c.yellow,
    );
    return;
  }
  ...
  logService(
    "gateway",
    `Starting on port ${config.localGatewayPort}...`,
    c.cyan,
  );
  ...
  const cors = [
    ...
    `http://${process.env.OH_PUBLIC_HOST || "192.168.1.191"}:${config.ingressPort}`,
  ].join(",");
  ...
  spawnService(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        HOST: "127.0.0.1",
        PORT: String(config.localGatewayPort),
        KEYCLOAK_URL: process.env.KEYCLOAK_URL || "http://127.0.0.1:18180",
        KEYCLOAK_REALM: process.env.KEYCLOAK_REALM || "creanova",
        KEYCLOAK_CLIENT_ID: process.env.KEYCLOAK_CLIENT_ID || "creanova-local",
        KEYCLOAK_CLIENT_SECRET:
          process.env.KEYCLOAK_CLIENT_SECRET || "creanova-local-secret",
        AUTH_BACKEND: config.localAuthBackend || "local",
        SESSION_SECRET:
          process.env.SESSION_SECRET ||
          `creanova-local-${config.sessionApiKey || "dev"}`,
        AGENT_SERVER_URL: `http://127.0.0.1:${config.agentServerPort}`,
        AGENT_SERVER_API_KEY: config.sessionApiKey || "",
        DATABASE_URL:
          process.env.LOCAL_GATEWAY_DATABASE_URL ||
          (process.env.OH_DATA_BACKEND === "postgres"
...
function ensureKeycloak(config) {
  if (!config.localAuthEnabled) return;
  if (config.localAuthBackend !== "keycloak") {
    logService(
      "keycloak",
      `Skipped (AUTH_BACKEND=${config.localAuthBackend || "local"} — no Docker needed)`,
      c.dim,
    );
    return;
  }
  ...
  if (!existsSync(composeFile)) {
    logService("keycloak", `Compose file missing: ${composeFile}`, c.yellow);
    return;
  }
  if (!commandExists("docker")) {
    logService(
      "keycloak",
      "docker not found — set OH_LOCAL_AUTH_BACKEND=local (default) or install Docker",
      c.yellow,
    );
    return;
  }
  logService(
    "keycloak",
    "Ensuring Keycloak is up (subnet 10.240.122.0/24)...",
    c.cyan,
  );
  const result = spawnSync(
    "docker",
    ["compose", "-f", composeFile, "up", "-d"],
    { stdio: "inherit", cwd: dirname(composeFile) },
  );
  if (result.status !== 0) {
    logService(
      "keycloak",
      "docker compose failed — login will not work until Keycloak is up",
      c.yellow,
    );
  }
}
...
function startVite(config) {
  ...
  if (config.localAuthEnabled) {
    viteEnv.VITE_LOCAL_AUTH_ENABLED = "true";
  }
 ...
 * Creanova_AUTOMATION_API_KEY so agents can authenticate with the
...
async function seedAutomationSecret(config, options = {}) {
  ...
  const secretName = "Creanova_AUTOMATION_API_KEY";
...
function printBanner(config) {
  ...
  if (config.localAuthEnabled) {
    console.log(
      ansiPadEnd(
        `${c.green}${c.bold}║${c.reset}  Login:        ${c.cyan}http://localhost:${config.ingressPort}/login${c.reset}`,
        BOX_INNER,
      ) + `${c.green}${c.bold}║${c.reset}`,
    );
    console.log(
      ansiPadEnd(
        `${c.green}${c.bold}║${c.reset}  Auth backend: ${c.cyan}${config.localAuthBackend || "local"}${c.reset}`,
        BOX_INNER,
      ) + `${c.green}${c.bold}║${c.reset}`,
    );
    if (config.localAuthBackend === "keycloak") {
      console.log(
        ansiPadEnd(
          `${c.green}${c.bold}║${c.reset}  Keycloak:     ${c.cyan}http://127.0.0.1:18180${c.reset}`,
          BOX_INNER,
        ) + `${c.green}${c.bold}║${c.reset}`,
      );
    }
...
async function main(options = {}) {
  ...
  // 2b. Seed standard Creanova LiteLLM settings (model + base URL + optional API key)
  if (agentServerReady) {
    const { seedLlmSettings } = await import("./seed-llm-settings.mjs");
    await seedLlmSettings({
      agentServerPort: config.agentServerPort,
      log: (msg) => logService("llm-seed", msg, c.cyan),
    });
  }

  // 2c. Local auth: Keycloak + gateway (optional via OH_LOCAL_AUTH=1)
  if (config.localAuthEnabled) {
    ensurePostgres(config);
    ensureKeycloak(config);
    startLocalGateway(config);
    await waitForService(
      "gateway",
      `http://127.0.0.1:${config.localGatewayPort}/healthz`,
      60000,
    );
  }
