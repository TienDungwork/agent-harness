...
# `@Creanova/typescript-client` git dependency and silently falls back to a
# stale cached copy — producing the dreaded
# `[MISSING_EXPORT] ConversationClient is not exported by
# node_modules/@Creanova/typescript-client/dist/clients.js` at bundle time.
#
...
#
# See https://github.com/Creanova/agent-canvas/issues/384 for the original
# bug report.
set -euo pipefail
