...
OPTIONS:
  ...
  --lock-to-cloud <cloud-url>  Lock backend setup to a single Creanova Cloud
 ...
 *     2. Written to `Creanova-agent-server-config.sessionApiKey` in
 ...
 *
 * - `lockToCloud`: an Creanova Cloud URL exposed as
...
function makeConfigInjectionScript(
  ...
  if (sessionApiKey) {
    ...
    parts.push(
      `try{` +
        `var _k='Creanova-agent-server-config',` +
