...
const AGENT_SERVER_GIT_REPO = UPSTREAM_GIT_REPO;
// Filesystem / git subdirectory names for the upstream SDK checkout.
const LOCAL_AGENT_SERVER_SUBDIRS = [
  SHARED_DEFAULTS.packages.agentServer,
  SHARED_DEFAULTS.packages.sdk,
  SHARED_DEFAULTS.packages.tools,
  SHARED_DEFAULTS.packages.workspace,
].map(gitSubdir);
const DEFAULT_AGENT_SERVER_VERSION = SHARED_DEFAULTS.versions.agentServer;
const HOME_DIR_NAME = SHARED_DEFAULTS.paths.homeDirName || ".creanova";
// Temporary transitive-dep pin: hold acp <0.11 until the SDK ships a fix.
...
// (`Creanova-backends` localStorage) stay in sync without users needing to
// set anything in `.env`.
//
...
// same encryption key when both use ~/.Creanova as their state directory.
 ...
 *   packages (Creanova-sdk, Creanova-tools, Creanova-workspace) so source
...
export function buildAgentServerCommand(env = process.env) {
  ...
  const pkgServer = pypiPackage(SHARED_DEFAULTS.packages.agentServer);
  const pkgSdk = pypiPackage(SHARED_DEFAULTS.packages.sdk);
  const pkgTools = pypiPackage(SHARED_DEFAULTS.packages.tools);
  const pkgWorkspace = pypiPackage(SHARED_DEFAULTS.packages.workspace);
  const dirServer = gitSubdir(SHARED_DEFAULTS.packages.agentServer);
  const dirSdk = gitSubdir(SHARED_DEFAULTS.packages.sdk);
  const dirTools = gitSubdir(SHARED_DEFAULTS.packages.tools);
  const dirWorkspace = gitSubdir(SHARED_DEFAULTS.packages.workspace);
  ...
  if (localPath) {
    ...
    uvxArgs.push(
      "--reinstall",
      "--from",
      path.join(localPath, dirServer),
      "--with-editable",
      path.join(localPath, dirSdk),
      "--with-editable",
      path.join(localPath, dirTools),
      "--with-editable",
      path.join(localPath, dirWorkspace),
      "agent-server",
    );
    ...
    uvxArgs.push(
      "--reinstall",
      "--from",
      `${baseGitUrl}#subdirectory=${dirServer}`,
      "--with",
      `${baseGitUrl}#subdirectory=${dirSdk}`,
      "--with",
      `${baseGitUrl}#subdirectory=${dirTools}`,
      "--with",
      `${baseGitUrl}#subdirectory=${dirWorkspace}`,
      "agent-server",
    );
    source = `git (${gitRef})`;
  } else if (version) {
    ...
    uvxArgs.push(
      "--from",
      `${pkgServer}==${DEFAULT_AGENT_SERVER_VERSION}`,
      "--with",
      `${pkgSdk}==${DEFAULT_AGENT_SERVER_VERSION}`,
      "--with",
      `${pkgTools}==${DEFAULT_AGENT_SERVER_VERSION}`,
      "--with",
      `${pkgWorkspace}==${DEFAULT_AGENT_SERVER_VERSION}`,
    );
    if (AGENT_CLIENT_PROTOCOL_CONSTRAINT) {
      uvxArgs.push("--with", AGENT_CLIENT_PROTOCOL_CONSTRAINT);
    }
...
function buildConfigFromPorts(ports, cwd, env) {
  ...
  const stateDir = path.resolve(
    ...
    env.OH_CANVAS_SAFE_STATE_DIR ||
      path.join(homedir(), HOME_DIR_NAME, "agent-canvas"),
  );
  ...
  // ~/.Creanova/agent-canvas/secret-key.txt. Persisting ensures dev mode
  // and Docker mode share the same encryption key when they mount the same
  // ~/.Creanova directory (docker/entrypoint.sh reads/writes the same file).
  const secretKeyPath =
  ...
  // persisted to ~/.Creanova/agent-canvas/api-key.txt. Persisting on disk
  // keeps the agent-server, the Vite-baked VITE_SESSION_API_KEY, and any
  // `Creanova-backends` localStorage entries the frontend has cached all
  ...
  return {
    ...
    // ~/.Creanova/agent-canvas), matching where the rest of dev state lives
    ...
    // "error connecting to .../Creanova (No such file or directory)".
    //
...
export function buildAgentServerEnv(config) {
  return {
    ...
    // Parent of stateDir (= ~/.Creanova) so settings/secrets match Docker.
    ...
    // Creanova SDK boilerplate emitted by automation prompt/plugin
