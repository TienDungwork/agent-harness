 ...
 * @param {string} [options.automation.authEnvVar="Creanova_AUTOMATION_API_KEY"]
...
export function buildRuntimeServicesInfo(options) {
  ...
  const services = {
    agent_server: {
      description:
        "The Creanova Agent Server this agent is running inside. " +
  ...
  if (automationBaseUrl) {
    ...
    const authEnvVar = automation.authEnvVar ?? "Creanova_AUTOMATION_API_KEY";
    services.automation = {
      description:
        "Creanova Automations service. All routes are mounted under " +
