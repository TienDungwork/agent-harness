#!/usr/bin/env node
/**
 * CLI entry point for @Creanova/agent-canvas
 *
...
if (args.includes("--info")) {
  ...
  console.log(`@Creanova/agent-canvas ${version}
...
EXAMPLES:
  ...
  npx @Creanova/agent-canvas

  # Pin a specific key (local mode, key auto-injected into frontend)
  LOCAL_BACKEND_API_KEY=my-key npx @Creanova/agent-canvas

  # Public mode — users must enter the API key in the browser
  LOCAL_BACKEND_API_KEY=my-secret npx @Creanova/agent-canvas --public

  # Use a specific port
  npx @Creanova/agent-canvas --port 3000

  # Start only the static frontend behind ingress
  npx @Creanova/agent-canvas --frontend-only

  # Start only the agent-server and automation backend behind ingress
  npx @Creanova/agent-canvas --backend-only

  # Show default stack versions and ports
  npx @Creanova/agent-canvas --info

  # Use local SDK checkout for development
  OH_AGENT_SERVER_LOCAL_PATH=/path/to/sdk npx @Creanova/agent-canvas
`);
  process.exit(0);
}
...
try {
  ...
  console.error("  npm install -g @Creanova/agent-canvas@latest");
  console.error(`\nError: ${err.message}`);
  process.exit(1);
}
