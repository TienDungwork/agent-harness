...
3. **Run Agent Canvas** — generate a key with `openssl rand -base64 32`, then `export LOCAL_BACKEND_API_KEY=<key>` and `npx @Creanova/agent-canvas --public`
...
`npx @Creanova/agent-canvas --public` spins up the static frontend server,
...
npx @Creanova/agent-canvas --public
```
...
tmux new-session -d -s canvas 'npx @Creanova/agent-canvas --public'
...
ExecStart=npx @Creanova/agent-canvas --public
Restart=on-failure
RestartSec=5
