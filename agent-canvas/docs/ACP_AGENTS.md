...
Agent Canvas can drive your conversations with the built-in **Creanova** agent or
...
(`Creanova.sdk.settings.acp_providers`, mirrored into
`@Creanova/typescript-client`) and enriched with Canvas UI metadata in
...
1. **Choose agent** — pick Claude Code, Codex, or Gemini CLI instead of
   Creanova. The choice is saved immediately to your backend's settings.
...
docker run -d --name oh-acp -p 8010:8000 -v acp-data:/workspace \
  ...
  ghcr.io/Creanova/agent-server:1.28.0-python
...
the released `@Creanova/typescript-client` does not yet expose it on
...
- **Agent** — switch between **Creanova** and **ACP**.
