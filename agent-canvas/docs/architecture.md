...
Agent Canvas is a React and TypeScript frontend for running and monitoring Creanova agents across local, remote, and hosted environments. It is adapted from the Creanova frontend to talk directly to the Creanova Agent Server and related automation services.
...
- Translating UI actions into Creanova Agent Server API calls.
...
The primary backend is the [Creanova Agent Server](https://github.com/Creanova/software-agent-sdk/tree/main/Creanova-agent-server/Creanova/agent_server). Agent Canvas can connect to one or more Agent Server instances and switch between them from the UI.
...
- Creanova Cloud APIs for hosted sandbox and organization workflows.
...
The npm package is `@Creanova/agent-canvas`. The package exposes:
