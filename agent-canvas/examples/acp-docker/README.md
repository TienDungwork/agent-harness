...
This starts `ghcr.io/Creanova/agent-server:latest-python` on
...
```bash
AGENT_SERVER_IMAGE=ghcr.io/Creanova/agent-server:$(gh api repos/Creanova/software-agent-sdk/commits/main --jq '.sha[0:7]')-python docker compose up
```
