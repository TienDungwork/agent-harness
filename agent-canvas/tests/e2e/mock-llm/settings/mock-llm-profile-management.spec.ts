 ...
 *
 *   3. Creanova provider hidden base_url preservation:
...
test.describe("same-model profile identity", () => {
  ...
  test("chat header shows the correct profile when two profiles share the same model", async ({
    ...
    await test.step("profile switcher shows the correct profile name", async () => {
      // In a local Creanova conversation the chat input renders the
...
// ═══════════════════════════════════════════════════════════════════════
// Test 3 — Creanova provider hidden base_url preservation
// ═══════════════════════════════════════════════════════════════════════

test.describe("Creanova provider hidden base_url preservation", () => {
  // The public Creanova provider model remains the profile identity, but a
  ...
  const Creanova_PROFILE = "Creanova-basic-save-test";
  const Creanova_MODEL = "Creanova/claude-opus-4-5-20251101";
  const LEGACY_TRANSPORT_MODEL = "litellm_proxy/claude-opus-4-5-20251101";
  const EXPECTED_Creanova_MODELS = [Creanova_MODEL, LEGACY_TRANSPORT_MODEL];
  const CUSTOM_BASE_URL = "https://custom-Creanova-proxy.example/v1";
  ...
  test.afterAll(async ({ browser }) => {
    ...
    try {
      ...
      await dismissAnalyticsModal(page);
      await waitForTestId(page, "add-llm-profile");
      await deleteProfileIfExists(page, Creanova_PROFILE);
    } catch {
      // best-effort
    } finally {
      await page.close();
  ...
  test("re-saving an Creanova profile from Basic view preserves hidden base_url", async ({
    page,
    request,
  }) => {
    // ── Setup: create a public Creanova profile with a custom base_url through
    ...
    await deleteProfileIfExists(page, Creanova_PROFILE);
    await createProfileViaUI(page, {
      profileName: Creanova_PROFILE,
      model: Creanova_MODEL,
    ...
    await test.step("open profile in edit mode", async () => {
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        if (text?.includes(Creanova_PROFILE)) {
          targetRow = row;
          break;
        }
      }
      expect(
        targetRow,
        `Could not find profile row for "${Creanova_PROFILE}"`,
      ).not.toBeNull();
    ...
    await test.step("verify base_url is preserved after save", async () => {
      const config = await getProfileConfig(request, Creanova_PROFILE);
      expect(
        config.base_url,
        "Basic-tab re-save without a model change must not clear hidden base_url",
      ).toBe(CUSTOM_BASE_URL);
      expect(EXPECTED_Creanova_MODELS).toContain(config.model);
    });
