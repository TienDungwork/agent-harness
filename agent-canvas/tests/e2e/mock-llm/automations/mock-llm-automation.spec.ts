...
test.describe("mock-LLM automation lifecycle", () => {
  ...
  test("step 1: setup LLM profile and register automation trajectory", async ({
    ...
    // Auth uses $Creanova_AUTOMATION_API_KEY which the agent-server
    ...
    // sandboxes the execution environment), so $Creanova_AUTOMATION_API_KEY
    ...
    // Public skills are bundled from @Creanova/extensions at build time.
    ...
    // Creanova-automation skill, triggering this internal call.
