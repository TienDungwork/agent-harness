...
test.describe("onboarding recent regressions", () => {
  ...
  test("keeps the modal open on backdrop click and Escape", async ({
    ...
    await expect
      .poll(
        () =>
          page.evaluate(() =>
            window.localStorage.getItem("Creanova-onboarded"),
          ),
        {
          message:
  ...
  // First-run LLM setup should match DEFAULT_SETTINGS (Creanova default)
  // and must not push local users to a SaaS account CTA.

  test("defaults the LLM setup step to Creanova minimax-m2.7", async ({
    page,
  }) => {
    await showOnboarding(page, {
      apiKey: SESSION_API_KEY,
    ...
    await expect(
      providerInput,
      "first-run onboarding should default to the Creanova provider",
    ).toHaveValue("Creanova", { timeout: 10_000 });
    // The model input displays the model ID without the provider prefix.
    await expect(
      modelInput,
      "first-run onboarding should default to minimax-m2.7",
    ).toHaveValue("minimax-m2.7", {
      timeout: 10_000,
    });
    await expect(
      page.getByTestId("Creanova-account-help"),
      "Creanova account helper should stay hidden on local backends",
    ).toHaveCount(0);
  });
});
