 ...
 *
 *   Step 0 — Choose Agent: selects Creanova and advances.
 ...
 *   - `Creanova-onboarded` is set in localStorage.
...
test.describe("onboarding happy path", () => {
  ...
  test("completes the full onboarding flow and launches a conversation", async ({
    ...
    // Show the onboarding modal (clears Creanova-onboarded, seeds backend)
    ...
    await test.step("verify Creanova-onboarded is set in localStorage", async () => {
      await expect
        .poll(
          () =>
            page.evaluate(() =>
          ...
          { message: "Creanova-onboarded should be '1' after completing the flow" },
        )
        .toBe("1");
    });
