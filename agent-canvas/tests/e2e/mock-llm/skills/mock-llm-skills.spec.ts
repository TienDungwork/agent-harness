 ...
 *
 * 2. **User skills** from `~/.Creanova/skills/` are loaded and trigger
...
test.describe("skill loading: project, user, and deletion", () => {
  ...
  // ── Test 2: User skill loaded from ~/.Creanova/skills/ ──────────

  test("user skill in ~/.Creanova/skills/ triggers on matching keyword", async ({
    page,
    request,
  }) => {
    await ensureMockLLMProfile(page);
