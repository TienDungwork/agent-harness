...
export async function seedLocalStorage(page: Page) {
  await page.addInitScript(
    ({ apiKey }) => {
      ...
      window.localStorage.setItem("Creanova-telemetry-consent", "denied");
      window.localStorage.setItem("Creanova-telemetry-first-use", "true");
      window.localStorage.setItem("Creanova-onboarded", "1");
...
export async function ensureMockLLMProfile(
  ...
  try {
    ...
    // default agent is already Creanova, so the LLM settings route stays usable.
  }
...
/**
 * Reset agent type back to Creanova through the Settings → Agent profiles UI.
 * Used in afterAll cleanup to restore the default agent for subsequent tests.
 */
export async function resetToCreanovaAgentViaUI(page: Page) {
  await openAgentProfileEditor(page, "default");

  await selectDropdownOption(page, /Agent/, /Creanova/);

  // The LLM-profile selector only appears for Creanova-kind profiles, and
...
/**
 * @deprecated Use `resetToCreanovaAgentViaUI(page)` to exercise the UI path.
 * Kept only for callers that cannot open a page (should not exist in new tests).
 */
export async function resetToCreanovaAgent(request: APIRequestContext) {
  ...
  if (!resp.ok()) {
    console.warn(`[cleanup] Reset to Creanova failed: ${resp.status()}`);
  }
}
