...
// 1024×1024 Creanova raised-hands app icon. Used as the BrowserWindow.icon
 ...
 * `Creanova-agent-server` and its workspace deps from PyPI, which can
...
function createMainWindow() {
  ...
  mainWin.webContents.setWindowOpenHandler(({ url }) => {
    // The "Login with Creanova Cloud" device-flow opens about:blank immediately
...
async function startStack() {
  ...
  //     uvx the first time, downloading Python + installing Creanova-
  ...
  if (result?.agentServerReady === false) {
    throw new Error(
      "The agent server did not finish starting in time. " +
        ...
        "Python and the Creanova agent-server from PyPI. " +
...
app.whenReady().then(async () => {
  ...
  // Creanova logo instead of the default Electron logo. In a packaged
