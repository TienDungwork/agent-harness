...
Helm chart for running the [Creanova agent-canvas](https://github.com/Creanova/agent-canvas)
...
## Relationship to Creanova Enterprise
...
[Creanova Enterprise (OHE)](https://www.all-hands.ai/enterprise) is the
...
| `PersistentVolumeClaim` (per pod)   | Backs `~/.Creanova` and `~/workspace` (both mounted from the same PVC via `subPath`): settings, encrypted secrets, conversation history, automation SQLite DB, cloned repos, generated files. |
...
subdirectories of the Creanova user's HOME (`/home/Creanova`) via
...
- `~/.Creanova` — agent-server settings, encrypted secrets,
...
The pod runs as `Creanova` (UID 10001) from the upstream image;
...
config:
  extraEnv:
    - name: LLM_MODEL
      value: Creanova/claude-sonnet-4-5-20250929
    - name: LLM_API_KEY
      valueFrom:
        secretKeyRef:
          name: my-llm-secret
...
agent sandboxes, see [Creanova Enterprise](#relationship-to-Creanova-enterprise).
