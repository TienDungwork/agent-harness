...
4. **Lưu ý giao diện biểu đồ (Phase 2 polish)**:
   - Tin nhắn không có biểu đồ (vd: "xin chào", câu hỏi văn bản thuần) **không** hiện khung placeholder biểu đồ thừa để tránh che khuất tin nhắn.
   - Khung `.chart-slot` chỉ xuất hiện khi phản hồi mang dữ liệu biểu đồ (canvas Chart.js / PNG fallback) hoặc khi có yêu cầu biểu đồ nhưng rỗng số liệu (hiển thị empty state gọn gàng).
...
| **4** | **Bar Chart (Biểu đồ cột)** | Gửi câu hỏi: *"Vẽ biểu đồ cột lượt xe theo loại hôm nay"* | - Phía trên tin nhắn trả lời xuất hiện canvas **Chart.js** dạng cột (`bar`) nằm trong wrapper `.chart-canvas-wrapper`, canvas không bị méo hoặc blank.<br>- Có tiêu đề (title) tiếng Việt rõ ràng.<br>- Nhãn category hiển thị tiếng Việt (ví dụ: Ô tô, Xe máy, Xe tải,... thay vì chỉ mã thô CAR, MOTORBIKE nếu đã được ánh xạ).<br>- Màu sắc các cột rõ ràng, tương phản tốt; trục Y bắt đầu từ 0 (`beginAtZero`).<br>- Rê chuột (hover) vào từng cột hiển thị tooltip số liệu chi tiết.<br>- *(Không yêu cầu UI cũ đã bỏ như tab xem trước/phóng to PNG)*. | `[ ]` |
| **5** | **Pie Chart (Biểu đồ tròn)** | Gửi câu hỏi: *"Vẽ biểu đồ tròn tỷ lệ loại xe"* | - Phía trên tin nhắn trả lời xuất hiện canvas **Chart.js** dạng hình tròn / bánh (`pie`) trong `.chart-canvas-wrapper`.<br>- Tiêu đề (title) tiếng Việt rõ ràng nếu có.<br>- Legend đặt ở phía dưới (`position: 'bottom'`), đọc được với nhãn tiếng Việt kèm tỷ lệ % rõ ràng (ví dụ: "Ô tô: 35%").<br>- Các lát cắt phân biệt rõ nét với đường viền trắng (`borderColor: '#ffffff'`).<br>- Rê chuột (hover) vào từng lát cắt hiển thị tooltip có tỷ lệ phần trăm (%) và số lượng định dạng tiếng Việt. | `[ ]` |
...
> **Ghi chú UI biểu đồ (Phase 2):** Tin nhắn không chart (vd. chào hỏi, hỏi đáp văn bản thuần) không hiện khung placeholder biểu đồ thừa. Khung `.chart-slot` chỉ xuất hiện khi câu trả lời có payload biểu đồ hoặc khi rỗng dữ liệu thì hiển thị empty state ngắn gọn ("Chưa có dữ liệu để hiển thị biểu đồ").
