...
def test_graph_emits_orchestrator_events_in_stream():
    ...
    plan_event = next(
        ev for ev in done_orchestrator
        if isinstance(ev.get("output"), dict) and ev["output"].get("orchestrator_plan")
    )
    assert plan_event.get("input")
    assert plan_event.get("output")
    plan = plan_event["output"].get("orchestrator_plan") or {}
    assert len(plan.get("steps") or []) >= 2
    assert "orchestrator_collect" in node_ids
...
def test_graph_nodes_and_structure():
    ...
    assert "guardrail_input" in node_names
    assert "guardrail_output" in node_names
    assert "orchestrator_collect" in node_names
    assert "orchestrator_respond" not in node_names
    assert "respond_inline" in node_names
...
def test_intent_routing(mock_invoke_structured, mock_offline, mock_rewrite):
    ...
    # "Thời tiết hôm nay" được phân loại bởi classify (out_of_scope intent)
...
def test_greeting_offline_fast_path_graph_events(greeting: str):
    ...
    with patch("src.llm.client.use_offline_tools", return_value=True):
        ...
        assert node_ids[-1] == "guardrail_output"
...
def test_stat_question_routes_to_sql_path_offline(question: str):
    ...
    with patch("src.llm.client.use_offline_tools", return_value=True), \
        ...
        # Orchestrator is the central coordinator node
...
def test_pipeline_intent_fake_inline_answer_cleared_routes_to_orchestrator():
    """Pipeline intent with fake LLM answer must clear answer and route through orchestrator to retrieve_schema."""
...
def test_docs_intent_fake_inline_answer_cleared_routes_to_docs():
    """Docs intent with fake LLM answer must clear answer and route through orchestrator to retrieve_docs."""
    from src.llm.schemas import IntentResult
...
def test_cross_db_hay_comparison_routes_through_respond_node():
    ...
    reset_graph()
    q = "Trong khoảng từ 23/09/2026 đến 24/09/2026, phương tiện hay vùng cấm xảy ra nhiều hơn?"
    with patch("src.agent.generate_sql.use_offline_tools", return_value=True), \
         patch("src.agent.execute_sql.use_offline_tools", return_value=True), \
    ...
    node_ids = [ev.get("node_id") for ev in state.get("events", []) if ev.get("node_id")]
    assert "orchestrator_collect" in node_ids
    assert node_ids.count("retrieve_schema") == 1
    assert node_ids.count("generate_sql") == 1
    assert node_ids.count("execute_sql") == 1
    assert node_ids.count("orchestrator") == 1
    assert "respond" in node_ids
    gen_ev = next(ev for ev in state.get("events", []) if ev.get("node_id") == "generate_sql")
    assert gen_ev.get("output", {}).get("batch_size") == 2
    assert len(state.get("orchestrator_step_results") or []) == 2
...
def test_all_branches_terminate_at_respond_node():
    """Mọi nhánh graph đi qua respond rồi kết thúc tại guardrail_output."""
    import uuid
    from unittest.mock import patch
    from src.agent.graph import _get_graph, reset_graph
    ...
    cases = [
        ...
        ("Thời tiết Hà Nội thế nào?", {"respond_inline"}),
        ...
        (
            ...
            {"orchestrator", "orchestrator_collect", "retrieve_schema"},
        ),
    ]
    ...
    with patch("src.llm.client.use_offline_tools", return_value=True), \
        ...
        for question, expected_subset in cases:
            ...
            assert "guardrail_output" in node_ids, f"{question!r} missing guardrail_output"
            assert node_ids.index("guardrail_output") > node_ids.index("respond")
            assert node_ids[-1] == "guardrail_output", f"{question!r} last node={node_ids[-1]}"
...
def test_all_events_multihop_routes_through_respond_with_template():
    ...
    assert node_ids.count("retrieve_schema") == 1
    assert node_ids.count("execute_sql") == 1
    assert len(state.get("orchestrator_step_results") or []) >= 5
