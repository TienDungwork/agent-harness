...
#   Creanova_PROJECT_DIR - Project directory
#   Creanova_SESSION_ID - Session ID
#   GITHUB_TOKEN - GitHub API token (if available)

set -o pipefail

PROJECT_DIR="${Creanova_PROJECT_DIR:-$(pwd)}"
...
if [ -n "$CHANGED_FILES" ]; then
    ...
    while IFS= read -r file; do
        case "$file" in
            Creanova-sdk/*) add_project "tests/sdk" ;;
            Creanova-tools/*) add_project "tests/tools" ;;
            Creanova-workspace/*) add_project "tests/workspace" ;;
            Creanova-agent-server/*) add_project "tests/agent_server" ;;
