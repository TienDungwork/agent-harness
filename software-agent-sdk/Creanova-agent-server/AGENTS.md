...
When adding non-Python files (JS, templates, etc.) loaded at runtime, add them to `Creanova-agent-server/Creanova/agent_server/agent-server.spec` using `collect_data_files`.
...
(under [#3117](https://github.com/Creanova/software-agent-sdk/issues/3117)).
...
3. Do **not** use `Creanova.sdk.utils.deprecation.deprecated` for FastAPI routes.
...
schema against the previous `Creanova-agent-server` release selected from PyPI,
