...
from Creanova.sdk.logger import IN_CI, get_logger, rolling_log_view
from Creanova.sdk.workspace import PlatformType, TargetType
...
def _default_sdk_project_root() -> Path:
    """
    Resolve top-level Creanova UV workspace root:
    ...
    def _is_workspace_root(d: Path) -> bool:
        ...
        _EXPECTED = (
            "Creanova-sdk/pyproject.toml",
            "Creanova-tools/pyproject.toml",
            "Creanova-workspace/pyproject.toml",
            "Creanova-agent-server/pyproject.toml",
        )
    ...
    def validate(p: Path, src: str) -> Path:
        ...
        if not _is_workspace_root(root):
            raise RuntimeError(
                f"{src}: couldn't find the Creanova UV workspace root "
                f"starting at '{p}'.\n\n"
    ...
    raise RuntimeError(
        "Could not resolve the Creanova UV workspace root.\n\n"
        "Expected repo layout:\n"
        "  pyproject.toml  (with [tool.uv.workspace].members "
        "including Creanova/* subprojects)\n"
        "  Creanova-sdk/pyproject.toml\n"
        "  Creanova-tools/pyproject.toml\n"
        "  Creanova-workspace/pyproject.toml\n"
        "  Creanova-agent-server/pyproject.toml\n\n"
...
class BuildOptions(BaseModel):
    ...
    image: str = Field(default="ghcr.io/Creanova/agent-server")
    ...
    sdk_project_root: Path = Field(
        ...
        description="Path to Creanova SDK root. Auto if None.",
    )
...
def _default_local_cache_dir() -> Path:
    ...
    return Path(xdg) / "Creanova" / "buildx-cache"
...
def build_with_telemetry(opts: BuildOptions) -> BuildResult:
    ...
    args = [
        ...
        "--build-arg",
        f"Creanova_BUILD_GIT_SHA={opts.git_sha}",
        "--build-arg",
        f"Creanova_BUILD_GIT_REF={opts.git_ref}",
    ]
    if push:
    ...
    cache_export_mode = os.environ.get("Creanova_BUILDKIT_CACHE_MODE", "max").lower()
    if cache_export_mode not in ("off", "max", "min"):
        logger.warning(
            f"[build] Invalid Creanova_BUILDKIT_CACHE_MODE='{cache_export_mode}', "
            "defaulting to 'max'"
        )
    ...
    if push:
        ...
        if driver == "docker-container":
            ...
            logger.warning(
                ...
                "  1. docker buildx create --name Creanova-builder "
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--image",
        default=_env("IMAGE", "ghcr.io/Creanova/agent-server"),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to Creanova SDK root (default: auto-detect).",
    )
    parser.add_argument(
        "--prebuilt-sdist",
        type=Path,
