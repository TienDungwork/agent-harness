"""Plugins service for Creanova Agent Server.
...
* Installed-plugin management — thin wrappers over the SDK's installed-plugins
  subsystem (``Creanova.sdk.plugin``) — plus listing the locally-available
  plugins.
* The *plugins-only* marketplace catalog. It returns only true plugins from the
  Creanova extensions marketplace — entries whose ``source`` lives under
...
def service_list_available_plugins(
    ...
    auto-load" ticket (``Creanova.sdk.plugin.discovery``). It is imported lazily
    so this module imports cleanly before that ticket is merged; this endpoint
    becomes functional once it lands.
    """
    from Creanova.sdk.plugin import load_available_plugins  # type: ignore
...
# The Creanova extensions marketplace lists both skills and true plugins under
...
def service_get_plugins_marketplace_catalog(
    ...
    Args:
        ...
        installed_dir: Directory of installed plugins to check status against.
            Defaults to ``~/.Creanova/plugins/installed/``.
...
def _fetch_plugin_catalog_entries(
    ...
    # ``.plugin/`` or ``.claude-plugin/`` — the real Creanova/extensions layout,
