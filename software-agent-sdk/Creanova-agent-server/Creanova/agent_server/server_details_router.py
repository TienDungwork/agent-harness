...
from Creanova.tools.terminal.timeout_policy import (
...
class ServerInfo(BaseModel):
    uptime: float
    idle_time: float
    title: str = "Creanova Agent Server"

    version: str = Field(
        default_factory=lambda: _package_version("Creanova-agent-server")
    )
    sdk_version: str = Field(default_factory=lambda: _package_version("Creanova-sdk"))
    tools_version: str = Field(
        default_factory=lambda: _package_version("Creanova-tools")
    )
    workspace_version: str = Field(
        default_factory=lambda: _package_version("Creanova-workspace")
    )

    build_git_sha: str = Field(
        default_factory=lambda: os.environ.get("Creanova_BUILD_GIT_SHA", "unknown")
    )
    build_git_ref: str = Field(
        default_factory=lambda: os.environ.get("Creanova_BUILD_GIT_REF", "unknown")
    )
