...
from Creanova.agent_server.conversation_service import ConversationService
from Creanova.agent_server.dependencies import get_conversation_service
from Creanova.agent_server.openai.models import (
    OpenAIChatCompletionRequest,
    OpenAIChatCompletionResponse,
    OpenAIModelListResponse,
)
from Creanova.agent_server.openai.service import (
...
def check_openai_api_key(
    ...
    """Accept the same session key through Creanova and OpenAI auth shapes.
...
def _parse_observability_overrides(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail="X-Creanova-Observability-Metadata must be a JSON object",
        ) from exc
...
async def create_chat_completion(
    ...
    request: Request,
    response: Response,
    x_Creanova_server_conversation_id: Annotated[
        UUID | None, Header(alias="X-Creanova-ServerConversation-ID")
    ] = None,
    x_Creanova_observability_span_name: Annotated[
        str | None, Header(alias="X-Creanova-Observability-Span-Name")
    ] = None,
    x_Creanova_observability_tags: Annotated[
        str | None, Header(alias="X-Creanova-Observability-Tags")
    ] = None,
    x_Creanova_observability_metadata: Annotated[
        str | None, Header(alias="X-Creanova-Observability-Metadata")
    ] = None,
    ...
    result = await run_chat_completion(
        ...
        reusable_conversation_id=x_Creanova_server_conversation_id,
        observability_overrides=_parse_observability_overrides(
            span_name=x_Creanova_observability_span_name,
            tags=x_Creanova_observability_tags,
            metadata=x_Creanova_observability_metadata,
        ),
    )
    ...
    response.headers["X-Creanova-ServerConversation-ID"] = conversation_id
