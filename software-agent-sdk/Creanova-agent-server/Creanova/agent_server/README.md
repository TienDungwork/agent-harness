...
The Creanova Agent Server is a minimal REST API and WebSocket server that provides a programmatic interface for interacting with Creanova AI agents. It uses the local filesystem to store conversations, events, and workspace files, making it ideal for development, testing, and lightweight deployments.
...
uv run python -m Creanova.agent_server

# Start with custom host and port
uv run python -m Creanova.agent_server --host localhost --port 3000

# Start with auto-reload (for dev)
uv run python -m Creanova.agent_server --reload
```
...
|----------|-------------|---------|
| `Creanova_AGENT_SERVER_CONFIG_PATH` | Path to JSON configuration file | `workspace/Creanova_agent_server_config.json` |
...
Create a JSON configuration file (default: `workspace/Creanova_agent_server_config.json`):
...
```
workspace/
├── Creanova_agent_server_config.json    # Configuration file
...
# Run with coverage
uv run pytest tests/agent_server/ --cov=Creanova.agent_server
```
...
3. **Configuration not found**: Check the `Creanova_AGENT_SERVER_CONFIG_PATH` environment variable
