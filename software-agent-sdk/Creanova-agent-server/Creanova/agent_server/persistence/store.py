...
Following the same pattern as Creanova app-server's FileSettingsStore
...
from Creanova.agent_server.persistence.models import (
    CustomSecret,
    PersistedSettings,
    PersistedWorkspaces,
    Secrets,
...
from Creanova.sdk.profiles.agent_profile_store import AgentProfileStore
...
DEFAULT_PERSISTENCE_DIR = Path("workspace/.Creanova")
...
def _get_persistence_dir(config: Config | None = None) -> Path:
    ...
    if config is not None:
        return config.conversations_path.parent / ".Creanova"
...
def _get_profile_persistence_dir() -> Path:
    ...
    ``OH_PERSISTENCE_DIR`` they fall back to the user's ``~/.Creanova``
...
def get_llm_profile_store() -> LLMProfileStore:
    ...
    when set, else ``~/.Creanova``. This honors isolated agent-server
