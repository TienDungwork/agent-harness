"""Hooks service for Creanova Agent Server.
...
Hook Sources:
- Project hooks: {workspace}/.Creanova/hooks.json
- User hooks: ~/.Creanova/hooks.json (future)
"""
...
def load_hooks_from_workspace(project_dir: str | None = None) -> HookConfig | None:
    """Load hooks from the workspace .Creanova/hooks.json file.

    This function reads the hooks configuration from the project's
    .Creanova/hooks.json file if it exists.
    ...
    hooks_path = Path(project_dir) / ".Creanova" / "hooks.json"
