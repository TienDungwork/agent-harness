...
from Creanova.agent_server.config import Config, WebhookSpec
from Creanova.agent_server.conversation_lease import (
    DEFAULT_LEASE_TTL_SECONDS,
    ConversationLeaseHeldError,
)
from Creanova.agent_server.event_service import (
...
from Creanova.agent_server.pub_sub import Subscriber
from Creanova.agent_server.server_details_router import update_last_execution_time
from Creanova.agent_server.skills_service import discover_profile_skills
from Creanova.agent_server.utils import safe_rmtree, utc_now
from Creanova.sdk import LLM, AgentContext, Event, Message
...
from Creanova.sdk.conversation.title_utils import (
    extract_message_text,
    generate_title_from_message,
)
from Creanova.sdk.event import MessageEvent
from Creanova.sdk.event.conversation_state import ConversationStateUpdateEvent
from Creanova.sdk.git.exceptions import GitCommandError, GitRepositoryError
from Creanova.sdk.git.utils import run_git_command, validate_git_repository
from Creanova.sdk.mcp.utils import MCPToolProvider
from Creanova.sdk.tool import BROWSER_TOOL_NAME, Tool, is_tool_usable
...
def _create_conversation_worktree(
    ...
    branch = f"Creanova/{conversation_id}"
...
def _resolve_agent_from_profile(
    ...
    """
    from Creanova.agent_server.persistence.store import (
        get_agent_profile_store,
        get_llm_profile_store,
    )
    from Creanova.sdk.profiles.resolver import ProfileNotFound, resolve_agent_profile
    from Creanova.sdk.settings.model import CreanovaAgentSettings
    ...
    # Creanova profiles get the discovered catalog minus their ``disabled_skills``
    ...
    if profile.agent_kind == "Creanova":
        try:
    ...
    if isinstance(settings_config, CreanovaAgentSettings):
...
def _register_agent_definitions(
    ...
    *,
    ...
    """
    from Creanova.sdk.subagent.registry import (
...
class ConversationService:
    ...
    async def _search_conversations(
        ...
        for conversation_id, record in records[start_index:]:
            if len(items) >= limit:
                ...
                break
            conversation_info = await self._conversation_info(conversation_id, record)
    ...
    async def _start_conversation(
        ...
        if request.agent_profile_id is not None:
            ...
            from Creanova.agent_server.persistence import (
                PersistedSettings,
                get_settings_store,
            )
    ...
    def get_instance(cls, config: Config) -> "ConversationService":
        ...
        from Creanova.agent_server.mcp_oauth_store import (
            create_settings_backed_mcp_tool_provider,
        )
        from Creanova.agent_server.persistence import get_settings_store
