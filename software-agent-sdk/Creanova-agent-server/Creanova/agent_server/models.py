...
from Creanova.sdk.conversation.request import (  # re-export for backward compat
...
from Creanova.sdk.conversation.types import ConversationTags
from Creanova.sdk.event.base import Event
from Creanova.sdk.hooks import HookConfig
from Creanova.sdk.llm.message import (  # re-export
...
from Creanova.sdk.utils import CreanovaUUID, utc_now
from Creanova.sdk.utils.models import (
...
class StoredConversation(StartConversationRequest):
    ...
    forked_from_conversation_id: CreanovaUUID | None = Field(
        default=None,
        description=(
...
class _ConversationInfoBase(BaseModel):
    ...
    created_at: datetime = Field(default_factory=utc_now)
    updated_at: datetime = Field(default_factory=utc_now)
    # Plain ``UUID`` (not ``CreanovaUUID``) so it JSON-serializes dashed, exactly
    ...
    current_model_id: str | None = Field(
        ...
        description=(
            ...
            "still initializing. Native Creanova agents leave this ``None`` — "
    ...
    available_models: list[ACPModelInfo] = Field(
        ...
        description=(
            ...
            "the (UNSTABLE) capability and for native Creanova agents. "
    ...
    supports_runtime_model_switch: bool = Field(
        ...
        description=(
            ...
            "``False`` for native "
            "Creanova agents, for a known provider that declares no support, "
...
def trim_conversation_response_skills(info: ConversationInfo) -> ConversationInfo:
    ...
    actually reads from the HTTP response (agent-canvas, Creanova
...
class BashEventPage(CreanovaModel):
