...
from Creanova.tools.terminal.constants import (
    HISTORY_LIMIT,
    TMUX_SESSION_HEIGHT,
    TMUX_SESSION_WIDTH,
    TMUX_SOCKET_NAME,
)
from Creanova.tools.terminal.env import (
    build_terminal_env,
    normalize_terminal_env,
)
from Creanova.tools.terminal.metadata import CmdOutputMetadata
from Creanova.tools.terminal.terminal import TerminalInterface
from Creanova.tools.terminal.terminal.interface import parse_ctrl_key
...
class TmuxTerminal(TerminalInterface):
    ...
    def initialize(self) -> None:
        ...
        # Use a dedicated socket to isolate Creanova sessions from the user's tmux
        self.server = libtmux.Server(socket_name=TMUX_SOCKET_NAME, environment=env)
        _shell_command = "/bin/bash"
        if self.username in ["root", "Creanova"]:
        ...
        session_name = f"Creanova-{self.username}-{uuid.uuid4()}"
    ...
    def is_running(self) -> bool:
        ...
        try:
            ...
            from Creanova.tools.terminal.constants import CMD_OUTPUT_PS1_END
