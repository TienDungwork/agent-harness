"""Discovery of locally-available plugins for Creanova SDK.
...
from Creanova.sdk.plugin.installed import (
    get_installed_plugins_dir,
    load_installed_plugins,
)
from Creanova.sdk.plugin.plugin import Plugin
from Creanova.sdk.skills.skill import _find_git_repo_root
...
def _load_plugins_from_dir(plugins_dir: Path, skip_dirs: set[Path]) -> list[Plugin]:
    ...
    ``~/.Creanova/plugins/installed`` — a child of ``~/.Creanova/plugins`` — and
...
def load_user_plugins() -> list[Plugin]:
    ...
    Scans ``~/.agents/plugins`` then ``~/.Creanova/plugins`` (earlier wins on a
    name conflict), then appends enabled installed plugins from
    ``~/.Creanova/plugins/installed`` (lower precedence than directory plugins).
...
def load_project_plugins(work_dir: str | Path) -> list[Plugin]:
    ...
    Scans ``{root}/.agents/plugins`` and ``{root}/.Creanova/plugins`` for the
