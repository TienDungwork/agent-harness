...
Callers outside the SDK (e.g. ``Creanova-agent-server``, the ``Creanova``
frontend, and the ``@Creanova/typescript-client`` mirror) can import
...
class ACPFileSecretSpec(BaseModel):
    ...
    :attr:`secret_name` — and :class:`~Creanova.sdk.agent.ACPAgent` materialises
    ...
    :attr:`~Creanova.sdk.agent.ACPAgent.acp_file_secrets` to support other ACP
...
class ACPProviderInfo:
    ...
    pointing at it) that :class:`~Creanova.sdk.agent.ACPAgent` materialises
    ...
    :meth:`~Creanova.sdk.settings.model.ACPAgentSettings.resolve_acp_command`
    ...
    :attr:`~Creanova.sdk.agent.ACPAgent.acp_isolate_data_dir`.
    """
...
# Creanova-agent-server/Creanova/agent_server/docker/Dockerfile — a bump must
...
def default_acp_file_secrets() -> tuple[ACPFileSecretSpec, ...]:
    ...
    :attr:`~Creanova.sdk.agent.ACPAgent.acp_file_secrets`, which a downstream
