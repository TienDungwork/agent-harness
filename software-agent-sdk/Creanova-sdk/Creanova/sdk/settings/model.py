...
from Creanova.sdk.context.agent_context import AgentContext
from Creanova.sdk.conversation.request import SendMessageRequest
...
from Creanova.sdk.llm.utils.Creanova_provider import (
    canonicalize_Creanova_llm_payload,
)
...
from Creanova.sdk.tool import Tool
from Creanova.sdk.utils.pydantic_secrets import (
    serialize_secret,
    validate_secret,
)
...
if TYPE_CHECKING:
    from Creanova.sdk.agent import ACPAgent, Agent
    from Creanova.sdk.agent.base import AgentBase
    from Creanova.sdk.context.condenser import CondenserBase, LLMSummarizingCondenser
    from Creanova.sdk.critic.base import CriticBase
...
class SettingsFieldSchema(BaseModel):
    ...
    variant: str | None = Field(
        ...
        description=(
            ...
            "variant (``'Creanova'`` or ``'acp'``). The GUI filters fields by the "
...
class SettingsSectionSchema(BaseModel):
    ...
    variant: str | None = Field(
        ...
        description=(
            ...
            "variant (e.g. ``'Creanova'`` or ``'acp'``). The GUI filters sections by "
...
class NoOpCondenserSettings(CondenserSettings):
    ...
    def build_condenser(self, llm: LLM) -> CondenserBase | None:  # noqa: ARG002
        ...
        from Creanova.sdk.context.condenser import NoOpCondenser
...
class AgentSettingsBase(BaseModel):
    ...
    either the :class:`CreanovaAgentSettings` or :class:`ACPAgentSettings`
    ...
    def create_agent(self) -> AgentBase:
        ...
        Subclasses (:class:`CreanovaAgentSettings`, :class:`ACPAgentSettings`)
        override this to return the appropriate
        :class:`~Creanova.sdk.agent.base.AgentBase` subclass.
...
def _migrate_agent_settings_v1_to_v2(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    Before the v1.19.0 ``LLMAgentSettings`` → ``CreanovaAgentSettings`` rename,
    persisted payloads carried ``agent_kind: 'llm'``. The two classes are
    field-compatible (``LLMAgentSettings`` is a subclass of
    ``CreanovaAgentSettings`` that only narrows the discriminator literal),
    ...
    ``CreanovaAgentSettings`` (the canonical class) accept legacy data
    without losing any fields.
    """
    migrated = dict(payload)
    migrated["schema_version"] = 2
    if migrated.get("agent_kind") in ("llm", "openhands"):
        migrated["agent_kind"] = "Creanova"
    return migrated
...
def _migrate_agent_settings_v3_to_v4(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(llm, dict):
        migrated["llm"] = canonicalize_Creanova_llm_payload(llm)
...
class ConversationSettings(BaseModel):
    ...
    agent_settings: AgentSettingsConfig | None = Field(
        ...
        description=(
            ...
            "``CreanovaAgentSettings`` or ``ACPAgentSettings`` variant."
        ),
    )
...
AgentKind = Literal["Creanova", "llm", "acp"]
...
:data:`~Creanova.sdk.settings.acp_providers.ACP_PROVIDERS`.
"""


class CreanovaAgentSettings(AgentSettingsBase):
    ...
    agent_kind: Literal["Creanova"] = Field(
        default="Creanova",
        description=(
            "Discriminator for the ``AgentSettings`` union. ``'Creanova'`` selects "
            "the standard built-in Creanova agent."
        ),
    )
    agent: str = Field(
    ...
    tools: list[Tool] | None = Field(
        ...
        description=(
            ...
            "standard exec set (see Creanova.sdk.tool.defaults), plus the "
    ...
    def create_agent(self) -> Agent:
        ...
        """
        from Creanova.sdk.agent import Agent
        from Creanova.sdk.llm.auth.openai import create_subscription_llm_from_config
        from Creanova.sdk.tool.builtins import BUILT_IN_TOOLS, SwitchLLMTool
        from Creanova.sdk.tool.defaults import default_tool_specs
...
class ACPAgentSettings(AgentSettingsBase):
    ...
    :class:`CreanovaAgentSettings` do not apply here.
    ...
    mcp_config: dict[str, MCPServer] = Field(
        ...
        description=(
            ...
            "Creanova agent — where these become in-process MCP tools — the "
    ...
    acp_isolate_data_dir: bool = Field(
        ...
        description=(
            ...
            ":attr:`~Creanova.sdk.agent.ACPAgent.acp_isolate_data_dir`; off by "
    ...
    def api_key_env_var(self) -> str | None:
        ...
        Delegates to the :data:`~Creanova.sdk.settings.acp_providers.ACP_PROVIDERS`
    ...
    def resolve_acp_command(self) -> list[str]:
        ...
        up the default from :data:`~Creanova.sdk.settings.acp_providers.ACP_PROVIDERS`.
    ...
    def create_agent(self) -> ACPAgent:
        ...
        registry.
        """
        from Creanova.sdk.agent import ACPAgent
...
class LLMAgentSettings(CreanovaAgentSettings):
    """Legacy ``agent_kind='llm'`` variant of :class:`CreanovaAgentSettings`.

    ``LLMAgentSettings`` was the public class name before the v1.19.0 rename.
    The public import aliases (``from Creanova.sdk import LLMAgentSettings`` and
    ``from Creanova.sdk.settings import LLMAgentSettings``) were removed in
    v1.24.0 — use :class:`CreanovaAgentSettings` for all new code.

    The class itself is retained (reachable at
    ``Creanova.sdk.settings.model.LLMAgentSettings``) because it remains a
    ...
    # to CreanovaAgentSettings ("Creanova").
    agent_kind: Literal["llm"] = Field(  # type: ignore[assignment]
        default="llm",
        description=(
            "Discriminator for the ``AgentSettings`` union. ``'llm'`` selects "
            "the standard LLM-backed agent. Deprecated; use ``'Creanova'``."
        ),
    )


def _agent_settings_discriminator(value: Any) -> str:
    """Discriminator for :data:`AgentSettingsConfig` — defaults to ``'Creanova'``.

    Existing persisted payloads predate ``agent_kind`` and carry only
    Creanova-agent fields. Treating a missing discriminator as ``'Creanova'``
    ...
    """
    if isinstance(value, BaseModel):
        return getattr(value, "agent_kind", "Creanova")
    if isinstance(value, dict):
        return value.get("agent_kind", "Creanova")
    return "Creanova"


AgentSettingsConfig = Annotated[
    Annotated[CreanovaAgentSettings, Tag("Creanova")]
...
_AGENT_SETTINGS_ADAPTER: TypeAdapter[
    CreanovaAgentSettings | LLMAgentSettings | ACPAgentSettings
...
def validate_agent_settings(
    ...
    *,
    ...
    ``CreanovaAgentSettings`` rename.
    """
    if isinstance(data, CreanovaAgentSettings | ACPAgentSettings):
        return data
    payload = _apply_persisted_migrations(
        data,
    ...
    # always returns a ``{Creanova, acp}`` variant.
    if payload.get("agent_kind") in ("llm", "openhands"):
        payload["agent_kind"] = "Creanova"
...
def apply_agent_settings_diff(
    ...
    *,
    context: Mapping[str, Any] | None = None,
) -> CreanovaAgentSettings | ACPAgentSettings:
    ...
    * When ``diff`` changes ``agent_kind``, start from a *fresh* base for the
      ...
      validation (ACP's nullable ``agent_context`` is invalid for Creanova) or
...
def default_agent_settings() -> CreanovaAgentSettings:
    """Return a default :class:`CreanovaAgentSettings` instance.
    ...
    """
    return CreanovaAgentSettings()


def create_agent_from_settings(
    settings: CreanovaAgentSettings | ACPAgentSettings,
) -> AgentBase:
    """Dispatch to the variant's ``create_agent()`` method.

    Returns either :class:`~Creanova.sdk.agent.Agent` (LLM variant) or
    :class:`~Creanova.sdk.agent.ACPAgent` (ACP variant).
    """
...
def export_agent_settings_schema() -> SettingsSchema:
    ...
    carry a ``variant`` tag (``'Creanova'``, ``'acp'``, or ``None`` for
    shared) so the frontend can filter by the page's variant.
    """
    llm_schema = CreanovaAgentSettings.export_schema()
    ...
    _merge(llm_schema, default_variant="Creanova")
