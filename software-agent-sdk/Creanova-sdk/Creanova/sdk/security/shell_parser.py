...
- ``Creanova-tools`` for command splitting and escape doubling
  (``terminal.utils.command``).
- ``Creanova-sdk`` for AST-aware security detection
  (the planned ``security.defense_in_depth.*`` analyzers).

Hosting the parser inside ``Creanova-sdk`` lets both consumers depend
...
``Creanova-tools/Creanova/tools/terminal/utils/command.py``: sharing
