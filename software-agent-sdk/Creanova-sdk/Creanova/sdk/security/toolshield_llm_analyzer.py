...
Example (bare guardrail):
    ...
    >>> from Creanova.sdk.security import ToolShieldLLMSecurityAnalyzer
...
Example (with ToolShield tool-specific safety experiences):
    Install the optional extra ``pip install Creanova-sdk[toolshield]``
...
# per the actor's system prompt (see Creanova.sdk.event.llm_convertible.
...
class ToolShieldLLMSecurityAnalyzer(SecurityAnalyzerBase):
    ...
    ``pip install Creanova-sdk[toolshield]`` and pass the rendered
    ...
    safety_experiences: str = Field(
        ...
        description=(
            ...
            "(``pip install Creanova-sdk[toolshield]``). Callers with "
