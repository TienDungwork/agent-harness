...
Example usage:
    from Creanova.sdk import register_agent, Agent, AgentContext
...
from Creanova.sdk.subagent.load import (
    load_project_agents,
    load_user_agents,
)
from Creanova.sdk.subagent.schema import AgentDefinition
from Creanova.sdk.utils.deprecation import warn_deprecated
...
def agent_definition_to_factory(
    ...
    if agent_def.skills:
        from Creanova.sdk.skills import load_available_skills
...
def register_file_agents(work_dir: str | Path) -> list[str]:
    ...
    `.Creanova/agents`, and user-level `~/.agents/agents` and `~/.Creanova/agents`
    directories.

    Project-level definitions take priority over user-level ones, and within
    each level `.agents/` takes priority over `.Creanova/`.
