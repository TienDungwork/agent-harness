...
`.agents/agents` or `.Creanova/agents` directories at the project or user level.
...
Directory convention (in priority order):
    ...
    ~/.Creanova/               # User-level, legacy (lowest file priority)
        agents/
            my-global-agent.md
...
Priority (highest to lowest):
  ...
  4. Project-level `.Creanova/agents/*.md`
  5. User-level `~/.agents/agents/*.md`
  6. User-level `~/.Creanova/agents/*.md`
"""
...
def load_project_agents(project_dir: str | Path) -> list[AgentDefinition]:
    ...
    Searches for
        ...
        - project_dir/.Creanova/agents (in that order).
...
def discover_agents(
    ...
    *,
    ...
    ``Creanova-tools`` (not an SDK dependency), so callers wanting them add
