...
This package (`Creanova.sdk.subagent`) centralizes **subagent discovery** and **registration**.
...
2. `{project}/.Creanova/agents/*.md`

**User-level:**

3. `~/.agents/agents/*.md`
4. `~/.Creanova/agents/*.md`
...
3. **Project** file-based agents
   - `{project}/.agents/agents/*.md` then `{project}/.Creanova/agents/*.md`
4. **User** file-based agents
   - `~/.agents/agents/*.md` then `~/.Creanova/agents/*.md`
...
1. **Within a level** (`load_project_agents` / `load_user_agents`):
   - `.agents/agents` wins over `.Creanova/agents` for the same agent name.
...
- Docs PR tracking this feature: https://github.com/Creanova/docs/pull/358
