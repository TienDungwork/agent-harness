...
from Creanova.sdk.git.cached_repo import GitHelper, try_cached_clone_or_update
from Creanova.sdk.logger import get_logger
from Creanova.sdk.skills.exceptions import SkillValidationError
from Creanova.sdk.utils.path import to_posix_path


if TYPE_CHECKING:
    from Creanova.sdk.skills.skill import Skill, SkillResources
...
def load_and_categorize(
    ...
    from Creanova.sdk.skills.skill import Skill

    skill = Skill.load(path, skill_base_dir)

    # AgentSkills (SKILL.md directories) are a separate category from Creanova skills.
...
def get_skills_cache_dir() -> Path:
    ...
    Returns:
        Path to the skills cache directory (~/.Creanova/cache/skills).
    """
    cache_dir = Path.home() / ".Creanova" / "cache" / "skills"
...
def update_skills_repository(
    ...
    Uses the shared git caching infrastructure from Creanova.sdk.git.cached_repo.
...
def discover_skill_resources(skill_dir: Path) -> SkillResources:
    ...
    """
    # Import here to avoid circular dependency
    from Creanova.sdk.skills.skill import SkillResources
