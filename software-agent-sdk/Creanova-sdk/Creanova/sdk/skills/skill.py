...
from Creanova.sdk.skills.exceptions import SkillError, SkillValidationError
from Creanova.sdk.skills.execute import render_content_with_commands
from Creanova.sdk.skills.trigger import (
    KeywordTrigger,
    PathTrigger,
    TaskTrigger,
)
from Creanova.sdk.skills.types import InputMetadata
...
from Creanova.sdk.utils import DEFAULT_TRUNCATE_NOTICE, maybe_truncate
from Creanova.sdk.utils.path import to_posix_path
...
class Skill(BaseModel):
    ...
    This model supports both Creanova-specific fields and AgentSkills standard
    ...
    def load(
        ...
        Supports both Creanova-specific frontmatter fields and AgentSkills
        ...
        if path.name.lower() == "skill.md":
            ...
            return cls._load_legacy_Creanova_skill(path, file_content, skill_base_dir)
    ...
    @classmethod
    def _load_legacy_Creanova_skill(
        cls, path: Path, file_content: str, skill_base_dir: Path | None
    ) -> "Skill":
        """Load a skill from a legacy Creanova-format file.
...
def load_skills_from_dir(
    ...
    - Creanova format: skills/*.md files
    - AgentSkills format: skills/skill-name/SKILL.md directories

    Note, legacy repo instructions will not be loaded here.

    Args:
        skill_dir: Path to the skills directory (e.g. .Creanova/skills)
...
USER_SKILLS_DIRS = [
    ...
    Path.home() / ".Creanova" / "skills",
    Path.home() / ".Creanova" / "microagents",  # Legacy support
]
...
def load_user_skills() -> list[Skill]:
    ...
    Searches for skills in ~/.agents/skills/, ~/.Creanova/skills/, and
    ~/.Creanova/microagents/ (legacy). Skills from all directories are merged,
    with earlier entries in USER_SKILLS_DIRS taking precedence for duplicate
    names.

    Also loads enabled installed skills from ~/.Creanova/skills/installed/
    ...
    try:
        from Creanova.sdk.skills.installed import load_installed_skills
...
def load_project_skills(work_dir: str | Path) -> list[Skill]:
    ...
    Searches for skills in {work_dir}/.agents/skills/, {work_dir}/.Creanova/skills/,
    and {work_dir}/.Creanova/microagents/ (legacy).
    ...
    Use .agents/skills for new skills. .Creanova/skills is the legacy Creanova
    location, and .Creanova/microagents is deprecated.

    Example: If "my-skill" exists in both .agents/skills/ and .Creanova/skills/,
    ...
    # search root. This ensures they are loaded even if .Creanova/skills doesn't
    # exist.
    ...
    # Load project-specific skills from .agents/skills, .Creanova/skills,
    # and legacy microagents (priority order; first wins for duplicates)
    for root in search_roots:
        project_skills_dirs = [
            root / ".agents" / "skills",
            root / ".Creanova" / "skills",
            root / ".Creanova" / "microagents",  # Legacy support
        ]
...
PUBLIC_SKILLS_REPO = "https://github.com/Creanova/extensions"
...
def load_marketplace_skill_names(
    ...
    Uses the existing Marketplace model from Creanova.sdk.plugin to parse
    ...
    """
    from Creanova.sdk.marketplace import Marketplace
...
def load_public_skills(
    ...
    """Load skills from the public Creanova skills repository.

    This function maintains a local git clone of the public skills registry at
    https://github.com/Creanova/extensions. On first run, it clones the repository
    to ~/.Creanova/skills-cache/. On subsequent runs within the same process, it
    ...
    Example:
        ...
        >>> from Creanova.sdk.skills import load_public_skills
        >>>
        >>> # Load public skills
...
def load_available_skills(
    ...
    *,
    ...
    Args:
        ...
        include_public: Load public skills from the Creanova extensions repo.
