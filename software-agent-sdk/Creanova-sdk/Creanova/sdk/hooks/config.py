...
from Creanova.sdk.hooks.types import HookEventType
...
class HookConfig(BaseModel):
    ...
    Hooks can be configured either by loading from `.Creanova/hooks.json` or
    by directly instantiating with typed fields:
        ...
        # Load from JSON file:
        config = HookConfig.load(".Creanova/hooks.json")
    """
    ...
    def load(
        ...
        """Load config from path or search .Creanova/hooks.json locations.

        Args:
            path: Explicit path to hooks.json file. If provided, working_dir is ignored.
            working_dir: Project directory for discovering .Creanova/hooks.json.
        ...
        if path is None:
            ...
            search_paths = [
                base_dir / ".Creanova" / "hooks.json",
                Path.home() / ".Creanova" / "hooks.json",
            ]
