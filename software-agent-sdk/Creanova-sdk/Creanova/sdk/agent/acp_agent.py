...
The Agent Client Protocol (ACP) lets Creanova power conversations using
...
the ACPAgent relays user messages and collects the response. Creanova
...
from Creanova.sdk.agent.acp_models import ACPModelInfo
...
from Creanova.sdk.llm import LLM, ImageContent, Message, MessageToolCall, TextContent
from Creanova.sdk.logger import get_logger
from Creanova.sdk.mcp.config import MCPServer
from Creanova.sdk.observability.laminar import maybe_init_laminar, observe
from Creanova.sdk.settings.acp_providers import (
    ACPFileSecretSpec,
...
from Creanova.sdk.tool import Tool  # noqa: TC002
from Creanova.sdk.tool.builtins.finish import FinishAction, FinishObservation
from Creanova.sdk.utils import maybe_truncate
from Creanova.sdk.utils.pydantic_secrets import (
    serialize_secret,
    validate_secret,
)
from Creanova.sdk.utils.redact import redact_text_secrets
...
def _mcp_config_to_acp_servers(
    ...
    """Translate Creanova MCP servers into ACP MCP server objects.
...
class _CreanovaACPBridge:
    """Bridge between Creanova and ACP that accumulates session updates.
...
class ACPAgent(AgentBase):
    ...
    _client: Any = PrivateAttr(default=None)  # _CreanovaACPBridge
    ...
    @property
    def supports_Creanova_tools(self) -> bool:
        """``False`` — the ACP server manages its own toolset."""
        return False

    @property
    def supports_Creanova_mcp(self) -> bool:
        """``False`` — Creanova does not create in-process MCP tools here.
    ...
    def supports_runtime_model_switch(self) -> bool:
        ...
        See
        :meth:`~Creanova.sdk.conversation.impl.local_conversation.LocalConversation.switch_acp_model`.
        """
    ...
    def init_state(
        ...
        from Creanova.sdk.utils.async_executor import AsyncExecutor
    ...
    def _acp_file_secret_dir(self, state: ConversationState, subdir: str) -> Path:
        ...
        if state.persistence_dir:
            ...
            root = Path(state.workspace.working_dir) / ".Creanova" / "acp" / subdir
    ...
    def _reset_client_for_turn(
        ...
        ``_CreanovaACPBridge.session_update`` as tool-call notifications
    ...
    def _finalize_successful_turn(
        ...
        # _CreanovaACPBridge.session_update: one early ``started`` event per
