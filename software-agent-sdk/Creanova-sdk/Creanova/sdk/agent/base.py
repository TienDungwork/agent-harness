...
from Creanova.sdk.context.condenser import CondenserBase
from Creanova.sdk.context.prompts.presets import PromptPreset, create_registry
from Creanova.sdk.context.prompts.prompt import render_template
from Creanova.sdk.context.prompts.section import Platform, PromptContext
from Creanova.sdk.critic.base import CriticBase
from Creanova.sdk.llm import LLM
from Creanova.sdk.llm.utils.model_prompt_spec import get_model_prompt_spec
...
from Creanova.sdk.tool.builtins.vision_inspect import (
    VisionInspectTool,
    has_vision_profile_available,
)
from Creanova.sdk.utils.models import DiscriminatedUnionMixin


if TYPE_CHECKING:
    from Creanova.sdk.conversation import ConversationState, LocalConversation
...
# -- SOUL.md loader -------------------------------------------------------
# SOUL.md is the agent's identity file (~/.Creanova/SOUL.md).  When present
# it replaces the default identity in the system prompt.

_SOUL_PATH = os.path.join(os.path.expanduser("~"), ".Creanova", "SOUL.md")
_DEFAULT_SOUL = (
...
def _load_soul_md() -> str:
    """Load ``~/.Creanova/SOUL.md``, falling back to the built-in default."""
    try:
...
class AgentBase(DiscriminatedUnionMixin, ABC):
    """Abstract base class for Creanova agents.
    ...
    system_prompt: str | None = Field(
        ...
        description=(
            ...
            "task).  Setting this will override Creanova' built-in system "
    ...
    def supports_Creanova_tools(self) -> bool:
        """``True`` if Creanova can inject tools into this agent.

        ``False`` for :class:`~Creanova.sdk.agent.acp_agent.ACPAgent` — the
    ...
    def supports_Creanova_mcp(self) -> bool:
        """``True`` if Creanova can create in-process MCP tools for this agent.

        ``False`` for :class:`~Creanova.sdk.agent.acp_agent.ACPAgent` — ACP
    ...
    def supports_condenser(self) -> bool:
        """``True`` if Creanova context condensing is supported for this agent.
    ...
    @property
    def agent_kind(self) -> Literal["Creanova", "acp"]:
