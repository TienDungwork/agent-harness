...
from Creanova.sdk.tool.schema import Action, Observation, Schema
...
def register_client_tools(specs: Sequence[ClientToolSpec]) -> list["Tool"]:
    ...
    Returns:
        A list of :class:`~Creanova.sdk.tool.spec.Tool` specs (one per input
        spec) to inject into an agent's ``tools`` so ``_initialize()`` can
        resolve them.
    """
    from Creanova.sdk.tool.registry import list_registered_tools, register_tool
    from Creanova.sdk.tool.spec import Tool
