"""Canonical default tool names for the standard Creanova agent.
...
the canonical defaults here lets ``Creanova-sdk`` (which must not import
``Creanova-tools``) default a toolset from data alone — ``Tool`` is a spec
(name + params) resolved to an implementation only at runtime via the registry.

``Creanova.tools.preset.default.get_default_tools`` remains the constructor
...
"""Names of the standard exec tools every default Creanova agent gets."""
...
def default_tool_specs(
    *,
    ...
    """Default tool specs for an Creanova agent whose settings carry no tools.
