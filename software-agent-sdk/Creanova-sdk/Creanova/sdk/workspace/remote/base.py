...
from Creanova.sdk.git.models import GitChange, GitDiff
from Creanova.sdk.logger import get_logger
from Creanova.sdk.mcp.config import MCPServer
from Creanova.sdk.settings import SecretsListResponse, SettingsResponse
from Creanova.sdk.workspace.base import BaseWorkspace
from Creanova.sdk.workspace.models import CommandResult, FileOperationResult
from Creanova.sdk.workspace.remote.remote_workspace_mixin import RemoteWorkspaceMixin
...
if TYPE_CHECKING:
    ...
    from Creanova.sdk.llm.llm import LLM
    from Creanova.sdk.secret import LookupSecret
    from Creanova.sdk.settings import CreanovaAgentSettings
    from Creanova.sdk.settings.model import ACPAgentSettings, LLMAgentSettings
...
class RemoteWorkspace(RemoteWorkspaceMixin, BaseWorkspace):
    """Remote workspace implementation that connects to an Creanova agent server.

    RemoteWorkspace provides access to a sandboxed environment running on a remote
    Creanova agent server. This is the recommended approach for production deployments
    ...
    def default_conversation_tags(self) -> dict[str, str] | None:
        ...
        Subclasses (e.g., CreanovaCloudWorkspace) can override this to provide
    ...
    # settings endpoints. Subclasses like CreanovaCloudWorkspace may override
    ...
    def _fetch_agent_settings(
        ...
        (``CreanovaAgentSettings`` or ``ACPAgentSettings``).
        """
    ...
    def get_mcp_config(self) -> dict[str, MCPServer]:
        ...
        # Runtime MCP tools only exist on CreanovaAgentSettings, not ACPAgentSettings.
        if not isinstance(settings, CreanovaAgentSettings):
            return {}
    ...
    def clone_repos(
        ...
        'Creanova-cli' instead of 'repo_0'). Automatically fetches GitHub,
    ...
    def _convert_skill_data_to_skill(self, skill_data: dict[str, Any]) -> "Skill":
        ...
        if triggers:
            # Determine trigger type based on content (same logic as Creanova)
    ...
    def load_skills_from_agent_server(
        ...
        Args:
            ...
            load_public: Load public skills from Creanova/extensions repo.
            load_user: Load user skills from ~/.Creanova/skills/.
