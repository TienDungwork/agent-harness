...
:data:`~Creanova.sdk.settings.model.AgentSettingsConfig` embeds the resolved
...
from Creanova.sdk.profiles.agent_profile import (
    ACPAgentProfile,
    CreanovaAgentProfile,
)
from Creanova.sdk.settings.acp_providers import get_acp_provider
from Creanova.sdk.settings.model import (
    AGENT_SETTINGS_SCHEMA_VERSION,
    AgentSettingsConfig,
    validate_agent_settings,
)
from Creanova.sdk.skills import Skill
from Creanova.sdk.utils.pydantic_secrets import REDACTED_SECRET_VALUE


if TYPE_CHECKING:
    from Creanova.sdk.llm.llm import LLM
    from Creanova.sdk.llm.llm_profile_store import LLMProfileLoader
...
class AgentProfileDiagnostics(BaseModel):
    ...
    # Skill selection (Creanova only). ``disabled_skills`` is a deny-list over
...
def _build_Creanova_settings(
    profile: CreanovaAgentProfile,
    llm: LLM,
    mcp_config: dict[str, MCPServer],
    filtered_skills: list[Skill],
) -> AgentSettingsConfig:
    """Compose the resolved ``CreanovaAgentSettings`` from a profile + LLM.
...
def resolve_agent_profile(
    profile: CreanovaAgentProfile | ACPAgentProfile,
    *,
    ...
    Creanova profile keeps all of it except the names in ``disabled_skills``.
    ...
    Raises:
        ProfileNotFound: ``llm_profile_ref`` does not exist (Creanova path).
    ...
    if isinstance(profile, CreanovaAgentProfile):
        ...
        return _build_Creanova_settings(profile, llm, filtered_mcp, filtered_skills)
...
def resolve_agent_profile_dry_run(
    ...
    *,
    ...
    # Skill selection report (Creanova only; ACP injects no user/public skills).
    ...
    if diagnostics.valid:
        ...
        try:
            if isinstance(profile, CreanovaAgentProfile):
                ...
                if llm is None:
                    raise RuntimeError(
                        "Creanova profile marked valid without a resolved LLM"
                    )
                settings = _build_Creanova_settings(
