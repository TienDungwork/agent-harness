...
:data:`~Creanova.sdk.settings.model.AgentSettingsConfig`: the profile carries
...
class ProfileVerificationSettings(BaseModel):
    ...
    :class:`~Creanova.sdk.settings.model.VerificationSettings`: ``critic_api_key``
...
class AgentProfileBase(BaseModel):
    ...
    (or an Creanova payload carrying ``acp_*``) is rejected rather than
    ...
    # selects them by *exclusion* (``CreanovaAgentProfile.disabled_skills``, a
...
class CreanovaAgentProfile(AgentProfileBase):
    """``agent_kind="Creanova"`` profile — references an LLM profile by name.

    Mirrors the configurable surface of
    :class:`~Creanova.sdk.settings.model.CreanovaAgentSettings`, except the
    ...
    agent_kind: Literal["Creanova"] = Field(
        default="Creanova",
        description=(
            "Discriminator for the ``AgentProfile`` union. ``'Creanova'`` "
            "selects the standard built-in Creanova agent."
        ),
    )
...
class ACPAgentProfile(AgentProfileBase):
    ...
    :data:`~Creanova.sdk.settings.acp_providers.ACP_PROVIDERS` registry. The
...
def _agent_profile_discriminator(value: Any) -> str:
    """Discriminator for :data:`AgentProfile` — defaults to ``'Creanova'``.

    A payload without an explicit ``agent_kind`` is treated as the standard
    Creanova variant, mirroring
    :func:`~Creanova.sdk.settings.model._agent_settings_discriminator`.
    """
...
AgentProfile = Annotated[
    Annotated[CreanovaAgentProfile, Tag("Creanova")]
...
def _migrate_v1_to_v2(payload: dict[str, Any]) -> dict[str, Any]:
    if (
        payload.get("agent_kind", "Creanova") == "Creanova"
...
_AGENT_PROFILE_ADAPTER: TypeAdapter[CreanovaAgentProfile | ACPAgentProfile] = (
...
def validate_agent_profile(
    ...
    *,
...
) -> CreanovaAgentProfile | ACPAgentProfile:
    ...
    """
    if isinstance(data, CreanovaAgentProfile | ACPAgentProfile):
        return data
