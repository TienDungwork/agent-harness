...
An ``CreanovaAgentProfile.llm_profile_ref`` is a soft FK onto an LLM-profile
...
:class:`~Creanova.sdk.profiles.agent_profile_store.AgentProfileStoreProtocol`,
...
from Creanova.sdk.profiles.agent_profile_store import PROFILE_NAME_REGEX


if TYPE_CHECKING:
    from Creanova.sdk.llm.llm_profile_store import LLMProfileMutator
    from Creanova.sdk.profiles.agent_profile_store import AgentProfileStoreProtocol
...
def _scan_referrers(
    ...
    instantiation). Only the Creanova variant carries ``llm_profile_ref``.
    """
    return [
        str(summary["name"])
        for summary in store.list_summaries()
        if summary.get("agent_kind", "Creanova") == "Creanova"
