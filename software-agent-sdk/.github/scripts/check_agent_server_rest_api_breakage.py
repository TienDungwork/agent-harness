#!/usr/bin/env python3
"""REST API breakage detection for Creanova-agent-server using oasdiff.
...
1) REST deprecations must use FastAPI/OpenAPI metadata
   - FastAPI route handlers must not use `Creanova.sdk.utils.deprecation.deprecated`.
...
AGENT_SERVER_PYPROJECT = REPO_ROOT / "Creanova-agent-server" / "pyproject.toml"
PYPI_DISTRIBUTION = "Creanova-agent-server"
...
sys.path = [
    str(source_tree / "Creanova-agent-server"),
    str(source_tree / "Creanova-sdk"),
    str(source_tree / "Creanova-tools"),
    str(source_tree / "Creanova-workspace"),
] + sys.path
...
def _fetch_pypi_metadata(distribution: str) -> dict:
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "Creanova-agent-server-openapi-check/1.0"},
        method="GET",
    )
    ...
    for node in ast.walk(tree):
        ...
        for decorator in node.decorator_list:
            ...
            if dotted_name in deprecated_names or (
                dotted_name == "Creanova.sdk.utils.deprecation.deprecated"
            ):
            ...
            if (
                ...
                if base_name in deprecation_module_names or (
                    base_name == "Creanova.sdk.utils.deprecation"
                ):
                    uses_sdk_deprecated = True
        ...
        if has_route_decorator and uses_sdk_deprecated:
            ...
            errors.append(
                ...
                "Creanova.sdk.utils.deprecation.deprecated; use the route "
...
def _find_sdk_deprecated_fastapi_routes(repo_root: Path) -> list[str]:
    app_root = repo_root / "Creanova-agent-server" / "Creanova" / "agent_server"
    errors: list[str] = []
