...
_PYPI_USER_AGENT = "Creanova-persisted-settings-compat/1.0"
...
try:
    from Creanova.sdk import Tool
except Exception:
    from Creanova.sdk.tool import Tool

import Creanova.sdk.settings as settings_mod

AgentSettingsCls = (
    getattr(settings_mod, "CreanovaAgentSettings", None)
...
try:
    from Creanova.agent_server.persistence import PersistedSettings
except Exception:
...
def generate_baseline_payloads(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="persisted-settings-baseline-") as tmp_dir:
        ...
        packages = [f"Creanova-sdk=={sdk_version}"]
        if agent_server_version is not None:
            packages.append(f"Creanova-agent-server=={agent_server_version}")
        ...
        for item in raw_cases:
            ...
            cases.append(
                BaselinePayloadCase(
                    source=(
                        f"PyPI baseline Creanova-sdk=={sdk_version}"
                        if agent_server_version is None
                        else (
                            f"PyPI baseline Creanova-sdk=={sdk_version}, "
                            f"Creanova-agent-server=={agent_server_version}"
                        )
                    ),
                    key=key,
                    surface_key=surface_key,
...
def _resolve_pypi_baselines() -> tuple[str | None, str | None, str | None]:
    sdk_current = read_version_from_pyproject(
        REPO_ROOT / "Creanova-sdk" / "pyproject.toml"
    )
    sdk_baseline = get_pypi_baseline_version("Creanova-sdk", sdk_current)
    if sdk_baseline is None:
        return None, None, None

    cutoffs = [get_pypi_release_cutoff("Creanova-sdk", sdk_baseline)]
    agent_server_current = read_version_from_pyproject(
        REPO_ROOT / "Creanova-agent-server" / "pyproject.toml"
    )
    agent_server_baseline = get_pypi_baseline_version(
        "Creanova-agent-server", agent_server_current
    )
...
def main() -> int:
    ...
    if sdk_baseline is None or baseline_cutoff is None:
        print(
            "::warning title=Persisted settings baseline::No published Creanova-sdk "
    ...
    baseline_summary = f"Creanova-sdk=={sdk_baseline}"
    if agent_server_baseline is not None:
        baseline_summary += f", Creanova-agent-server=={agent_server_baseline}"
    print(
