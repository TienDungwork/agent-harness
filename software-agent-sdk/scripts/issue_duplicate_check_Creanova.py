...
Creanova_BASE_URL = os.environ.get("Creanova_BASE_URL", "https://app.all-hands.dev")
...
Creanova_SENSITIVE_KEYS = frozenset({"session_api_key"})


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Start an Creanova Cloud conversation that checks a GitHub issue "
            "for duplicates."
        )
    )
    parser.add_argument(
...
def github_headers() -> dict[str, str]:
    headers = {
        ...
        "User-Agent": "Creanova-issue-duplicate-check",
...
def Creanova_headers() -> dict[str, str]:
    api_key = os.environ.get("Creanova_API_KEY")
    if not api_key:
        raise RuntimeError("Creanova_API_KEY environment variable is required")
    return {
...
def summarize_Creanova_item(item: dict[str, Any]) -> str:
    summary = {}
    for key in Creanova_DEBUG_KEYS:
        if key not in item:
            continue
        value = item[key]
        if value in (None, "", [], {}):
    ...
    available_keys = sorted(
        key
        for key in item
        if key not in summary and key not in Creanova_SENSITIVE_KEYS
    )
    if available_keys:
        summary["available_keys"] = available_keys
    sensitive_keys_present = sorted(
        key for key in item if key in Creanova_SENSITIVE_KEYS
    )
...
def poll_start_task(
    ...
    while time.time() < deadline:
        payload = request_json(
            ...
            headers={"Authorization": Creanova_headers()["Authorization"]},
        )
        ...
        if status in {"ERROR", "FAILED"}:
            raise RuntimeError(
                f"Creanova start task failed: {summarize_Creanova_item(item)}"
            )
        time.sleep(poll_interval_seconds)
    raise TimeoutError(
...
def poll_conversation(
    ...
    while time.time() < deadline:
        ...
        if execution_status in FAILED_EXECUTION_STATUSES:
            raise RuntimeError(
                "Creanova conversation ended with "
                f"{execution_status}: {summarize_Creanova_item(item)}"
            )
...
def main() -> int:
    ...
    if not app_conversation_id:
        ...
        if not task_id:
            raise RuntimeError(
                ...
                f"{summarize_Creanova_item(start_task)}"
            )
        ...
        if not app_conversation_id:
            raise RuntimeError(
                ...
                f"{summarize_Creanova_item(ready_task)}"
            )
    ...
    conversation_url = (
        ...
        or f"{Creanova_BASE_URL}/conversations/{app_conversation_id}"
    )
    ...
    if session_api_key_value and not isinstance(session_api_key_value, str):
        raise RuntimeError(
            "session_api_key had unexpected type in the Creanova conversation: "
        ...
        try:
            ...
            if not session_api_key:
                raise RuntimeError(
                    ...
                    "session_api_key was missing from the Creanova conversation"
                ) from exc
