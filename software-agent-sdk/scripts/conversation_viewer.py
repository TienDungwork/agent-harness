"""Streamlit app to explore Creanova conversation logs.
...
``Creanova`` when recording sessions). You can override the location via:

* Environment variable ``Creanova_CONVERSATIONS_ROOT``
...
ENV_ROOT = os.getenv("Creanova_CONVERSATIONS_ROOT")
...
st.set_page_config(page_title="Creanova Agent-SDK Conversation Viewer", layout="wide")
...
def main() -> None:
    st.title("Creanova Conversation Viewer")
    ...
    root_input = st.sidebar.text_input(
        ...
        help="Root folder containing Creanova conversation dumps",
    )
