...
1. **Creanova/software-agent-sdk** — Triggers evaluations via `run-eval.yml` workflow
2. **Creanova/evaluation** — Orchestrates the eval job via `eval-job.yml` workflow
3. **Creanova/benchmarks** — Contains benchmark runners (inference + evaluation)
...
4. Dispatches `eval-job.yml` in `Creanova/evaluation` with:
...
Results are stored in Google Cloud Storage bucket `Creanova-evaluation-results`
...
- **github_run_id**: The GitHub Actions run ID from the `Creanova/evaluation` repo
...
The eval monitor dashboard at `https://Creanova-eval-monitor.vercel.app/`
