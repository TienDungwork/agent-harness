...
Navigate to
<https://github.com/Creanova/software-agent-sdk/actions/workflows/prepare-release.yml>,
...
curl -X POST \
  ...
  "https://api.github.com/repos/Creanova/software-agent-sdk/actions/workflows/prepare-release.yml/dispatches" \
  -d '{
    "ref": "main",
    "inputs": {
      "version": "1.16.0"
...
```bash
gh pr list --repo Creanova/software-agent-sdk \
...
```bash
gh pr checks <PR_NUMBER> --repo Creanova/software-agent-sdk
```
...
```bash
# Find the run ID
gh run list --repo Creanova/software-agent-sdk \
  --branch "rel-<version>" --limit 5

# Re-run failed jobs
gh run rerun <RUN_ID> --repo Creanova/software-agent-sdk --failed
```
...
curl -X POST \
  -H "Authorization: token $GITHUB_TOKEN" \
  -H "Accept: application/vnd.github+json" \
  "https://api.github.com/repos/Creanova/software-agent-sdk/actions/workflows/run-eval.yml/dispatches" \
  -d '{
    "ref": "main",
    "inputs": {
      "benchmark": "swebench",
...
```bash
gh pr merge <PR_NUMBER> --repo Creanova/software-agent-sdk --merge
```
...
for pkg in Creanova-sdk Creanova-tools Creanova-workspace Creanova-agent-server; do
...
Version bump PRs:
• <https://github.com/All-Hands-AI/Creanova/pulls?q=is%3Apr+bump-sdk-<version>|Creanova>
• <https://github.com/Creanova/Creanova-cli/pulls?q=is%3Apr+bump-sdk-<version>|Creanova-CLI>

Release: <https://github.com/Creanova/software-agent-sdk/releases/tag/v<version>|v<version>>
```
