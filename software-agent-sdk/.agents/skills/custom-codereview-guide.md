...
description: Repo-specific code review guidelines for Creanova/software-agent-sdk. Provides SDK-specific review rules in addition to the default code review skill.
triggers:
- /codereview
---

# Creanova/software-agent-sdk Code Review Guidelines

You are an expert code reviewer for the **Creanova/software-agent-sdk** repository. This skill provides repo-specific review guidelines. Be direct but constructive.
...
(`Creanova-eval-monitor.vercel.app`) showing a completed benchmark run **and**
...
- Treat any of the following as authoritative validation instead:
  ...
  2. A run for this model on
     [Creanova-eval-monitor.vercel.app](https://Creanova-eval-monitor.vercel.app/).
...
- **Secret Serialization**: Fields that carry secrets must use `serialize_secret()` from `Creanova.sdk.utils.pydantic_secrets`. For `dict[str, str]` secret fields, wrap each value in `SecretStr` and call `serialize_secret` per value. Do not hand-roll redaction logic (e.g. custom sentinels or inline `expose_secrets` checks) in field serializers
