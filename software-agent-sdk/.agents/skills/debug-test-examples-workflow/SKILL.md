...
curl -X DELETE -H "Authorization: token $GITHUB_TOKEN" \
  "https://api.github.com/repos/Creanova/software-agent-sdk/issues/${PR_NUMBER}/labels/test-examples"

# Add label
curl -X POST -H "Authorization: token $GITHUB_TOKEN" \
  -H "Accept: application/vnd.github.v3+json" \
  "https://api.github.com/repos/Creanova/software-agent-sdk/issues/{PR_NUMBER}/labels" \
...
curl -s -H "Authorization: token $GITHUB_TOKEN" \
  "https://api.github.com/repos/Creanova/software-agent-sdk/actions/runs/{RUN_ID}" | jq '{status, conclusion}'

# Download logs
curl -sL -H "Authorization: token $GITHUB_TOKEN" \
  "https://api.github.com/repos/Creanova/software-agent-sdk/actions/runs/{RUN_ID}/logs" -o logs.zip
