...
from Creanova.sdk import apply_agent_settings_diff, validate_agent_settings
...
def test_switch_Creanova_to_acp_replaces_with_fresh_variant() -> None:
    base = {"agent_kind": "Creanova", "llm": {"model": "gpt"}}
    ...
    # ACP's agent_context is nullable; the Creanova llm.model is not carried.
    assert result.agent_context is None


def test_switch_acp_to_Creanova_replaces_with_fresh_variant() -> None:
    base = {"agent_kind": "acp", "acp_server": "claude-code"}
    ...
    assert isinstance(result, CreanovaAgentSettings)
    assert result.agent_kind == "Creanova"
...
def test_inline_fields_land_on_fresh_base_during_switch() -> None:
    ...
    result = apply_agent_settings_diff(
        base, {"agent_kind": "Creanova", "llm": {"model": "model-c"}}
    )
...
def test_base_may_be_a_settings_instance() -> None:
    base = CreanovaAgentSettings.model_validate({"llm": {"model": "gpt"}})
...
def test_secret_in_base_survives_same_kind_merge() -> None:
    base = CreanovaAgentSettings.model_validate(
...
def test_validate_canonicalizes_llm_tag_at_current_schema_version() -> None:
    ...
    assert type(result) is CreanovaAgentSettings
    assert result.agent_kind == "Creanova"
    assert result.llm.model == "legacy"


def test_apply_diff_on_llm_tagged_base_returns_Creanova() -> None:
    base = {
        "agent_kind": "llm",
        "schema_version": AGENT_SETTINGS_SCHEMA_VERSION,
        "llm": {"model": "legacy"},
...
def test_llm_agent_settings_remains_importable() -> None:
    from Creanova.sdk.settings.model import LLMAgentSettings as _LLM

    assert issubclass(_LLM, CreanovaAgentSettings)
