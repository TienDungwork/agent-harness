...
from Creanova.sdk.hooks.config import HookConfig, HookDefinition, HookMatcher, HookType
...
class TestHookConfig:
    ...
    def test_load_discovers_config_in_working_dir(self):
        """Test that load() discovers .Creanova/hooks.json in working_dir."""
        hook = {"type": "command", "command": "test-hook.sh"}
        data = {"hooks": {"PreToolUse": [{"matcher": "*", "hooks": [hook]}]}}

        with tempfile.TemporaryDirectory() as tmpdir:
            # Create .Creanova/hooks.json in the working directory
            import os

            hooks_dir = os.path.join(tmpdir, ".Creanova")
...
def test_issue_2749():
    ...
    https://github.com/Creanova/software-agent-sdk/issues/2749
    """
    data = {
        "hooks": {
            "Stop": [
