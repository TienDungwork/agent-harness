...
from Creanova.sdk.git.git_changes import get_changes_in_repo, get_git_changes
from Creanova.sdk.git.models import GitChange, GitChangeStatus
...
def test_get_git_changes_skips_vanished_nested_repo():
    ...
    from Creanova.sdk.git.exceptions import GitRepositoryError
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            "Creanova.sdk.git.git_changes.get_changes_in_repo",
...
def push_agent_branch(repo: Path) -> None:
    """Create ``Creanova/pr-branch`` with one committed file and push it,
    leaving the branch in sync with its upstream (the post-PR state)."""
    run_bash_command("git checkout -b Creanova/pr-branch", str(repo))
    (repo / "new.txt").write_text("pr change")
    run_bash_command("git add .", str(repo))
    run_bash_command("git commit -m 'agent pr work'", str(repo))
    run_bash_command("git push -u origin Creanova/pr-branch", str(repo))
...
def test_get_changes_in_repo_no_remote_worktree_shows_committed_changes():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        run_bash_command(
            f"git worktree add -b Creanova/conv1 {worktree} main", str(repo)
        )
