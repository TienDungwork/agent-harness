...
from Creanova.agent_server.models import StartConversationRequest
from Creanova.sdk import (
    LLM,
    ACPAgentSettings,
    Agent,
    AgentContext,
...
from Creanova.sdk.agent.acp_agent import ACPAgent
from Creanova.sdk.context.condenser import LLMSummarizingCondenser, NoOpCondenser
from Creanova.sdk.critic.base import IterativeRefinementConfig
from Creanova.sdk.critic.impl.api import APIBasedCritic
from Creanova.sdk.mcp.config import MCPServer, coerce_mcp_config, dump_mcp_config
from Creanova.sdk.secret import StaticSecret
from Creanova.sdk.security.confirmation_policy import AlwaysConfirm, ConfirmRisky
...
from Creanova.sdk.settings.model import ACPServerKind
from Creanova.sdk.workspace import LocalWorkspace
...
def test_llm_agent_settings_export_schema_groups_sections() -> None:
    schema = CreanovaAgentSettings.export_schema()

    assert schema.model_name == "CreanovaAgentSettings"
...
def test_acp_agent_settings_export_schema_has_acp_section() -> None:
    ...
    # mcp_config is exposed as a single object field (matching the Creanova
...
def test_conversation_settings_validates_observability_metadata() -> None:
    settings = ConversationSettings(observability_metadata={"repo": "Creanova/sdk"})
    assert settings.observability_metadata == {"repo": "Creanova/sdk"}
...
def test_conversation_settings_create_request() -> None:
    ...
    agent = CreanovaAgentSettings(llm=LLM(model="test-model")).create_agent()
...
def test_export_agent_settings_schema_emits_variant_tagged_sections() -> None:
    ...
    # field-level variant="Creanova" tags (so they hide on the ACP page).
    ...
    for f in general.fields:
        assert f.variant == "Creanova", (
            f"expected field {f.key} variant=Creanova, got {f.variant}"
        )

    # LLM-variant sections.
    assert ("llm", "Creanova") in by_keyvariant
    assert ("condenser", "Creanova") in by_keyvariant
    assert ("verification", "Creanova") in by_keyvariant
...
def test_default_agent_settings_returns_Creanova_variant() -> None:
    s = default_agent_settings()
    assert isinstance(s, CreanovaAgentSettings)
    assert s.agent_kind == "Creanova"


def test_validate_agent_settings_defaults_to_Creanova_when_discriminator_missing() -> (
    None
):
    """Existing persisted payloads predate ``agent_kind`` — they must round-trip."""
    v = validate_agent_settings({"llm": {"model": "test-model"}})
    assert isinstance(v, CreanovaAgentSettings)
    assert v.llm.model == "test-model"


def test_validate_agent_settings_dispatches_on_agent_kind() -> None:
    Creanova = validate_agent_settings(
        {"agent_kind": "Creanova", "llm": {"model": "m"}}
    )
    assert isinstance(Creanova, CreanovaAgentSettings)
    assert Creanova.agent_kind == "Creanova"

    legacy_llm = validate_agent_settings(
        {"agent_kind": "llm", "llm": {"model": "legacy-model"}}
    )
    assert isinstance(legacy_llm, CreanovaAgentSettings)
    assert legacy_llm.agent_kind == "Creanova"
...
def test_validate_agent_settings_migrates_v0_llm_payload() -> None:
    ...
    assert isinstance(settings, CreanovaAgentSettings)
    assert settings.schema_version == AGENT_SETTINGS_SCHEMA_VERSION
    assert settings.agent_kind == "Creanova"
...
def test_validate_agent_settings_canonicalizes_legacy_llm_kind() -> None:
    ...
    the canonical ``'Creanova'`` discriminator on read."""
    settings = validate_agent_settings(
        {
            "schema_version": 1,
            "agent_kind": "llm",
...
def test_validate_agent_settings_migrates_legacy_Creanova_proxy_llm() -> None:
    settings = validate_agent_settings(
        {
            "schema_version": 3,
            "agent_kind": "Creanova",
    ...
    assert isinstance(settings, CreanovaAgentSettings)
    assert settings.schema_version == AGENT_SETTINGS_SCHEMA_VERSION
    assert settings.llm.model == "Creanova/claude-opus-4-8"
...
def test_Creanova_mcp_config_reject_unknown_server_fields() -> None:
    with pytest.raises(ValidationError):
        CreanovaAgentSettings.model_validate(
            {
                "mcp_config": {
                    "server": {
                        "url": "https://mcp.example.com/mcp",
...
def test_llm_create_agent_uses_settings_llm_and_tools() -> None:
    ...
    settings = CreanovaAgentSettings(llm=llm, tools=tools)
...
def test_create_agent_defaults_tools_when_none() -> None:
    ...
    settings = CreanovaAgentSettings(llm=LLM(model="test-model"))
...
def test_create_agent_default_tools_honor_enable_sub_agents() -> None:
    settings = CreanovaAgentSettings(
...
def test_create_agent_empty_tools_stays_bare() -> None:
    ...
    settings = CreanovaAgentSettings(llm=LLM(model="test-model"), tools=[])
    agent = settings.create_agent()
    assert agent.tools == []
...
def test_tool_concurrency_limit_defaults_to_one_when_omitted_from_payload() -> None:
    ...
    settings = CreanovaAgentSettings.model_validate({"agent_kind": "Creanova"})
...
def test_tool_concurrency_limit_valid_values_round_trip(
    ...
    reloaded = CreanovaAgentSettings.model_validate(settings.model_dump(mode="json"))
...
def test_tool_concurrency_limit_invalid_values_rejected(raw: Any) -> None:
    with pytest.raises(ValidationError) as exc_info:
        CreanovaAgentSettings(llm=LLM(model="test-model"), tool_concurrency_limit=raw)
    assert any(
...
def test_llm_agent_settings_validates_mcp_config_as_typed_model() -> None:
    settings = CreanovaAgentSettings.model_validate(
...
def test_llm_create_agent_serializes_typed_mcp_config_compactly() -> None:
    ...
    settings = CreanovaAgentSettings(mcp_config=mcp_config)
...
def test_Creanova_agent_settings_defaults_legacy_condenser_payload() -> None:
    settings = CreanovaAgentSettings.model_validate(
        {
            "condenser": {
                "enabled": True,
...
def test_Creanova_agent_settings_dispatches_no_op_condenser_payload() -> None:
    settings = CreanovaAgentSettings.model_validate(
        {
            "condenser": {
                "enabled": True,
...
def test_Creanova_agent_settings_upgrades_base_condenser_settings_instance() -> None:
    settings = CreanovaAgentSettings.model_validate(
...
def test_llm_create_agent_builds_no_op_condenser_variant() -> None:
    settings = CreanovaAgentSettings(condenser=NoOpCondenserSettings())
...
def test_llm_roundtrip_preserves_llm_model() -> None:
    ...
    restored = CreanovaAgentSettings.model_validate(data)
...
def test_llm_agent_settings_public_alias_removed() -> None:
    ...
    import Creanova.sdk as _sdk_mod
    import Creanova.sdk.settings as _settings_mod
    ...
    from Creanova.sdk.settings.model import LLMAgentSettings

    assert issubclass(LLMAgentSettings, CreanovaAgentSettings)
...
def test_conversation_settings_agent_settings_field_accepts_both_variants() -> None:
    ...
    llm_conv = ConversationSettings(
        agent_settings=CreanovaAgentSettings(llm=LLM(model="m")),
    )
    assert isinstance(llm_conv.agent_settings, CreanovaAgentSettings)
...
def test_Creanova_agent_settings_mcp_config_redacts_env_and_headers() -> None:
    mcp_config = coerce_mcp_config(
        {
            "leaky": {
                "command": "echo",
...
def test_mcp_config_encrypts_env_and_headers_with_cipher() -> None:
    ...
    """
    from Creanova.sdk.utils.cipher import Cipher
    ...
    restored = CreanovaAgentSettings.model_validate(dumped, context={"cipher": cipher})
...
def test_Creanova_agent_settings_create_agent_preserves_mcp_auth_model() -> None:
    mcp_config = coerce_mcp_config(
        {
            "linear": {
                "url": "https://mcp.linear.app/mcp",
    ...
    agent = CreanovaAgentSettings(mcp_config=mcp_config).create_agent()
...
def test_Creanova_agent_settings_mcp_config_decrypt_legacy_plaintext_on_disk() -> None:
    ...
    restored = CreanovaAgentSettings.model_validate(
...
def test_Creanova_agent_settings_mcp_config_expose_encrypted_requires_cipher() -> None:
    ...
    from Creanova.sdk.utils.pydantic_secrets import MissingCipherError
...
def test_Creanova_agent_settings_mcp_config_expose_plaintext_passes_through() -> None:
...
def test_Creanova_agent_settings_create_agent_keeps_real_mcp_secrets() -> None:
...
def test_agent_settings_base_is_parent_of_both_variants() -> None:
    assert issubclass(CreanovaAgentSettings, AgentSettingsBase)
    assert issubclass(ACPAgentSettings, AgentSettingsBase)


def test_agent_settings_base_schema_version_inherited() -> None:
    Creanova = CreanovaAgentSettings()
    acp = ACPAgentSettings(acp_command=["x"])
    assert Creanova.schema_version == AGENT_SETTINGS_SCHEMA_VERSION
...
def test_agent_settings_base_export_schema_works_on_both_variants() -> None:
    Creanova_schema = CreanovaAgentSettings.export_schema()
    acp_schema = ACPAgentSettings.export_schema()
    assert Creanova_schema.model_name == "CreanovaAgentSettings"
...
def test_agent_settings_base_create_agent_is_callable_via_interface() -> None:
    ...
    settings: AgentSettingsBase = CreanovaAgentSettings(llm=LLM(model="test-model"))
    agent = settings.create_agent()
    assert isinstance(agent, Agent)
...
def test_acp_resolve_command_uses_registry_defaults(
    ...
    from Creanova.sdk.settings.acp_providers import ACP_PROVIDERS
...
def test_regular_agent_supports_all_capabilities() -> None:
    agent = CreanovaAgentSettings(llm=LLM(model="test-model")).create_agent()
    assert agent.supports_Creanova_tools is True
    assert agent.supports_condenser is True
    assert agent.agent_kind == "Creanova"


def test_acp_agent_reports_no_Creanova_capabilities() -> None:
    from Creanova.sdk.agent.acp_agent import ACPAgent

    agent = ACPAgent(acp_command=["x"])
    assert agent.supports_Creanova_tools is False
...
def test_llm_create_agent_resolves_subscription_llm(monkeypatch) -> None:
    from Creanova.sdk.llm.auth import openai
    ...
    agent = CreanovaAgentSettings(llm=original_llm).create_agent()
...
def test_create_subscription_llm_from_config_preserves_runtime_llm(monkeypatch) -> None:
    import Creanova.sdk.llm.auth.openai as openai_auth
    from Creanova.sdk.llm.auth.credentials import OAuthCredentials
...
def test_llm_from_persisted_rebuilds_serialized_subscription_runtime(
    ...
    import Creanova.sdk.llm.auth.openai as openai_auth
    from Creanova.sdk.llm.auth.credentials import OAuthCredentials
    from Creanova.sdk.llm.auth.openai import OpenAISubscriptionAuth
