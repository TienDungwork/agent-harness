...
def test_load_project_skills_agents_md_without_skills_directory(tmp_path):
    """Test that AGENTS.md is loaded even when .Creanova/skills doesn't exist.

    This is a regression test for the bug where third-party skill files like
    AGENTS.md were not loaded when the .Creanova/skills directory didn't exist.
    """
    # Create AGENTS.md in the work directory (no .Creanova/skills)
...
def test_load_project_skills_third_party_with_skills_directory(tmp_path):
    """Test third-party files are loaded alongside skills from .Creanova/skills."""
    # Create AGENTS.md in work directory
    (tmp_path / "AGENTS.md").write_text("# AGENTS.md content")

    # Create .Creanova/skills directory with a skill
    skills_dir = tmp_path / ".Creanova" / "skills"
...
def test_load_project_skills_with_skills_directory(tmp_path):
    """Test load_project_skills loads from .Creanova/skills directory."""
    # Create .Creanova/skills directory
    skills_dir = tmp_path / ".Creanova" / "skills"
    skills_dir.mkdir(parents=True)
...
def test_load_project_skills_agents_directory_precedence(tmp_path):
    ...
    skills_dir = tmp_path / ".Creanova" / "skills"
    microagents_dir = tmp_path / ".Creanova" / "microagents"
    ...
    (skills_dir / "duplicate.md").write_text(
        "---\nname: duplicate\n---\nFrom .Creanova/skills."
    )
    (microagents_dir / "duplicate.md").write_text(
        "---\nname: duplicate\n---\nFrom .Creanova/microagents."
    )
...
def test_load_project_skills_merges_agents_and_Creanova(tmp_path):
    """Test loading unique skills from .agents/skills and .Creanova/skills."""
    agents_dir = tmp_path / ".agents" / "skills"
    Creanova_dir = tmp_path / ".Creanova" / "skills"
    agents_dir.mkdir(parents=True)
    Creanova_dir.mkdir(parents=True)

    (agents_dir / "agent_skill.md").write_text(
        "---\nname: agent_skill\n---\nAgent skill content."
    )
    (Creanova_dir / "legacy_skill.md").write_text(
...
def test_load_project_skills_with_microagents_directory(tmp_path):
    """Test load_project_skills loads from .Creanova/microagents directory (legacy)."""
    # Create .Creanova/microagents directory
    microagents_dir = tmp_path / ".Creanova" / "microagents"
    microagents_dir.mkdir(parents=True)
