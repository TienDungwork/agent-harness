...
from Creanova.sdk.skills.utils import update_skills_repository
...
def mock_repo_with_agentskills_references(tmp_path):
    ...
    See: https://github.com/Creanova/software-agent-sdk/issues/1981
    """
    repo_dir = tmp_path / "mock_repo"
    repo_dir.mkdir()
...
def test_load_public_skills_success(mock_repo_dir, tmp_path):
    ...
    with (
        patch(
            "Creanova.sdk.skills.skill.update_skills_repository",
            side_effect=mock_update_repo,
        ),
        patch(
            "Creanova.sdk.skills.skill.get_skills_cache_dir",
...
def test_update_skills_repository_clone_new(tmp_path):
    ...
    with patch(
        "Creanova.sdk.git.utils.subprocess.run", return_value=mock_result
    ) as mock_run:
        repo_path = update_skills_repository(
            "https://github.com/Creanova/extensions",
            "main",
            cache_dir,
        )
...
def test_update_skills_repository_clone_timeout(tmp_path):
    ...
    with patch(
        "Creanova.sdk.git.utils.subprocess.run",
...
def test_load_public_skills_excludes_reference_markdown_in_agentskills_folders(
    ...
    https://github.com/Creanova/software-agent-sdk/issues/1981
...
def mock_repo_with_marketplace(tmp_path):
    ...
    marketplace = {
        "name": "default",
        "owner": {"name": "Creanova", "email": "test@test.com"},
