...
from Creanova.sdk.profiles import (
    ACPAgentProfile,
    AgentProfileStore,
    CreanovaAgentProfile,
    ProfileLimitExceeded,
)
from Creanova.sdk.profiles.agent_profile import AGENT_PROFILE_SCHEMA_VERSION
...
@pytest.fixture
def Creanova_profile() -> CreanovaAgentProfile:
    return CreanovaAgentProfile(
        name="oh",
...
def test_list_with_profiles(
    agent_store: AgentProfileStore, Creanova_profile: CreanovaAgentProfile
) -> None:
    agent_store.save(Creanova_profile)
    agent_store.save(Creanova_profile.model_copy(update={"name": "oh2"}))
...
def test_load_migrates_untouched_v1_default_tools(
    ...
    assert isinstance(profile, CreanovaAgentProfile)
...
def test_save_persists_id_inside_file(
    ...
    agent_store.save(Creanova_profile)
    data = json.loads((agent_store.base_dir / "oh.json").read_text())
    assert data["id"] == str(Creanova_profile.id)
...
def test_save_with_invalid_profile_name(
    ...
    profile = CreanovaAgentProfile(name=name, llm_profile_ref="default")
...
def test_save_writes_valid_json(
    ...
    agent_store.save(Creanova_profile)
    data = json.loads((agent_store.base_dir / "oh.json").read_text())
    assert data["agent_kind"] == "Creanova"
...
def test_save_overwrites_existing(
    ...
    agent_store.save(Creanova_profile)
    agent_store.save(Creanova_profile.model_copy(update={"llm_profile_ref": "other"}))

    loaded = agent_store.load("oh")
    assert isinstance(loaded, CreanovaAgentProfile)
...
def test_roundtrip_Creanova(
    ...
    assert isinstance(loaded, CreanovaAgentProfile)
    assert loaded.id == Creanova_profile.id
...
def test_rename_syncs_internal_name_and_preserves_id(
    ...
    agent_store.save(Creanova_profile)
    original_id = Creanova_profile.id
    agent_store.rename("oh", "renamed")
...
def test_rename_target_exists_raises(
    ...
    agent_store.save(Creanova_profile)
    agent_store.save(Creanova_profile.model_copy(update={"name": "taken"}))
...
def test_list_summaries_returns_fk_fields(
    agent_store: AgentProfileStore,
    Creanova_profile: CreanovaAgentProfile,
    ...
    oh = by_name["oh"]
    assert oh["id"] == str(Creanova_profile.id)
    assert oh["agent_kind"] == "Creanova"
...
def test_save_with_max_profiles_blocks_over_limit(
    ...
    agent_store.save(Creanova_profile.model_copy(update={"name": "a"}))
    agent_store.save(Creanova_profile.model_copy(update={"name": "b"}))

    with pytest.raises(ProfileLimitExceeded, match="2"):
        agent_store.save(
            Creanova_profile.model_copy(update={"name": "c"}), max_profiles=2
        )
...
def test_save_with_max_profiles_allows_overwrite(
    ...
    agent_store.save(Creanova_profile.model_copy(update={"name": "a"}), max_profiles=2)
...
def test_save_with_max_profiles_ignores_invalid_filenames(
    ...
    agent_store.save(Creanova_profile.model_copy(update={"name": "real"}))
    (agent_store.base_dir / ".hidden.json").write_text('{"name": "x"}')
    (agent_store.base_dir / "bad@name.json").write_text('{"name": "x"}')

    agent_store.save(
        Creanova_profile.model_copy(update={"name": "another"}), max_profiles=2
    )
...
def test_concurrent_reads_and_writes(tmp_path: Path) -> None:
    ...
    for i in range(5):
        store.save(
            CreanovaAgentProfile(name=f"profile_{i}", llm_profile_ref=f"llm_{i}")
        )
    ...
    def write_profile(index: int) -> None:
        try:
            store.save(
                CreanovaAgentProfile(name=f"new_profile_{index}", llm_profile_ref="x")
            )
...
def test_full_workflow(agent_store: AgentProfileStore) -> None:
    profile = CreanovaAgentProfile(name="wf", llm_profile_ref="default", revision=1)
