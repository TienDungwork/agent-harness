...
from Creanova.sdk.settings.model import VerificationSettings, validate_agent_settings
...
class InMemoryAgentProfileStore:
    ...
    def __init__(self) -> None:
        self._profiles: dict[str, CreanovaAgentProfile | ACPAgentProfile] = {}
    ...
    def list_summaries(self) -> list[dict[str, Any]]:
        ...
        for name, p in self._profiles.items():
            ...
            kind = d.get("agent_kind", "Creanova")
            out.append(
                {
                    "id": d.get("id"),
                    "name": name,
                    ...
                    "llm_profile_ref": (
                        d.get("llm_profile_ref") if kind == "Creanova" else None
                    ),
    ...
    def set_llm_profile_ref(self, name, new_ref) -> None:
        ...
        if isinstance(p, CreanovaAgentProfile):
...
def _seed(store: InMemoryAgentProfileStore) -> None:
    save_profile_preserving_identity(
        store, CreanovaAgentProfile(name="reviewer", llm_profile_ref="gpt")
    )
    save_profile_preserving_identity(
        store, CreanovaAgentProfile(name="coder", llm_profile_ref="gpt")
    )
    save_profile_preserving_identity(
        store, CreanovaAgentProfile(name="cheap", llm_profile_ref="haiku")
    )
    save_profile_preserving_identity(
...
def test_save_preserving_identity_mints_then_keeps_id_and_bumps_revision():
    ...
    created = save_profile_preserving_identity(
        store, CreanovaAgentProfile(name="p", llm_profile_ref="gpt")
    )
    ...
    again = save_profile_preserving_identity(
        store, CreanovaAgentProfile(name="p", llm_profile_ref="haiku")
    )
    ...
    other = save_profile_preserving_identity(
        store, CreanovaAgentProfile(name="q", llm_profile_ref="gpt")
    )
...
def test_save_preserving_identity_enforces_limit():
    store = InMemoryAgentProfileStore()
    save_profile_preserving_identity(
        store, CreanovaAgentProfile(name="a", llm_profile_ref="gpt"), max_profiles=1
    )
    with pytest.raises(ProfileLimitExceeded):
        save_profile_preserving_identity(
            store,
            CreanovaAgentProfile(name="b", llm_profile_ref="gpt"),
            max_profiles=1,
        )
    # Overwriting the existing one is allowed at the cap.
    save_profile_preserving_identity(
        store, CreanovaAgentProfile(name="a", llm_profile_ref="haiku"), max_profiles=1
    )


def test_build_seed_profile_Creanova_branch():
    settings = validate_agent_settings({"agent_kind": "Creanova"})
    ...
    assert isinstance(fallback, CreanovaAgentProfile)
