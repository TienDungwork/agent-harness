...
_ADAPTER: TypeAdapter[CreanovaAgentProfile | ACPAgentProfile] = TypeAdapter(
    AgentProfile
)
...
def test_Creanova_profile_round_trips() -> None:
    ...
    assert isinstance(reloaded, CreanovaAgentProfile)
    assert reloaded == profile
    assert reloaded.agent_kind == "Creanova"
...
def test_Creanova_profile_new_field_defaults() -> None:
    ...
    reloaded = validate_agent_profile(
        {"agent_kind": "Creanova", "name": "oh", "llm_profile_ref": "default"}
    )
    assert isinstance(reloaded, CreanovaAgentProfile)
...
def test_acp_profile_has_no_skill_field() -> None:
    ...
    from Creanova.sdk.profiles import ACPAgentProfile
...
def test_validate_dispatches_on_agent_kind() -> None:
    Creanova = validate_agent_profile(
        {"agent_kind": "Creanova", "name": "oh", "llm_profile_ref": "default"}
    )
    assert isinstance(Creanova, CreanovaAgentProfile)
...
def test_missing_discriminator_defaults_to_Creanova() -> None:
    profile = validate_agent_profile({"name": "oh", "llm_profile_ref": "default"})
    assert isinstance(profile, CreanovaAgentProfile)
    assert profile.agent_kind == "Creanova"
...
def test_Creanova_rejects_acp_fields() -> None:
...
def test_Creanova_requires_llm_profile_ref() -> None:
    with pytest.raises(ValidationError):
        validate_agent_profile({"agent_kind": "Creanova", "name": "oh"})
...
def test_mcp_server_refs_default_is_null() -> None:
    profile = CreanovaAgentProfile(name="oh", llm_profile_ref="d")
...
def test_id_is_uuid_and_autogenerated() -> None:
    ...
    other = CreanovaAgentProfile(name="oh", llm_profile_ref="d")
...
def test_Creanova_profile_persists_no_secret_fields() -> None:
    dumped = CreanovaAgentProfile(name="oh", llm_profile_ref="default").model_dump(
        mode="json"
    )
...
def test_Creanova_profile_has_no_embedded_skills_field() -> None:
