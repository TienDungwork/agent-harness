...
from Creanova.sdk.settings.model import ACPAgentSettings, CreanovaAgentSettings
...
def test_Creanova_resolves_to_settings_with_injected_llm(
...
def test_Creanova_resolves_default_exec_tools(
    ...
    profile = CreanovaAgentProfile(name="oh", llm_profile_ref="default")
...
def test_Creanova_profile_tools_selection_is_passed_through(
    ...
    bare = CreanovaAgentProfile(name="bare", llm_profile_ref="default", tools=[])
    settings = resolve_agent_profile(
        bare,
        llm_store=llm_store,
        mcp_config={},
...
def test_Creanova_copies_verification(
    llm_store: LLMProfileStore, mcp_config: dict[str, MCPServer]
) -> None:
    profile = CreanovaAgentProfile(name="oh", llm_profile_ref="default")
...
def test_Creanova_resolve_sets_load_project_skills(
    llm_store: LLMProfileStore, mcp_config: dict[str, MCPServer]
) -> None:
...
def test_missing_llm_ref_raises_profile_not_found(
    ...
    profile = CreanovaAgentProfile(name="oh", llm_profile_ref="does-not-exist")
...
def test_seed_then_resolve_with_narrower_catalog_does_not_dangle(
    ...
    from Creanova.sdk.profiles import build_seed_profile
    from Creanova.sdk.settings.model import validate_agent_settings
    ...
    assert isinstance(resolved, CreanovaAgentSettings)
...
def test_dry_run_Creanova_valid_and_redacted(
    llm_store: LLMProfileStore, mcp_config: dict[str, MCPServer]
) -> None:
    profile = CreanovaAgentProfile(
        name="oh", llm_profile_ref="default", mcp_server_refs=["fetch"]
    ...
    assert diag.agent_kind == "Creanova"
