...
from Creanova.sdk.llm import LLM, LLM_PROFILE_SCHEMA_VERSION
...
def test_load_migrates_legacy_Creanova_proxy_profile(
    profile_store: LLMProfileStore,
) -> None:
    ...
    assert loaded.model == "Creanova/claude-opus-4-8"
    assert loaded.base_url is None


def test_list_summaries_migrates_legacy_Creanova_proxy_profile(
    profile_store: LLMProfileStore,
) -> None:
    profile_path = profile_store.base_dir / "legacy.json"
    profile_path.write_text(
    ...
    assert summaries == [
        {
            "name": "legacy",
            "model": "Creanova/claude-opus-4-8",
            "base_url": None,
            "api_key_set": False,
        }
    ]
