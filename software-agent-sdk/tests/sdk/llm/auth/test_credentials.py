...
def test_get_credentials_dir_default(monkeypatch):
    ...
    assert creds_dir == Path.home() / ".Creanova" / "auth"


def test_get_credentials_dir_xdg(monkeypatch, tmp_path):
    """Test credentials directory ignores XDG_DATA_HOME (uses ~/.Creanova/auth)."""
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path))
    creds_dir = get_credentials_dir()
    # Implementation uses ~/.Creanova/auth regardless of XDG_DATA_HOME
    assert creds_dir == Path.home() / ".Creanova" / "auth"
