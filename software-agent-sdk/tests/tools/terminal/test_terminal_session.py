...
def test_truncation_preserves_metadata_in_llm_content(monkeypatch, terminal_type):
    ...
    from Creanova.sdk.utils.truncate import DEFAULT_TRUNCATE_NOTICE
    from Creanova.tools.terminal import definition as terminal_definition_mod
...
def test_environment_variable_inheritance_from_parent(terminal_type):
    ...
    test_var_name = "Creanova_TEST_INHERITANCE_VAR"
...
def test_multiline_command_loop(terminal_type):
    ...
    # https://github.com/Creanova/Creanova/issues/3143
...
def test_bash_remove_prefix(terminal_type):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            obs = _run_bash_action(
                session,
                "git init && git remote add origin https://github.com/Creanova/Creanova",
            )
            assert obs.metadata.exit_code == 0
            ...
            assert obs.metadata.exit_code == 0
            assert "https://github.com/Creanova/Creanova" in obs.text
