...
def test_list_open_issues_filters_by_duplicate_candidate_label(monkeypatch):
    ...
    assert module.list_open_issues("Creanova/agent-sdk") == [
        {"number": 1},
        {"number": 3},
    ]
    assert requested_paths == [
        "/repos/Creanova/agent-sdk/issues?state=open&labels=duplicate-candidate&per_page=100&page=1",
        "/repos/Creanova/agent-sdk/issues?state=open&labels=duplicate-candidate&per_page=100&page=2",
        "/repos/Creanova/agent-sdk/issues?state=open&labels=duplicate-candidate&per_page=100&page=3",
    ]
...
def test_list_issue_comments_paginates(monkeypatch):
    ...
    assert module.list_issue_comments("Creanova/agent-sdk", 7) == [
        {"id": 1},
        {"id": 2},
    ]
    assert requested_paths == [
        "/repos/Creanova/agent-sdk/issues/7/comments?per_page=100&page=1",
        "/repos/Creanova/agent-sdk/issues/7/comments?per_page=100&page=2",
        "/repos/Creanova/agent-sdk/issues/7/comments?per_page=100&page=3",
    ]
...
def test_list_comment_reactions_paginates(monkeypatch):
    ...
    assert module.list_comment_reactions("Creanova/agent-sdk", 99) == [
        {"id": 1},
        {"id": 2},
    ]
    assert requested_paths == [
        "/repos/Creanova/agent-sdk/issues/comments/99/reactions?per_page=100&page=1",
        "/repos/Creanova/agent-sdk/issues/comments/99/reactions?per_page=100&page=2",
        "/repos/Creanova/agent-sdk/issues/comments/99/reactions?per_page=100&page=3",
    ]
...
def test_list_helpers_raise_on_non_list_payloads(monkeypatch):
    ...
    with pytest.raises(
        ...
        module.list_open_issues("Creanova/agent-sdk")
    with pytest.raises(
        RuntimeError, match="Expected list response while listing comments"
    ):
        module.list_issue_comments("Creanova/agent-sdk", 7)
    with pytest.raises(
        RuntimeError, match="Expected list response while listing reactions"
    ):
        module.list_comment_reactions("Creanova/agent-sdk", 9)
...
def test_extract_duplicate_metadata_and_veto_helpers():
    ...
    assert module.extract_duplicate_metadata(
        "<!-- Creanova-duplicate-check canonical=42 auto-close=true -->"
    ) == (42, True)
...
def test_find_latest_auto_close_comment_prefers_newest_timestamp():
    ...
    comments = [
        {
            "body": "<!-- Creanova-duplicate-check canonical=10 auto-close=true -->",
            "created_at": "2026-04-20T00:00:00Z",
            "id": 1,
        },
        {
            "body": "<!-- Creanova-duplicate-check canonical=11 auto-close=true -->",
...
def test_find_latest_auto_close_comment_returns_latest_candidate():
    ...
    comments = [
        {"body": "plain comment"},
        {
            "body": "<!-- Creanova-duplicate-check canonical=10 auto-close=false -->",
            "id": 1,
        ...
        {
            "body": "<!-- Creanova-duplicate-check canonical=12 auto-close=true -->",
            "id": 3,
            "created_at": "2026-04-20T00:00:00Z",
        },
    ]
...
def test_close_issue_propagates_comment_failure(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="comment failed"):
        module.close_issue_as_duplicate("Creanova/agent-sdk", 123, 45, dry_run=False)

    assert calls == [
        ("POST", "/repos/Creanova/agent-sdk/issues/123/comments"),
    ]
...
def test_dry_run_helpers_skip_api_calls(monkeypatch):
    ...
    assert module.remove_candidate_label("Creanova/agent-sdk", 1, dry_run=True) is True
    assert module.post_veto_note("Creanova/agent-sdk", 1, dry_run=True) is True
    ...
    assert (
        module.close_issue_as_duplicate("Creanova/agent-sdk", 1, 2, dry_run=True)
        is None
    )
...
def test_close_issue_as_duplicate_removes_label_on_success(monkeypatch):
    ...
    assert calls == [
        ("POST", "/repos/Creanova/agent-sdk/issues/123/comments"),
        ("PATCH", "/repos/Creanova/agent-sdk/issues/123"),
        ("REMOVE_LABEL", "Creanova/agent-sdk#123:False"),
    ]
...
def test_keep_open_due_to_newer_comments_removes_candidate_label(monkeypatch):
    ...
    assert calls == [("Creanova/agent-sdk", 123, False)]
...
def test_auto_close_main_honors_author_veto(monkeypatch, capsys):
    ...
    comments = [
        {
            "id": 11,
            "body": "<!-- Creanova-duplicate-check canonical=45 auto-close=true -->",
            "created_at": old_timestamp,
        }
    ]
    ...
    monkeypatch.setattr(
        ...
        lambda: argparse.Namespace(
            repository="Creanova/agent-sdk", close_after_days=3, dry_run=False
        ),
    )
    ...
    assert removed == [("Creanova/agent-sdk", 123, False)]
    assert veto_notes == [("Creanova/agent-sdk", 123, False)]
...
def test_auto_close_main_closes_old_duplicate(monkeypatch, capsys):
    ...
    assert closed == [("Creanova/agent-sdk", 123, 45, False)]
...
def test_auto_close_main_skips_malformed_issue_data(monkeypatch, capsys):
    ...
    summary = json.loads(capsys.readouterr().out)
    assert summary == {"repository": "Creanova/agent-sdk", "results": []}
...
def test_auto_close_main_removes_label_when_newer_comment_exists(monkeypatch, capsys):
    ...
    assert keep_open_calls == [("Creanova/agent-sdk", 123, False)]
...
def test_parse_agent_json_handles_single_line_fenced_json():
    module = load_module("issue_duplicate_check_Creanova.py")
...
def test_build_prompt_includes_all_sections():
    ...
    prompt = module.build_prompt(
        "Creanova/agent-sdk",
        {
            ...
            "html_url": "https://github.com/Creanova/agent-sdk/issues/123",
        },
    )

    assert "Repository: Creanova/agent-sdk" in prompt
...
def test_build_prompt_handles_missing_fields():
    ...
    prompt = module.build_prompt("Creanova/agent-sdk", {"number": 5})
...
def test_Creanova_headers_requires_api_key(monkeypatch):
    module = load_module("issue_duplicate_check_Creanova.py")

    monkeypatch.delenv("Creanova_API_KEY", raising=False)

    with pytest.raises(
        RuntimeError, match="Creanova_API_KEY environment variable is required"
    ):
        module.Creanova_headers()
...
def test_app_conversation_helpers_preserve_raw_ids(monkeypatch):
    ...
    monkeypatch.setattr(module, "request_json", fake_request_json)
    monkeypatch.setattr(
        module, "Creanova_headers", lambda: {"Authorization": "Bearer test-token"}
    )
...
def test_poll_start_task_raises_on_failed_status(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="Creanova start task failed") as exc:
...
def test_poll_conversation_raises_on_failed_status(monkeypatch):
    ...
    with pytest.raises(
        RuntimeError, match="Creanova conversation ended with failed"
    ) as exc:
        module.poll_conversation(
            "conv-123", poll_interval_seconds=1, max_wait_seconds=10
        )
...
def test_issue_duplicate_main_waits_for_start_task_and_writes_output(
    ...
    assert result["repository"] == "Creanova/agent-sdk"
