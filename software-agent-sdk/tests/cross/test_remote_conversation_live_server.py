...
from Creanova.agent_server.__main__ import preload_modules
from Creanova.sdk import LLM, Agent, AgentContext, Conversation
from Creanova.sdk.conversation import RemoteConversation
...
from Creanova.sdk.hooks import HookConfig, HookDefinition, HookMatcher
from Creanova.sdk.skills import Skill
from Creanova.sdk.subagent import AgentDefinition
...
from Creanova.sdk.workspace import RemoteWorkspace
from Creanova.workspace.docker.workspace import find_available_tcp_port
...
def live_server_env(
    ...
    We set Creanova_AGENT_SERVER_CONFIG_PATH before creating the app so that
    ...
    monkeypatch.setenv("Creanova_AGENT_SERVER_CONFIG_PATH", str(cfg_file))
    ...
    from Creanova.agent_server.api import create_app
    from Creanova.agent_server.config import Config
...
def patched_llm(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_completion(
        ...
        **kwargs,
        ...
        from Creanova.sdk.llm.message import Message
...
def test_preloaded_custom_tool_resolves_in_live_server(
    ...
    from Creanova.sdk.tool import Tool, registry as tool_registry
...
def test_remote_conversation_over_real_server(server_env, patched_llm):
    ...
    for i in range(50):  # up to ~5s
        ...
        # may miss it. See: https://github.com/Creanova/software-agent-sdk/issues/1785
...
def test_openai_chat_completions_gateway_over_real_server(
    ...
    from Creanova.agent_server import (
    ...
    with patch(
        "Creanova.agent_server.openai.service.get_llm_profile_store",
        ...
        with live_server_env(tmp_path, monkeypatch) as env:
            with httpx.Client() as client:
                ...
                assert body["model"] == "Creanova_smoke"
                ...
                conversation_id = response.headers["X-Creanova-ServerConversation-ID"]
                ...
                reused_response = client.post(
                    f"{env['host']}/v1/chat/completions",
                    headers={"X-Creanova-ServerConversation-ID": conversation_id},
                    json={
                        "model": "Creanova_smoke",
                        "messages": [
                ...
                assert (
                    reused_response.headers["X-Creanova-ServerConversation-ID"]
                    == conversation_id
                )
...
def test_openai_gateway_replays_frozen_llm_fixtures(
    ...
    from Creanova.sdk import Message, TextContent
    from Creanova.sdk.llm.llm_profile_store import LLMProfileStore
    from Creanova.sdk.testing import TestLLM
        ...
        with live_server_env(tmp_path, monkeypatch) as env:
            for fixture in fixtures:
                ...
                client = OpenAI(
                    ...
                    base_url=f"{env['host']}/v1",
                    default_headers={
                        "X-Creanova-ServerConversation-ID": str(conversation_id)
                    },
                    timeout=10,
                )
...
def test_conversation_stats_with_live_server(
    ...
    def fake_completion_with_cost(
        ...
        **kwargs,
        ...
        from Creanova.sdk.llm.llm_response import LLMResponse
        from Creanova.sdk.llm.message import Message
        from Creanova.sdk.llm.utils.metrics import TokenUsage
        ...
        from Creanova.sdk.llm.utils.metrics import MetricsSnapshot
...
def test_events_not_lost_during_client_disconnection(
    ...
    See PR #1791 review for details: https://github.com/Creanova/software-agent-sdk/pull/1791#pullrequestreview-3694259068
    """
...
def test_interrupt_endpoint_cancels_running_conversation(
    ...
    with httpx.Client(base_url=host, timeout=15.0) as client:
        ...
        events_resp = client.get(
            ...
            params={
                "kind": ("Creanova.sdk.event.user_action.InterruptEvent"),
            },
        )
