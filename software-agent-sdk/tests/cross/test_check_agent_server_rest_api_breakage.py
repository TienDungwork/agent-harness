...
def test_find_sdk_deprecated_fastapi_routes_in_file_flags_direct_import(tmp_path):
    repo_root = tmp_path
    source = repo_root / "Creanova-agent-server" / "Creanova" / "agent_server"
    source.mkdir(parents=True)
    file_path = source / "router.py"
    file_path.write_text(
        "from Creanova.sdk.utils.deprecation import deprecated\n"
        "\n"
        '@router.get("/foo")\n'
    ...
    assert errors == [
        "Creanova-agent-server/Creanova/agent_server/router.py:5 FastAPI route "
        "`foo` uses Creanova.sdk.utils.deprecation.deprecated; use the route "
...
def test_find_sdk_deprecated_fastapi_routes_in_file_flags_alias_import(tmp_path):
    ...
    file_path = source / "router.py"
    file_path.write_text(
        "import Creanova.sdk.utils.deprecation as dep\n"
        "\n"
        '@router.post("/foo")\n'
