...
from Creanova.sdk import Agent
from Creanova.sdk.context import AgentContext, KeywordTrigger, Skill
...
def _rewrite_Creanova_llms_to_legacy_proxy(value: Any) -> None:
    if isinstance(value, dict):
        model = value.get("model")
        if isinstance(model, str) and model.startswith(Creanova_PROVIDER_PREFIX):
            model_name = model.removeprefix(Creanova_PROVIDER_PREFIX)
            value["model"] = f"{LITELLM_PROXY_PREFIX}{model_name}"
            value["base_url"] = Creanova_LLM_PROXY_BASE_URL
        for child in value.values():
            _rewrite_Creanova_llms_to_legacy_proxy(child)
        return
...
@patch("Creanova.sdk.llm.llm.litellm_completion")
...
@patch("Creanova.sdk.llm.llm.litellm_completion")
def test_Creanova_provider_restore_writes_public_model_shape(mock_completion):
    captured_completion_kwargs: list[dict[str, Any]] = []
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        tools = [Tool(name="TerminalTool"), Tool(name="FileEditorTool")]
        agent = _agent(
            llm_model="Creanova/claude-opus-4-8",
            tools=tools,
            condenser_max_size=80,
            skill_name="skill-v1",
            skill_keyword="alpha",
        ...
        assert llm_payload["model"] == "Creanova/claude-opus-4-8"
        ...
        assert (
            captured_completion_kwargs[-1]["api_base"] == Creanova_LLM_PROXY_BASE_URL
        )


@patch("Creanova.sdk.llm.llm.litellm_completion")
def test_conversation_restore_rewrites_legacy_Creanova_proxy_snapshot(
    mock_completion,
):
    captured_completion_kwargs: list[dict[str, Any]] = []
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        _rewrite_Creanova_llms_to_legacy_proxy(legacy_base_state["agent"])
        ...
        try:
            assert restored.id == initial["conversation_id"]
            assert len(restored.state.events) == initial["event_count"]
            assert restored.agent.llm.model == "Creanova/claude-opus-4-8"

            restored_base_state = lifecycle.read_base_state()
            restored_llm_payload = restored_base_state["agent"]["llm"]
            assert restored_llm_payload["model"] == "Creanova/claude-opus-4-8"
