...
os.environ.setdefault("Creanova_SUPPRESS_BANNER", "1")
...
def test_get_pypi_baseline_version_prefers_current_or_previous(monkeypatch) -> None:
    ...
    assert get_pypi_baseline_version("Creanova-sdk", "1.1.0") == "1.1.0"
    assert get_pypi_baseline_version("Creanova-sdk", "1.2.0") == "1.1.0"
...
def test_get_pypi_baseline_version_raises_on_metadata_failure(monkeypatch) -> None:
    ...
    with pytest.raises(
        PersistedSettingsCompatError,
        match="Failed to fetch PyPI metadata for Creanova-sdk",
    ):
        get_pypi_baseline_version("Creanova-sdk", "1.2.0")
