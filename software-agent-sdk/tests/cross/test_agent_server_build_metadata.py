...
def test_server_workflow_passes_git_metadata_build_args() -> None:
    ...
    assert "Creanova_BUILD_GIT_SHA=${{ env.SDK_SHA }}" in workflow_text
    assert "Creanova_BUILD_GIT_REF=${{ env.SDK_REF }}" in workflow_text


def test_agent_server_binary_copies_Creanova_distribution_metadata() -> None:
    """The frozen binary should preserve Creanova package metadata."""
