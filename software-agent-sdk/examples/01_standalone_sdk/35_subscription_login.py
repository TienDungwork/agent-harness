...
- Credential caching (~/.Creanova/auth/)
...
- Creanova_SUBSCRIPTION_MODEL: Model to use (default: gpt-5.2-codex)
- Creanova_SUBSCRIPTION_AUTH_METHOD: "browser" or "device_code"
  (default: browser)
- Creanova_SUBSCRIPTION_FORCE_LOGIN: Set to "1" to force fresh login
...
model = os.getenv("Creanova_SUBSCRIPTION_MODEL", "gpt-5.2-codex")
auth_method_env = os.getenv("Creanova_SUBSCRIPTION_AUTH_METHOD", "browser")
if auth_method_env not in ("browser", "device_code"):
    raise ValueError(
        "Creanova_SUBSCRIPTION_AUTH_METHOD must be 'browser' or 'device_code'"
    )
auth_method: AuthMethod = auth_method_env
force_login = os.getenv("Creanova_SUBSCRIPTION_FORCE_LOGIN") == "1"
