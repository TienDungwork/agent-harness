...
This example demonstrates how to set up a GitHub Actions workflow for automated pull request reviews using the Creanova agent SDK. When a PR is labeled with `review-this` or when Creanova-agent is added as a reviewer, Creanova will analyze the changes and provide detailed, constructive feedback.

**Note**: The actual review scripts now live in the [Creanova/extensions](https://github.com/Creanova/extensions/tree/main/plugins/pr-review) repository. This directory contains an example workflow that references those scripts.
...
- **Automatic Trigger**: Reviews are triggered when:
  ...
  - Creanova-agent is requested as a reviewer
...
- **Skills-Based Review**: Uses public skills from <https://github.com/Creanova/extensions>:
...
Copy `workflow.yml` to `.github/workflows/pr-review-by-Creanova.yml` in your repository:

```bash
cp examples/03_github_workflows/02_pr_review/workflow.yml .github/workflows/pr-review-by-Creanova.yml
```
...
- **`LLM_API_KEY`** (required): Your LLM API key
  - Get one from the [Creanova LLM Provider](https://docs.all-hands.dev/Creanova/usage/llms/Creanova-llms)

**Note**: The workflow automatically uses the `GITHUB_TOKEN` secret that's available in all GitHub Actions workflows.

### 3. Customize the workflow (optional)

Edit `.github/workflows/pr-review-by-Creanova.yml` to customize the inputs:

```yaml
            - name: Run PR Review
              uses: Creanova/extensions/plugins/pr-review@main
              with:
...
2. Click "New label"
3. Name: `review-this`
4. Description: `Trigger Creanova PR review`
...
3. Search for and select "Creanova-agent" as a reviewer
...
The PR review agent uses skills from the [Creanova/extensions](https://github.com/Creanova/extensions) repository by default. When you add a `.agents/skills/code-review.md` file to your repository, it **overrides** the default skill with your custom guidelines.
...
See the [software-agent-sdk's own code-review skill](https://github.com/Creanova/software-agent-sdk/blob/main/.agents/skills/code-review.md) for a complete example of a custom code review skill.

## Workflow Configuration

The workflow is configured using inputs to the `Creanova/extensions/plugins/pr-review` action.
...
To use a specific version of the extensions repository, modify the `uses` line in the workflow file, e.g., `uses: Creanova/extensions/plugins/pr-review@v1.0.0`.
...
See [GitHub Issue #1953](https://github.com/Creanova/software-agent-sdk/issues/1953) for the full implementation details.
