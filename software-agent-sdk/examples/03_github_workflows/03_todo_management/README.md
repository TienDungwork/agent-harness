...
This example demonstrates how to use the Creanova SDK to automatically scan a codebase for configurable TODO comments and create pull requests to implement them. This showcases practical automation and self-improving codebase capabilities.
...
2. **Agent** (`agent.py`) - Uses Creanova to implement individual TODOs
...
- 🤖 **AI Implementation**: Uses Creanova agent to automatically implement TODOs
...
1. **Scan Phase**: The workflow scans your codebase for configurable TODO comments
   - Default identifier: `TODO(Creanova)` (customizable via workflow input)
...
2. **Process Phase**: For each TODO found:
   ...
   - Uses Creanova agent to implement the TODO
...
- **`agent.py`**: Creanova agent for TODO implementation
...
```python
# TODO(Creanova): Add input validation for user email
def process_user_email(email):
    return email.lower()

# TODO(Creanova): Implement caching mechanism for API responses
...
- `# TODO(Creanova): description` (Python, Shell, etc.)
- `// TODO(Creanova): description` (TypeScript, Java, Rust, etc.)
...
3. (Optional) Configure parameters:
   ...
   - **TODO Identifier**: Custom identifier to search for (default: `TODO(Creanova)`)
...
- `--identifier, -i`: TODO identifier to search for (default: `TODO(Creanova)`)
