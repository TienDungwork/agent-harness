"""Example: Inherit SaaS credentials via CreanovaCloudWorkspace.

This example shows the simplified flow where your Creanova Cloud account's
...
Requirements:
  - Creanova_CLOUD_API_KEY: API key for Creanova Cloud (the only credential needed)

Optional:
  - Creanova_CLOUD_API_URL: Override the Cloud API URL (default: https://app.all-hands.dev)
...
from Creanova.tools.preset.default import get_default_agent
...
cloud_api_key = os.getenv("Creanova_CLOUD_API_KEY")
if not cloud_api_key:
    logger.error("Creanova_CLOUD_API_KEY required")
    exit(1)

cloud_api_url = os.getenv("Creanova_CLOUD_API_URL", "https://app.all-hands.dev")
logger.info(f"Using Creanova Cloud API: {cloud_api_url}")
...
with CreanovaCloudWorkspace(
    ...
    if secret_names:
        ...
        # No secret was configured on Creanova Cloud
