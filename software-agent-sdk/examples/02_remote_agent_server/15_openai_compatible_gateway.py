...
Python SDK. The returned ``X-Creanova-ServerConversation-ID`` header is passed
back on a second call to continue the same Creanova conversation.
"""
...
# The gateway runs a full Creanova agent, but OpenAI clients still need a
# normal model-like name. We create an LLM profile below and expose it as
# `Creanova_<profile_name>` through `/v1/models`.
...
gateway_model = f"Creanova_{profile_name}"
...
with ManagedAPIServer(
    ...
    # `gateway_demo` becomes visible to OpenAI clients as `Creanova_gateway_demo`.
    ...
    # response header that identifies the Creanova conversation created behind
    ...
    conversation_id = first_response.headers.get("X-Creanova-ServerConversation-ID")
    assert conversation_id is not None
    UUID(conversation_id)

    first_answer = first_completion.choices[0].message.content
    print(f"First answer: {first_answer}")
    print(f"Creanova conversation ID: {conversation_id}")
    ...
    second_completion = openai_client.chat.completions.create(
        ...
        extra_headers={"X-Creanova-ServerConversation-ID": conversation_id},
    )
