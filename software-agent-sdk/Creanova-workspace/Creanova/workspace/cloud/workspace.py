"""Creanova Cloud workspace implementation using Cloud API."""
...
from Creanova.sdk.mcp.config import MCPServer, MCPTransport
from Creanova.sdk.workspace.remote.base import RemoteWorkspace
from Creanova.sdk.workspace.repo import CloneResult, RepoMapping, RepoSource
...
# Standard exposed URL names from Creanova Cloud
...
class CreanovaCloudWorkspace(RemoteWorkspace):
    """Remote workspace using Creanova Cloud API.

    This workspace connects to Creanova Cloud (app.all-hands.dev) to provision
    ...
    running inside an Creanova Cloud Runtime sandbox.  Instead of creating or
    ...
    Example:
        ...
        workspace = CreanovaCloudWorkspace(
            cloud_api_url="https://app.all-hands.dev",
            cloud_api_key="your-api-key",
            sandbox_spec_id="ghcr.io/Creanova/agent-server:main-python",
        )

        # Running inside an Creanova Cloud Runtime (local agent-server mode)
        workspace = CreanovaCloudWorkspace(
            local_agent_server_mode=True,
            cloud_api_url="https://app.all-hands.dev",
            cloud_api_key=os.environ["Creanova_API_KEY"],
        )
    """
    ...
    local_agent_server_mode: bool = Field(
        default=False,
        description=(
            "When True, assume the SDK is running inside an Creanova Cloud "
    ...
    cloud_api_url: str = Field(
        description=(
            "Base URL of Creanova Cloud API "
    ...
    cloud_api_key: str = Field(
        description=(
            "API key for authenticating with Creanova Cloud. "
            "Required in all modes — used for get_llms / get_secrets."
        ),
    )
    ...
    def default_conversation_tags(self) -> dict[str, str]:
        ...
        When running inside an Creanova Cloud Runtime (local_agent_server_mode=True),
    ...
    def _api_headers(self) -> dict[str, str]:
        ...
        Uses Bearer token authentication as per Creanova Cloud API.
        """
    ...
    def _create_new_sandbox(self) -> None:
        ...
        logger.info("Starting sandbox via Creanova Cloud API...")
    ...
    def pause(self) -> None:
        ...
        Note: Creanova Cloud does not currently support pausing sandboxes.
        ...
        raise NotImplementedError(
            "CreanovaCloudWorkspace.pause() is not yet supported - "
    ...
    def get_llm(self, profile_name: str | None = None, **llm_kwargs: Any) -> LLM:
        ...
        Example:
            >>> with CreanovaCloudWorkspace(...) as workspace:
    ...
    def __enter__(self) -> CreanovaCloudWorkspace:
        return self
