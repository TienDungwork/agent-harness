...
- This directory (`Creanova-workspace/Creanova/workspace/`) contains workspace implementations under the `Creanova.workspace.*` namespace (Docker, Apptainer, cloud, and API-remote).
- Each backend lives in its own subpackage (e.g. `docker/`, `cloud/`) and typically exposes a `*Workspace` class from `workspace.py`.
- The published import surface is `Creanova-workspace/Creanova/workspace/__init__.py` (`__all__` is treated as public API). Keep imports lightweight so `import Creanova.workspace` does not pull in build-time dependencies.
- These classes should remain compatible with the SDK workspace interfaces and types (for example `Creanova.sdk.workspace.RemoteWorkspace`, `TargetType`, `PlatformType`).
