...
from Creanova.sdk.utils.command import execute_command
from Creanova.sdk.workspace import PlatformType, RemoteWorkspace
from Creanova.workspace.docker.workspace import (
...
class ApptainerWorkspace(RemoteWorkspace):
    ...
    This workspace creates an Apptainer container running a pre-built Creanova
    ...
    Example:
        with ApptainerWorkspace(
            server_image="ghcr.io/Creanova/agent-server:latest-python"
        ) as workspace:
    ...
    extra_bind_mounts: list[str] = Field(
        ...
        description=(
            ...
            "Use src[:dest[:opts]] syntax. Creanova_APPTAINER_EXTRA_BINDS can "
    ...
    def _start_container(self) -> None:
        ...
        env_extra_binds = [
            item.strip()
            for item in os.getenv("Creanova_APPTAINER_EXTRA_BINDS", "").split(",")
            if item.strip()
        ]
