...
class DockerDevWorkspace(DockerWorkspace):
    ...
    Note: This class requires the Creanova SDK workspace structure and should
    only be used within the Creanova development environment or when you have
    ...
    def _validate_images(self):
        ...
        if self.base_image and "ghcr.io/Creanova/agent-server" in self.base_image:
            raise ValueError(
    ...
    def _build_image_from_base(
        *, base_image: str, target: TargetType, platform: PlatformType
        ...
        """
        from Creanova.agent_server.docker.build import BuildOptions, build

        if "ghcr.io/Creanova/agent-server" in base_image:
            raise RuntimeError(
                "base_image cannot be a pre-built agent-server image. "
                "Use server_image=... instead."
            )
