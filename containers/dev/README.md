...
> [!WARNING]
> This way of running Creanova is not officially supported. It is maintained by the community and may not work.
...
It could take some time if you are running for the first time as Docker will pull all the  tools required for building Creanova. The next time you run again, it should be instant.

## Build and run

If everything goes well, you should be inside a container after Docker finishes building the `Creanova:dev` image similar to the following:
...
See details for [developing in docker](https://code.visualstudio.com/docs/devcontainers/containers) or simply ask `Creanova` ;-)
