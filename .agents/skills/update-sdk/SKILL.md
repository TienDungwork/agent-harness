---
name: update-sdk
description: This skill should be used when the user asks to "update SDK", "bump SDK version", "pin SDK to a commit", "test unreleased SDK", "update agent-server image", "bump the version", "prepare a release", "what files change for a release", or needs to know how SDK packages are managed in the Creanova repository. For detailed reference material, see references/docker-image-locations.md and references/sdk-pinning-examples.md in this skill directory.
---

# Update SDK

Bump SDK packages (`Creanova-sdk`, `Creanova-agent-server`, `Creanova-tools`), pin them to unreleased commits for testing, and cut an Creanova release.
...
| File | What to change |
|------|----------------|
| `pyproject.toml` | `Creanova-sdk`, `Creanova-agent-server`, `Creanova-tools` in **two** sections: the `dependencies` array (PEP 508) **and** `[tool.poetry.dependencies]` |
| `Creanova/app_server/sandbox/sandbox_spec_service.py` | `AGENT_SERVER_IMAGE` constant — set to `ghcr.io/Creanova/agent-server:<version>-python` |
...
|------|----------------|:------------------------:|
| `Creanova/app_server/sandbox/sandbox_spec_service.py` | `AGENT_SERVER_IMAGE = 'ghcr.io/Creanova/agent-server:<tag>-python'` | ✅ Yes |
...
> **Note:** `Creanova/version.py` reads the version from `pyproject.toml` at runtime — no manual edit needed there.

### Compose Files (2 files)

Both compose files should use `ghcr.io/Creanova/agent-server` with the current SDK version tag.
...
```bash
grep -n "Creanova-sdk\|Creanova-agent-server\|Creanova-tools" pyproject.toml
grep -n "AGENT_SERVER_IMAGE" Creanova/app_server/sandbox/sandbox_spec_service.py
...
Every push to `main` / `saas-rel-*` / `oss-rel-*` builds and publishes `ghcr.io/Creanova/Creanova` and `ghcr.io/Creanova/enterprise-server` images for that commit (tagged by SHA, short SHA, and branch name).
...
| `Creanova/app_server/sandbox/sandbox_spec_service.py` | `AGENT_SERVER_IMAGE` — use the merge-commit SHA tag, NOT the head-commit SHA |
