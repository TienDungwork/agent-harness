...
from Creanova.app_server.sandbox.docker_sandbox_spec_service import get_docker_client
...
class DockerSandboxService(SandboxService):
    ...
    async def start_sandbox(
        ...
        # frontend when running Creanova on a remote machine.
    ...
    async def delete_sandbox(self, sandbox_id: str) -> bool:
        ...
        try:
            ...
            try:
                volume_name = f'Creanova-workspace-{sandbox_id}'
...
class DockerSandboxServiceInjector(SandboxServiceInjector):
    ...
    host_port: int = Field(
        default=3000,
        description=(
            'The port on which the main Creanova app server is running. '
            'Used for webhook callbacks from agent-server containers. '
            'If running Creanova on a non-default port, set this to match. '
