...
from Creanova.agent_server import env_parser
from Creanova.app_server.errors import SandboxError
from Creanova.app_server.sandbox.sandbox_spec_models import (
    SandboxSpecInfo,
    SandboxSpecInfoPage,
)
from Creanova.app_server.services.injector import Injector
from Creanova.sdk.utils.models import DiscriminatedUnionMixin

_DEFAULT_REPOSITORY = 'ghcr.io/Creanova/agent-server'


@cache
def _bundled_agent_server_version() -> str:
    """Version of the installed Creanova-agent-server package.

    Creanova-agent-server is a hard runtime dependency of the V1 app server;
    ...
    """
    return importlib.metadata.version('Creanova-agent-server')
    ...
    if repository == _DEFAULT_REPOSITORY:
        ...
        if len(parts) == 2 and parts[0] != bundled_version:
            ...
            logging.getLogger(__name__).warning(
                ...
                f'Creanova-sdk (using {updated_tag} instead). Pin to a custom '
