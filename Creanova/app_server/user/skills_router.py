...
from Creanova.app_server.config import depends_user_context
...
# skills/ is at the repo root, two levels above the Creanova package __file__
GLOBAL_SKILLS_DIR = Path(Creanova.__file__).parent.parent / 'skills'
USER_SKILLS_DIR = Path.home() / '.Creanova' / 'microagents'
...
async def _clone_marketplace_repo(
    ...
    try:
        clone_dir = Path(
            tempfile.mkdtemp(prefix=f'Creanova_marketplace_{marketplace.name}_')
        )
