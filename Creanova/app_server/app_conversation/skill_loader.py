...
from Creanova.app_server.integrations.provider import ProviderHandler, ProviderType
...
async def _determine_org_repo_path(
    ...
    Examples:
        - GitHub/Bitbucket: ('owner/.Creanova', 'owner')
        - GitLab: ('owner/Creanova-config', 'owner')
        - Azure DevOps: ('org/Creanova-config/Creanova-config', 'org')
    """
    ...
    if is_gitlab:
        org_Creanova_repo = f'{org_name}/Creanova-config'
    elif is_azure_devops:
        org_Creanova_repo = f'{org_name}/Creanova-config/Creanova-config'
    else:
        org_Creanova_repo = f'{org_name}/.Creanova'

    return org_Creanova_repo, org_name


async def _get_org_repository_url(
    org_Creanova_repo: str, user_context: UserContext
) -> str | None:
    """Get authenticated Git URL for organization repository.

    Args:
        org_Creanova_repo: Organization repository path
    ...
    try:
        remote_url = await user_context.get_authenticated_git_url(
            org_Creanova_repo, is_optional=True
        )
        return remote_url
    except AuthenticationError as e:
        _logger.debug(
            f'org-level skill directory {org_Creanova_repo} not found: {str(e)}'
        )
        return None
    except Exception as e:
        _logger.debug(
            f'Failed to get authenticated URL for {org_Creanova_repo}: {str(e)}'
        )
        return None
...
def _candidate_repo_paths(provider: ProviderType, owner: str) -> list[str]:
    ...
    GitHub-style providers expose two independent repos (``.Creanova`` and
    ``.agents``) that are loaded concurrently. GitLab and Azure DevOps use a
    single ``Creanova-config`` repository (they have no ``.agents`` analog).
    ...
    Returns:
        List of repository paths (e.g., ['owner/.Creanova', 'owner/.agents']).
    """
    if provider == ProviderType.GITLAB:
        return [f'{owner}/Creanova-config']
    if provider == ProviderType.AZURE_DEVOPS:
        return [f'{owner}/Creanova-config/Creanova-config']
    return [f'{owner}/.Creanova', f'{owner}/.agents']
...
async def build_org_configs(
    ...
    resolving both ``.Creanova`` and ``.agents`` repos for GitHub-style
    ...
    if selected_repository and len(selected_repository.split('/')) >= 2:
        try:
            org_Creanova_repo, org_name = await _determine_org_repo_path(
                selected_repository, user_context
            )
            selected_provider = await _get_provider_type(
                selected_repository, user_context
            )
            candidates.append((org_Creanova_repo, org_name, selected_provider))
