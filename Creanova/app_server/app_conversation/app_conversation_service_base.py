...
from Creanova.app_server.sandbox.sandbox_models import SandboxInfo
...
from Creanova.app_server.utils.auth import looks_like_jwt
from Creanova.app_server.utils.git import ensure_valid_git_branch_name
from Creanova.sdk import AgentBase, LLMSummarizingCondenser
from Creanova.sdk.context import AgentContext
from Creanova.sdk.llm import LLM
from Creanova.sdk.security import (
    AlwaysConfirm,
...
def get_project_dir(
    ...
    `.Creanova/` configuration (setup.sh, pre-commit.sh, skills/, etc.).
    ...
    - .Creanova/setup.sh execution
    - .Creanova/pre-commit.sh (git hooks setup)
    - .Creanova/skills/ (project skills)
    - PLAN.md path

    Args:
        working_dir: Base working directory path in the sandbox
            (e.g., '/workspace/project' from sandbox_spec)
        selected_repository: Repository name (e.g., 'Creanova/software-agent-sdk')
...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        - Public skills (from Creanova/skills GitHub repo)
        - User skills (from ~/.Creanova/skills/)
        - Organization skills (from {org}/.Creanova repo)
        - Project/repo skills (from repo .agents/skills/, .Creanova/microagents/, and legacy .Creanova/skills/)
    ...
    async def run_setup_scripts(
        ...
        # for all .Creanova/ features (setup.sh, pre-commit.sh, skills).
    ...
    async def clone_or_init_git_repo(
        ...
        if request.selected_branch:
            ...
            Creanova_workspace_branch = f'Creanova-workspace-{random_str}'
            checkout_command = (
                f'git checkout -b {shlex.quote(Creanova_workspace_branch)}'
            )
    ...
    async def _configure_azure_devops_git_credential_helper(
        ...
        helper_path = git_dir / '.git' / 'Creanova-azure-devops-credential-helper'
        secret_path = (
...
"""
    ...
    async def maybe_setup_git_hooks(
        ...
        """Set up git hooks if .Creanova/pre-commit.sh exists in the project root.

        Args:
            workspace: Remote workspace for command execution.
            project_dir: Project root directory (repo root when a repo is selected).
        """
        command = 'mkdir -p .git/hooks && chmod +x .Creanova/pre-commit.sh'
        ...
        with tempfile.TemporaryFile(mode='w+t') as temp_file:
            ...
            if download_result.success:
                ...
                if 'This hook was installed by Creanova' not in temp_file.read():
