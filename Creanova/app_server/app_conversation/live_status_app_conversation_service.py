...
from Creanova.app_server.app_conversation.app_conversation_info_service import (
...
from Creanova.app_server.app_conversation.app_conversation_start_task_service import (
    AppConversationStartTaskService,
)
from Creanova.app_server.app_conversation.conversation_secret_enricher import (
    ConversationSecretEnricher,
)
from Creanova.app_server.app_conversation.hook_loader import (
    load_hooks_from_agent_server,
)
from Creanova.app_server.app_conversation.sql_app_conversation_info_service import (
    SQLAppConversationInfoService,
)
from Creanova.app_server.config import (
    get_event_callback_service,
    resolve_provider_llm_base_url,
)
from Creanova.app_server.errors import SandboxError
from Creanova.app_server.event.event_service import EventService
from Creanova.app_server.event_callback.event_callback_models import EventCallback
from Creanova.app_server.event_callback.event_callback_service import (
    EventCallbackService,
)
from Creanova.app_server.event_callback.set_title_callback_processor import (
    SetTitleCallbackProcessor,
)
from Creanova.app_server.integrations.provider import PROVIDER_TOKEN_TYPE, ProviderType
from Creanova.app_server.integrations.service_types import SuggestedTask
from Creanova.app_server.pending_messages.pending_message_service import (
    PendingMessageService,
)
from Creanova.app_server.sandbox.docker_sandbox_service import DockerSandboxService
from Creanova.app_server.sandbox.sandbox_models import (
    AGENT_SERVER,
    SandboxInfo,
    SandboxStatus,
)
from Creanova.app_server.sandbox.sandbox_service import SandboxService
from Creanova.app_server.sandbox.sandbox_spec_service import (
    SandboxSpecService,
    is_custom_sandbox_spec,
)
from Creanova.app_server.services.injector import InjectorState
from Creanova.app_server.services.jwt_service import JwtService
from Creanova.app_server.settings.llm_profiles import resolve_profile_llm
from Creanova.app_server.settings.marketplace_composition import (
...
from Creanova.app_server.utils.docker_utils import (
    replace_localhost_hostname_for_docker,
)
from Creanova.app_server.utils.git import ensure_valid_git_branch_name
from Creanova.app_server.utils.import_utils import get_impl
from Creanova.app_server.utils.llm_metadata import (
    get_llm_metadata,
    should_set_litellm_extra_body,
)
from Creanova.app_server.utils.redis_lock import (
    LockError,
    RedisLockUnavailable,
    refresh_lock_periodically,
    try_acquire_redis_lock,
)
from Creanova.sdk import Agent, AgentContext, LocalWorkspace
from Creanova.sdk.hooks import HookConfig
from Creanova.sdk.llm import LLM
from Creanova.sdk.llm.llm_profile_store import PROFILE_NAME_REGEX
from Creanova.sdk.mcp.config import MCPServer
from Creanova.sdk.plugin import PluginSource
from Creanova.sdk.secret import LookupSecret, StaticSecret
from Creanova.sdk.settings import ACPAgentSettings
from Creanova.sdk.subagent import get_registered_agent_definitions
from Creanova.sdk.tool.builtins import SwitchLLMTool
from Creanova.sdk.utils.redact import (
...
from Creanova.tools.preset.planning import (
...
def _expected_sdk_version() -> str | None:
    """App's pinned Creanova-sdk version, or None if its metadata is unresolvable."""
    try:
        return importlib.metadata.version('Creanova-sdk')
...
class LiveStatusAppConversationService(AppConversationServiceBase):
    ...
    web_url: str | None
    Creanova_provider_base_url: str | None
    ...
    async def _start_app_conversation(
        ...
        try:
            ...
            # ``Creanova-sdk`` release does not yet expose ``user_id`` on
            ...
            # the local SDK model knows about it. Remove this once Creanova
            ...
            try:
                ...
                # A custom image that 500s on create is usually an Creanova-sdk
                ...
                if is_custom_sandbox_spec(sandbox.sandbox_spec_id):
                    ...
                    raise SandboxError(
                        ...
                        f'{sandbox.sandbox_spec_id}. Verify its Creanova-sdk '
            ...
            if request_agent.agent_kind == 'acp':
                llm_model = getattr(request_agent, 'acp_model', None)
                agent_kind = 'acp'
    ...
    async def _seed_sandbox_profiles(
        ...
        from Creanova.app_server.settings.settings_router import LITE_LLM_API_URL
    ...
    def _configure_llm(self, user: UserInfo, llm_model: str | None) -> LLM:
        ...
        base_url = resolve_provider_llm_base_url(
            ...
            provider_base_url=self.Creanova_provider_base_url,
        )
    ...
    async def _maybe_refresh_managed_llm_key(self, user: UserInfo, llm: LLM) -> LLM:
        ...
        is_Creanova_provider = bool(llm.model and llm.model.startswith('Creanova/'))
        uses_Creanova_provider_proxy = is_Creanova_provider and (
            not normalized_base_url or 'all-hands.dev' in normalized_base_url.lower()
        )
        if normalized_base_url != managed_base_url and not uses_Creanova_provider_proxy:
            _logger.debug(
                ...
                extra={
                    ...
                    'is_Creanova_provider': is_Creanova_provider,
                },
            )
            return llm
        ...
        try:
            ...
            _logger.debug(
                ...
                extra={
                    ...
                    'model': llm.model,
                    'base_url': normalized_base_url or None,
                    'uses_Creanova_provider_proxy': uses_Creanova_provider_proxy,
                },
            )
            ...
            if rotation.status == ManagedLlmKeyStatus.ROTATED and rotation.new_key:
                _logger.info(
                    ...
                    extra={
                        ...
                        'org_id': str(org_id),
                        'model': llm.model,
                        'Creanova_type': getattr(rotation, 'Creanova_type', None),
                    },
                )
    ...
    async def _add_system_mcp_servers(
        ...
        """Add system-generated MCP servers (default Creanova server).
        ...
        Args:
            ...
            conversation_id: Conversation ID forwarded to the Creanova MCP server
        """
        if not self.web_url:
            return

        headers = {'X-Creanova-ServerConversation-ID': SecretStr(str(conversation_id))}
        ...
        # Add default Creanova MCP server (includes Tavily proxy if configured)
    ...
    def _merge_custom_mcp_config(
        ...
        Handles both Creanova and ACP agent settings: a resolved ACP profile's
    ...
    def _build_observability_context(
        ...
        *,
        ...
        tags = ['app:Creanova', f'agent_kind:{agent_kind}']
    ...
    def _apply_server_agent_overrides(
        ...
        * LLM tracing metadata for SaaS analytics.
        ...
        # LLM tracing metadata for Creanova/ models
    ...
    async def _load_hooks_from_workspace(
        ...
        """Load hooks from .Creanova/hooks.json in the remote workspace.

        This enables project-level hooks to be automatically loaded when starting
        a conversation, similar to how Creanova-CLI loads hooks from the workspace.
    ...
    async def _build_observability_metadata(
        ...
        Shared by the Creanova and ACP request builders. The commit is the
    ...
    async def _build_start_conversation_request_for_user(
        ...
        if api_secrets:
            from Creanova.app_server.constants import (
    ...
    async def _resolve_registered_marketplaces(
        ...
        if not marketplace_plugin_loading_enabled():
            return None
        try:
            from Creanova.app_server.shared import SettingsStoreImpl
    ...
    async def _build_acp_start_conversation_request(
        ...
        # subprocess (#15044 §7). Only custom servers — the system Creanova MCP
    ...
    async def _process_pending_messages(
        ...
        # The frontend uses task-{uuid.hex} format (no hyphens), matching CreanovaUUID serialization
...
class LiveStatusAppConversationServiceInjector(AppConversationServiceInjector):
    ...
    async def inject(
        ...
        async with (
            ...
            try:
                from Creanova.app_server.shared import server_config
            ...
            yield LiveStatusAppConversationService(
                ...
                web_url=web_url,
                Creanova_provider_base_url=config.Creanova_provider_base_url,
