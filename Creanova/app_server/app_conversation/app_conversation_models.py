...
from Creanova.app_server.integrations.service_types import ProviderType, SuggestedTask
...
from Creanova.sdk.conversation import ConversationExecutionStatus
from Creanova.sdk.llm import MetricsSnapshot
from Creanova.sdk.plugin import PluginSource
from Creanova.sdk.profiles import LaunchedAgentProfile
...
class AppConversationInfo(BaseModel):
    ...
    agent_kind: str = 'Creanova'

    metrics: MetricsSnapshot | None = None

    parent_conversation_id: CreanovaUUID | None = None
    sub_conversation_ids: list[CreanovaUUID] = Field(default_factory=list)
    ...
    def acp_server(self) -> str | None:
        ...
        ``agent_kind`` so a stray tag never reports a provider for an Creanova
        conversation.
        """
...
class AppConversationStartRequest(CreanovaModel):
...
class AppConversationStartTask(CreanovaModel):
    ...
    app_conversation_id: CreanovaUUID | None = Field(
...
class AppConversationStartTaskPage(CreanovaModel):
