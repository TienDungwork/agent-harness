"""Settings router for Creanova App Server.
...
from Creanova.analytics import get_analytics_service
...
from Creanova.app_server.settings.llm_profiles import (
...
def _post_merge_llm_fixups(settings: Settings) -> None:
    ...
    rules to :func:`Creanova.app_server.utils.llm.resolve_llm_base_url` so the
    personal-save and enterprise org-defaults paths stay in lockstep.
    """
    if not isinstance(settings.agent_settings, CreanovaAgentSettings):
        return
...
async def load_settings(
    ...
    try:
        ...
        if is_Creanova_model(llm.model):
