...
The Agent-Profile analogue of :class:`~Creanova.app_server.settings.llm_profiles.LLMProfiles`,
one level up: an :class:`~Creanova.sdk.profiles.AgentProfile` *references* an LLM
...
* an :class:`~Creanova.sdk.profiles.AgentProfileStoreProtocol` implementation, so the
...
# (Creanova-agent-server/.../agent_profiles_router.py) so both backends share
# one cap and ``save_profile_preserving_identity`` enforces it identically.
MAX_AGENT_PROFILES: Final[int] = 50

_AgentProfile: TypeAlias = CreanovaAgentProfile | ACPAgentProfile
...
class AgentProfiles(BaseModel):
    ...
    Conforms to :class:`~Creanova.sdk.profiles.AgentProfileStoreProtocol`: the
    ...
    def set_llm_profile_ref(self, name: str, new_ref: str) -> None:
        """Repoint one Creanova profile's ``llm_profile_ref`` (the FK cascade
