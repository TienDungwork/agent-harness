...
from Creanova.app_server.config_api.config_models import (
    LLMModel,
    LLMModelPage,
    Provider,
    ProviderPage,
...
from Creanova.app_server.utils.paging_utils import paginate_results
...
def _to_providers(models_response: ModelsResponse) -> list[Provider]:
    """Extract unique providers, sorted with ``Creanova`` first, then other
    ...
    # ``Creanova`` is the managed provider and should always appear first,
    # followed by other verified providers (alphabetical), then unverified
    # providers (alphabetical).
    providers.sort(key=lambda p: (not p.verified, p.name != 'Creanova', p.name))
    return providers
