"""Configuration for the Creanova App Server."""
...
import Creanova.app_server.event_callback  # noqa: F401
from Creanova.agent_server.env_parser import from_env
...
from Creanova.app_server.app_lifespan.app_lifespan_service import AppLifespanService
from Creanova.app_server.app_lifespan.oss_app_lifespan_service import (
    OssAppLifespanService,
)
from Creanova.app_server.config_api.config_models import AppMode
from Creanova.app_server.config_api.llm_model_service import (
    LLMModelService,
    LLMModelServiceInjector,
)
from Creanova.app_server.event.event_service import EventService, EventServiceInjector
from Creanova.app_server.event_callback.event_callback_service import (
    EventCallbackService,
    EventCallbackServiceInjector,
)
from Creanova.app_server.file_store.files import FileStore
from Creanova.app_server.file_store.local import LocalFileStore
from Creanova.app_server.pending_messages.pending_message_service import (
    PendingMessageService,
    PendingMessageServiceInjector,
)
from Creanova.app_server.sandbox.sandbox_service import (
    SandboxService,
...
from Creanova.app_server.services.db_session import (  # noqa: F401  (re-exported)
    depends_db_session,
    get_db_session,
)
from Creanova.app_server.services.db_session_injector import (
    DbSessionInjector,
)
from Creanova.app_server.services.httpx_client_injector import HttpxClientInjector
from Creanova.app_server.services.injector import InjectorState
from Creanova.app_server.services.jwt_service import JwtService, JwtServiceInjector
from Creanova.app_server.user.user_context import UserContext, UserContextInjector
from Creanova.app_server.utils.environment import StorageProvider, get_storage_provider
from Creanova.app_server.web_client.default_web_client_config_injector import (
    DefaultWebClientConfigInjector,
)
from Creanova.app_server.web_client.web_client_config_injector import (
    WebClientConfigInjector,
)
from Creanova.sdk.utils.models import CreanovaModel
...
def get_Creanova_provider_base_url() -> str | None:
    """Return the base URL for the Creanova provider, if configured.

    Falls back to LLM_BASE_URL for backward compatibility.
    """
    return os.getenv('Creanova_PROVIDER_BASE_URL') or os.getenv('LLM_BASE_URL') or None
...
# Creanova provider models use this proxy at the SDK transport boundary.
# Deployments (e.g. staging) may use a different LLM proxy, configured via
# Creanova_PROVIDER_BASE_URL.
...
def resolve_provider_llm_base_url(
    ...
    When the model uses the public ``Creanova/`` prefix and the stored
    ...
    Args:
        model: LLM model name (e.g. ``Creanova/gpt-5.5``).
        base_url: The base URL from user/org settings.
        provider_base_url: Deployment provider URL.  Falls back to
            ``get_Creanova_provider_base_url()`` when *None*.
    """
    if not model or not model.startswith('Creanova/'):
        return base_url
    ...
    if provider_base_url is None:
        provider_base_url = get_Creanova_provider_base_url()
...
def _get_default_lifespan():
    ...
    if 'saas' in (os.getenv('Creanova_CONFIG_CLS') or '').lower():
...
class AppServerConfig(CreanovaModel):
    ...
    web_url: str | None = Field(
        ...
        description='The URL where Creanova is running (e.g., http://localhost:3000)',
    )
    ...
    Creanova_provider_base_url: str | None = Field(
        default_factory=get_Creanova_provider_base_url,
        description='Base URL for the Creanova provider',
    )
    ...
    app_mode: AppMode = AppMode.Creanova
...
def config_from_env() -> AppServerConfig:
    # Import defaults...
    from Creanova.app_server.app_conversation.live_status_app_conversation_service import (  # noqa: E501
        LiveStatusAppConversationServiceInjector,
    )
    from Creanova.app_server.app_conversation.sql_app_conversation_info_service import (  # noqa: E501
        SQLAppConversationInfoServiceInjector,
    )
    from Creanova.app_server.app_conversation.sql_app_conversation_start_task_service import (  # noqa: E501
        SQLAppConversationStartTaskServiceInjector,
    )
    from Creanova.app_server.event.aws_event_service import (
        AwsEventServiceInjector,
    )
    from Creanova.app_server.event.filesystem_event_service import (
        FilesystemEventServiceInjector,
    )
    from Creanova.app_server.event.google_cloud_event_service import (
        GoogleCloudEventServiceInjector,
    )
    from Creanova.app_server.event_callback.sql_event_callback_service import (
        SQLEventCallbackServiceInjector,
    )
    from Creanova.app_server.sandbox.docker_sandbox_service import (
        DockerSandboxServiceInjector,
    )
    from Creanova.app_server.sandbox.docker_sandbox_spec_service import (
        DockerSandboxSpecServiceInjector,
    )
    from Creanova.app_server.sandbox.process_sandbox_service import (
        ProcessSandboxServiceInjector,
    )
    from Creanova.app_server.sandbox.process_sandbox_spec_service import (
        ProcessSandboxSpecServiceInjector,
    )
    from Creanova.app_server.sandbox.remote_sandbox_service import (
        RemoteSandboxServiceInjector,
    )
    from Creanova.app_server.sandbox.remote_sandbox_spec_service import (
        RemoteSandboxSpecServiceInjector,
    )
    from Creanova.app_server.user.auth_user_context import (
    ...
    if config.llm_model is None:
        from Creanova.app_server.config_api.default_llm_model_service import (
