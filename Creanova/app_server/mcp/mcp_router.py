...
from Creanova.app_server.integrations.azure_devops.azure_devops_service import (
    AzureDevOpsServiceImpl,
)
from Creanova.app_server.integrations.bitbucket.bitbucket_service import (
    BitBucketServiceImpl,
)
from Creanova.app_server.integrations.bitbucket_data_center.bitbucket_dc_service import (
    BitbucketDCServiceImpl,
)
from Creanova.app_server.integrations.github.github_service import GithubServiceImpl
from Creanova.app_server.integrations.gitlab.gitlab_service import GitLabServiceImpl
from Creanova.app_server.integrations.provider import ProviderToken
from Creanova.app_server.integrations.service_types import GitService, ProviderType
...
from Creanova.app_server.user_auth import (
    get_access_token,
    get_provider_tokens,
    get_user_id,
)
...
async def get_conversation_link(
    ...
    """Appends a followup link, in the PR body, to the Creanova conversation that opened the PR"""
...
async def create_pr(
    ...
    """Open a PR in GitHub"""
    logger.info('Calling Creanova MCP create_pr')

    request = get_http_request()
    headers = request.headers
    conversation_id = headers.get('X-Creanova-ServerConversation-ID', None)
...
async def create_mr(
    ...
    """Open a MR in GitLab"""
    logger.info('Calling Creanova MCP create_mr')
...
async def create_bitbucket_pr(
    ...
    logger.info('Calling Creanova MCP create_bitbucket_pr')
...
async def create_bitbucket_data_center_pr(
    ...
    logger.info('Calling Creanova MCP create_bitbucket_data_center_pr')
...
async def create_azure_devops_pr(
    ...
    logger.info('Calling Creanova MCP create_azure_devops_pr')
