...
# ---------------------------------------------------------------------------
# The ``Creanova-sdk`` package is the **single source of truth** for which
# models are verified and how bare LiteLLM names map to providers.
#
# Self-hosted mode builds the ``Creanova/…`` model list from the SDK's
# ``VERIFIED_Creanova_MODELS``.  SaaS mode overrides it with the database
# (via ``get_Creanova_models``).
# ---------------------------------------------------------------------------
from Creanova.sdk.llm.utils.verified_models import (  # noqa: E402
    VERIFIED_ANTHROPIC_MODELS as _SDK_ANTHROPIC,
)
from Creanova.sdk.llm.utils.verified_models import (
...
from Creanova.sdk.llm.utils.verified_models import (
    VERIFIED_Creanova_MODELS as _SDK_Creanova,
)

# Build the ``Creanova/…`` model list from the SDK.
Creanova_MODELS: list[str] = [f'Creanova/{m}' for m in _SDK_Creanova]
...
DEFAULT_Creanova_MODEL = 'Creanova/minimax-m2.7'
...
class ModelsResponse(BaseModel):
    ...
    * ``verified_models`` — model names (without provider prefix) that
      Creanova has verified to work well.
...
def is_Creanova_model(model: str | None) -> bool:
    """Return True when the model uses the public Creanova provider prefix."""
    return bool(model and model.startswith('Creanova/'))
...
def resolve_llm_base_url(
    ...
    * ``Creanova/app_server/settings/settings_router._post_merge_llm_fixups``
    ...
    * ``base_url is None`` + known Creanova / managed model → ``managed_proxy_url``.
...
def get_Creanova_models(
    verified_models: list[str] | None = None,
) -> list[str]:
    """Return the list of Creanova-provider model strings.

    In self-hosted mode *verified_models* is ``None`` (or empty) and the
    hardcoded ``Creanova_MODELS`` list is used.  In SaaS mode the caller
    passes the database-backed list which takes precedence.

    Args:
        verified_models: Optional list of ``"Creanova/<name>"`` strings
            loaded from the verified-models database table.

    Returns:
        A list such as ``["Creanova/claude-opus-4-6", ...]``.
    """
    return verified_models if verified_models else Creanova_MODELS
...
def _derive_verified_models(Creanova_models: list[str]) -> list[str]:
    """Extract the bare model names from the ``Creanova/…`` model list."""
    return [
        m.removeprefix('Creanova/')
        for m in Creanova_models
        if m.startswith('Creanova/')
    ]
...
def get_supported_llm_models(
    ...
    * a list of *verified* model names (the Creanova-curated subset),
    ...
    * the recommended default model.
    ...
    Creanova_models = get_Creanova_models(verified_models)

    # Assign canonical provider prefixes to bare LiteLLM names, then dedupe.
    all_models = (
        Creanova_models + CLARIFAI_MODELS + [_assign_provider(m) for m in model_list]
    )
    unique_models = sorted(set(all_models))

    return ModelsResponse(
        models=unique_models,
        verified_models=_derive_verified_models(Creanova_models),
        verified_providers=VERIFIED_PROVIDERS,
        default_model=DEFAULT_Creanova_MODEL,
    )
