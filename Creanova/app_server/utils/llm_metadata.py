"""Utility functions for LLM metadata in Creanova V1 conversations."""
...
def should_set_litellm_extra_body(model_name: str) -> bool:
    ...
    Only set litellm_extra_body for Creanova models to avoid issues
    with providers that don't support extra_body parameters.

    The SDK translates "Creanova/" models to the LiteLLM proxy only when
    ...
    """
    return model_name.startswith('Creanova/')
...
def get_llm_metadata(
    ...
    """Generate LLM metadata for Creanova V1 conversations.
    ...
    """
    Creanova_version = Creanova.__version__

    metadata: dict[str, Any] = {
        'trace_version': Creanova_version,
        'tags': [
            'app:Creanova',
            f'model:{model_name}',
            f'type:{llm_type}',
            f'web_host:{os.environ.get("WEB_HOST", "unspecified")}',
            f'Creanova_version:{Creanova_version}',
