...
### Agent
The core AI entity in Creanova that can perform software development tasks by interacting with tools, browsing the web, and modifying code.
...
#### CodeAct Agent
[A generalist agent in Creanova](https://arxiv.org/abs/2407.16741) designed to perform tasks by editing and executing code.
...
Settings that control various aspects of Creanova behavior, including runtime, security, and agent settings.
...
### Interface
Different ways to interact with Creanova.

#### CLI Mode
A command-line interface mode for interacting with Creanova agents without a graphical interface.

#### GUI Mode
A graphical user interface mode for interacting with Creanova agents through a web interface.

#### Headless Mode
A mode of operation where Creanova runs without a user interface, suitable for automation and scripting.
...
### Microagent
A specialized prompt that enhances Creanova with domain-specific knowledge, repository-specific context, and task-specific workflows.

#### Microagent Registry
A central repository of available microagents and their configurations.

#### Public Microagent
A general-purpose microagent available to all Creanova users, triggered by specific keywords. Located in `skills/`.

#### Repository Microagent
A type of microagent that provides repository-specific context and guidelines, stored in the `.Creanova/microagents/` directory.
