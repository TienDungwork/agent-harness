import { MessageEvent, CreanovaEvent } from "#/types/v1/core";
...
const findLastUserMessageIndex = (events: CreanovaEvent[]): number => {
 ...
 */
const findNewestDurableTimestamp = (events: CreanovaEvent[]): string | null => {
...
const getFinalAgentText = (event: CreanovaEvent): string | null => {
...
const finalizeStreamingDeltasInPlace = (
  finalEvent: CreanovaEvent,
  uiEvents: CreanovaEvent[],
): CreanovaEvent[] | null => {
...
const supersedeStreamingDeltasForAction = (
  ...
  const nextUiEvents: CreanovaEvent[] = [];
