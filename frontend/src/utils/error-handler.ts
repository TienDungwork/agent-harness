...
/**
 * Checks if an error message indicates an Creanova budget or credit limit issue.
 * Provider-side errors can also mention "credits"; those should stay as raw
 * provider errors instead of being rewritten as Creanova billing failures.
 */
export function isBudgetOrCreditError(errorMessage: string): boolean {
  ...
  const isCreanovaCreditError =
    /Creanova\s+credits?/.test(lowerCaseError) ||
    lowerCaseError.includes("credit limit reached");

  return isBudgetLimitError || isCreanovaCreditError;
}
