 ...
 * Synced with agent-canvas and the Creanova backend. Constrained to ^[a-z0-9]+$
 ...
 * (Creanova logo, Claude/OpenAI/Gemini mark), and the text is the model label.
 * ACP models use the curated label from the provider registry (falling back to
 * the raw id); native Creanova models use ``formatLlmModel``. For ACP
