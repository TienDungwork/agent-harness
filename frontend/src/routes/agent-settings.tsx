...
export default function AgentSettingsScreen() {
  ...
  // ── Parallel tool calls (Creanova mode) ─────────────────────────────────
  ...
  // ── ACP (ACP mode) ───────────────────────────────────────────────────────
  const [agentType, setAgentType] = useState<"Creanova" | "acp">("Creanova");
  ...
  const handleSave = async () => {
    ...
    if (isAcp) {
      ...
      agentSettingsDiff = { agent_kind: "Creanova" };
    } else {
  ...
  return (
    <div data-testid="agent-settings-screen" className="h-full relative">
      <div className="flex flex-col gap-8 pb-20">
          ...
          <section className="grid gap-4 xl:grid-cols-2">
            <SettingsDropdownInput
              ...
              onSelectionChange={(key) => {
                if (!key) return;
                const newType = key as "Creanova" | "acp";
        ...
        {/* Creanova: parallel tool calls — only when the schema exposes it */}
