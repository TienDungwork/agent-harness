...
import { CreanovaApiKeyHelp } from "#/components/features/settings/Creanova-api-key-help";
import {
...
const isProviderDefaultBaseUrl = (model: string, baseUrl: string) => {
  // For Creanova/* models, base_url is server-owned (auto-filled on save) —
  // never treat it as user customization that warrants the advanced view.
  // Accepted edge case: a deliberately customized base_url on an Creanova/*
  // model also lands on basic; values are preserved, Advanced is a click away.
  if (model.startsWith("Creanova/")) {
    return true;
  }
...
export function LlmSettingsScreen({
    ...
    ({ values, isDisabled, view, onChange }: SdkSectionHeaderProps) => {
      ...
      const activeProvider =
        view === "basic"
          ...
          : derivedProvider;
      const shouldUseCreanovaKey = isSaasMode && activeProvider === "Creanova";
      // The Creanova key help links to Creanova Cloud keys/pricing — only
      // meaningful on enterprise cloud, not self-hosted managed installs.
      const showCreanovaApiKeyHelp =
        isEnterpriseCloud &&
        modelValue.startsWith("Creanova/") &&
        allowUserLlmConfiguration;
        ...
        <div className="flex flex-col gap-6">
          ...
          {view === "basic" || !allowUserLlmConfiguration ? (
            ...
            >
              ...
              {showCreanovaApiKeyHelp ? (
                <CreanovaApiKeyHelp testId="Creanova-api-key-help" />
              ) : null}
            ...
            >
              ...
              {showCreanovaApiKeyHelp ? (
                <CreanovaApiKeyHelp testId="Creanova-api-key-help-2" />
              ) : null}
  ...
  const buildPayload = React.useCallback(
    (
      ...
      if (shouldUseCreanovaKey && llm.model !== undefined) {
        llm.api_key = "";
      ...
      if (profileFormMode === "edit") {
        ...
        if (shouldUseCreanovaKey && llm.api_key === undefined) {
          llm.api_key = "";
        }
        agentSettings.llm = llm;
      }
      ...
      // Creanova-managed saves force api_key to "" above — that's not a
      ...
      lastSavedApiKeyTypedRef.current =
        !shouldUseCreanovaKey && typedApiKey.length > 0;
        ...
        settingsSources={[
          {
            ...
            // The "llm" key exists under both the Creanova and acp union
            // variants; target Creanova so the acp duplicate is dropped.
            variant: "Creanova",
          },
        ]}
        header={buildHeader}
