...
export class SecretsService {
  ...
  static async searchSecrets(
    ...
    const { data } = await Creanova.get<CustomSecretPage>(url);
    return data;
  }
  ...
  static async createSecret(name: string, value: string, description?: string) {
    ...
    const { status } = await Creanova.post("/api/v1/secrets", secret);
    return status === 201;
  }
  ...
  static async updateSecret(id: string, name: string, description?: string) {
    ...
    const { status } = await Creanova.put(`/api/v1/secrets/${id}`, secret);
    return status === 200;
  }

  static async deleteSecret(id: string) {
    const { status } = await Creanova.delete<boolean>(`/api/v1/secrets/${id}`);
    return status === 200;
  }
  ...
  static async addGitProvider(providers: Record<Provider, ProviderToken>) {
    ...
    const { data } = await Creanova.post<boolean>(
  ...
  static async deleteGitProviders() {
    const { status } = await Creanova.delete("/api/v1/secrets/git-providers");
    return status === 200;
  }
}
