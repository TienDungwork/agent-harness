...
class ConversationService {
  ...
  static async getVSCodeUrl(
    ...
    const { data } = await Creanova.get<GetVSCodeUrlResponse>(url, {
  ...
  static async getTrajectory(
    ...
    const { data } = await Creanova.get<GetTrajectoryResponse>(url, {
      headers: this.getConversationHeaders(),
    });
    return data;
  }
  ...
  static async uploadFiles(
    ...
    const response = await Creanova.post<FileUploadSuccessResponse>(
      url,
      formData,
      {
        headers: {
