...
Services are the abstraction layer between frontend components and backend APIs. They encapsulate HTTP requests using the shared `Creanova` axios instance and provide typed methods for each endpoint.
...
export const featureService = {
  getFeature: async ({ id }: { id: string }) => {
    const { data } = await Creanova.get<Feature>(`/api/features/${id}`);
    return data;
  },

  createFeature: async ({ name, description }: CreateFeatureParams) => {
    const { data } = await Creanova.post<Feature>("/api/features", {
      name,
      description,
    });
    return data;
