...
class GitService {
  ...
  static async searchGitRepositories(
    ...
    const { data } = await Creanova.get<RepositoryPage>(
  ...
  static async getRepositoryBranches(
    ...
    const { data } = await Creanova.get<BranchPage>(
  ...
  static async getUserInstallations(
    ...
    const { data } = await Creanova.get<InstallationPage>(
  ...
  static async getGitChanges(conversationId: string): Promise<GitChange[]> {
    ...
    const { data } = await Creanova.get<GitChange[]>(url, {
  ...
  static async getGitChangeDiff(
    ...
    const { data } = await Creanova.get<GitChangeDiff>(url, {
      params: { path },
      headers: ConversationService.getConversationHeaders(),
    });
    return data;
