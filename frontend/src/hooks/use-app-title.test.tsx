...
describe("useAppTitle", () => {
  ...
  it("should return 'Creanova' if is OSS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() => expect(result.current).toBe("Creanova"));
  });

  it("should return 'Creanova Cloud' if is SaaS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    getConfigSpy.mockResolvedValue({
      app_mode: "saas",
    });
    ...
    await waitFor(() => expect(result.current).toBe("Creanova Cloud"));
  });

  it("should return '{some title} | Creanova' if is OSS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe("My Conversation | Creanova"),
    );
  });

  it("should return '{some title} | Creanova Cloud' if is SaaS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe(
        "Another Conversation Title | Creanova Cloud",
      ),
    );
  });
