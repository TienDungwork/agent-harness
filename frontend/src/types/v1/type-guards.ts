...
import type { CreanovaParsedEvent } from "../core/index";
 ...
 */
export const isMessageEvent = (event: CreanovaEvent): event is MessageEvent =>
 ...
 */
export const isActionEvent = (event: CreanovaEvent): event is ActionEvent =>
  event.source === "agent" &&
  "action" in event &&
  event.action !== null &&
 ...
 */
export const isDisplayableErrorEvent = (event: CreanovaEvent): boolean =>
...
/**
 * TEMPORARY: Type guard to check if an event is a V1 CreanovaEvent
...
export function isV1Event(
  event: CreanovaEvent | CreanovaParsedEvent,
): event is CreanovaEvent {
...
/**
 * TEMPORARY: Type guard to check if an event is a V0 CreanovaParsedEvent
 *
 * @deprecated This is temporary until full V1 migration is complete
 */
export function isV0Event(
  event: CreanovaEvent | CreanovaParsedEvent,
): event is CreanovaParsedEvent {
