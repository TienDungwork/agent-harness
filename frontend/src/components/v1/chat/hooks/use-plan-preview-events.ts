 ...
 */
function groupEventsByPhase(events: CreanovaEvent[]): CreanovaEvent[][] {
  const phases: CreanovaEvent[][] = [];
  let currentPhase: CreanovaEvent[] = [];
 ...
 */
export function usePlanPreviewEvents(allEvents: CreanovaEvent[]): Set<string> {
  return useMemo(() => {
