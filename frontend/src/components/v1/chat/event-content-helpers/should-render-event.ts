import { CreanovaParsedEvent } from "#/types/core";
import { CreanovaEvent } from "#/types/v1/core";
import {
  isActionEvent,
  isObservationEvent,
  isMessageEvent,
...
export const shouldRenderEvent = (event: CreanovaEvent) => {
...
export const hasUserEvent = (events: CreanovaEvent[]) =>
...
export const getRenderedV1Events = (
  events: ReadonlyArray<CreanovaEvent | CreanovaParsedEvent>,
): CreanovaEvent[] => events.filter(isV1Event).filter(shouldRenderEvent);
