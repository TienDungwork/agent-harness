...
describe("resolveAgentChip", () => {
  describe("Creanova branch", () => {
    it("returns kind=Creanova with prettified text and raw tooltip", () => {
    ...
    it("treats undefined agent_kind as the Creanova branch", () => {
      const chip = resolveAgentChip(undefined, "openai/gpt-4o");
      expect(chip?.kind).toBe("Creanova");
      expect(chip?.text).toBe("GPT-4o");
    });

    it("returns null when no llm_model is set", () => {
      expect(resolveAgentChip("Creanova", null)).toBeNull();
      expect(resolveAgentChip("Creanova", undefined)).toBeNull();
