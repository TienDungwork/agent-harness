...
describe("AgentSettingsScreen — minimal generic ACP UX", () => {
  ...
  it("clears acp_* fields when switching back to Creanova", async () => {
    vi.spyOn(OptionService, "getConfig").mockResolvedValue(baseConfig);
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue({
      ...MOCK_DEFAULT_USER_SETTINGS,
      agent_settings: {
    ...
    expect(saveSpy.mock.calls[0][0]).toEqual({
      agent_settings_diff: { agent_kind: "Creanova" },
    });
  });
  ...
  const CreanovaSettingsWithConcurrency = (toolConcurrencyLimit: number) => ({
    ...MOCK_DEFAULT_USER_SETTINGS,
    agent_settings: {
  ...
  it("saves a coerced tool_concurrency_limit on the Creanova path", async () => {
    vi.spyOn(OptionService, "getConfig").mockResolvedValue(baseConfig);
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      CreanovaSettingsWithConcurrency(1),
    );
    const saveSpy = vi
      .spyOn(SettingsService, "saveSettings")
      .mockResolvedValue(true);
