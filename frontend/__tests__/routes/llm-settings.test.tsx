...
async function selectProvider(providerLabel: "Creanova" | "OpenAI") {
...
describe("LlmSettingsScreen", () => {
  ...
  it("defaults to basic view when an Creanova managed model has no base URL", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettingsWithAdvancedToggle({
        llm_model: "Creanova/claude-opus-4-5-20251101",
        llm_base_url: "",
        agent_settings: {
          llm: {
            model: "Creanova/claude-opus-4-5-20251101",
          },
        },
      }),
    );
  ...
  it("opens basic view when an Creanova managed model has a server-filled base URL", async () => {
    // Managed Creanova/* profiles always come back with a base_url — the
    ...
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettingsWithAdvancedToggle({
        llm_model: "Creanova/claude-opus-4-5-20251101",
        llm_base_url: "http://Creanova-litellm:4000",
        agent_settings: {
          llm: {
            model: "Creanova/claude-opus-4-5-20251101",
            base_url: "http://Creanova-litellm:4000",
          },
        },
      }),
    );
  ...
  it("opens basic view when editing an org-scope Creanova managed profile with a server-filled base URL", async () => {
  ...
  it("opens basic view even when an Creanova model has a non-managed base URL", async () => {
    // Accepted edge case: the server owns base_url for Creanova/* models,
  ...
  it("treats a litellm_proxy model with the managed proxy URL as an explicit custom endpoint", async () => {
    ...
    expect(
      screen.queryByTestId("Creanova-api-key-help-2"),
    ).not.toBeInTheDocument();
  });
  ...
  it("uses the docs.Creanova.dev domain for the API key help link", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettings({
        llm_model: "openai/gpt-4o",
        agent_settings: {
    ...
    expect(helpLink.querySelector("a")).toHaveAttribute(
      "href",
      "https://docs.Creanova.dev/usage/local-setup#getting-an-api-key",
    );
  });
  ...
  it("hides the API key input for Creanova provider on enterprise cloud", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(buildSettings());
    ...
    expect(screen.getByTestId("Creanova-api-key-help")).toBeInTheDocument();
  });

  it("hides the Creanova API key help on self-hosted OHE", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(buildSettings());
    ...
    await screen.findByTestId("llm-settings-screen");
    expect(
      screen.queryByTestId("Creanova-api-key-help"),
    ).not.toBeInTheDocument();
  });

  it("shows the API key input for non-Creanova providers in SaaS mode", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettings({
        llm_model: "openai/gpt-4o",
  ...
  it("keeps an existing custom base URL when saving basic view without a model change", async () => {
    // Use a non-managed model: Creanova/* models treat any base_url as
  ...
  it("does not reveal all-only fields after save when refetch returns an Creanova managed model", async () => {
    const schema = structuredClone(
      MOCK_DEFAULT_USER_SETTINGS.agent_settings_schema!,
    );
    const llmSection = schema.sections.find((section) => section.key === "llm");
  ...
  describe("API key visibility in Basic Settings", () => {
    it("should hide API key input when SaaS mode is enabled and Creanova provider is selected", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings(),
      );
      ...
      await waitFor(() => {
        expect(providerInput).toHaveValue("Creanova");
      });
    ...
    it("should show API key input when SaaS mode is enabled and non-Creanova provider is selected", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings({
          llm_model: "openai/gpt-4o",
          agent_settings: { llm: { model: "openai/gpt-4o" } },
    ...
    it("should show API key input when OSS mode is enabled and Creanova provider is selected", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings(),
      );
    ...
    it("should show API key input when OSS mode is enabled and non-Creanova provider is selected", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings({
          llm_model: "openai/gpt-4o",
          agent_settings: { llm: { model: "openai/gpt-4o" } },
    ...
    it("should hide API key input when switching from non-Creanova to Creanova provider in SaaS mode", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings({
          llm_model: "openai/gpt-4o",
          agent_settings: { llm: { model: "openai/gpt-4o" } },
    ...
    it("should show API key input when switching from Creanova to non-Creanova provider in SaaS mode", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings(),
      );
      ...
      it("shows org profiles and management controls for admins", async () => {
        vi.mocked(OrgProfilesService.listProfiles).mockResolvedValue({
          profiles: [
            {
              name: "sonnet",
              model: "Creanova/claude-sonnet-4-5-20250929",
              base_url: null,
              api_key_set: true,
            },
          ],
  ...
  describe("Add Profile (create flow)", () => {
    ...
    it("does not preselect the active provider when creating a profile in SaaS mode", async () => {
      // Default settings use an Creanova model; previously the create form
    ...
    it("only shows the API key input once a key-taking provider is selected", async () => {
      ...
      // The managed Creanova provider keeps it hidden in SaaS mode (keys
    ...
    it("saves and activates a profile built from the blank create form", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings({
          llm_model: "Creanova/claude-opus-4-5-20251101",
          agent_settings: {
            llm: { model: "Creanova/claude-opus-4-5-20251101" },
          },
        }),
      );
      const saveSettingsSpy = vi
    ...
    it("hydrates the edit form with the canonical model when the profile stores a hidden alias", async () => {
      // Legacy profile model "Creanova/custom-llm" is a hidden alias that
