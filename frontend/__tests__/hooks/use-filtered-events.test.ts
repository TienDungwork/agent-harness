...
import type { CreanovaAction } from "#/types/core/actions";
import type { ActionEvent, MessageEvent } from "#/types/v1/core";
import { SecurityRisk } from "#/types/v1/core";

// --- V0 event factories ---

function createV0UserMessage(id: number): CreanovaAction {
  return {
    id,
    source: "user",
    action: "message",
...
function createV0AgentMessage(id: number): CreanovaAction {
  return {
    id,
    source: "agent",
    action: "message",
...
function createV0SystemEvent(id: number): CreanovaAction {
  return {
    id,
    source: "environment",
    action: "system",
    args: {
      content: "source .Creanova/setup.sh",
      tools: null,
      Creanova_version: null,
      agent_class: null,
    },
