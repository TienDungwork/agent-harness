...
function setupMocks({
  userOrgs = { provider: "github", organizations: ["Creanova", "AcmeCo"] },
  claims = [] as Array<{
    id: string;
    org_id: string;
    provider: string;
...
describe("useGitConversationRouting", () => {
  ...
  it("merges user git orgs with claims correctly", () => {
    ...
    const claimedOrg = result.current.orgs.find((o) => o.name === "Creanova");
  ...
  it("sets pending disconnecting state when disconnectOrg is called", () => {
    ...
    act(() => {
      result.current.disconnectOrg("github:Creanova");
    });
    ...
    const org = result.current.orgs.find((o) => o.id === "github:Creanova");
  ...
  it("does not claim an already claimed org", () => {
    ...
    act(() => {
      result.current.claimOrg("github:Creanova");
    });
