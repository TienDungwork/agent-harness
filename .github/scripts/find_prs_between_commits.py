#!/usr/bin/env python3
"""
Find all PRs that went in between two commits in the Creanova/Creanova repository.
Handles cherry-picks and different merge strategies.

This script is designed to run from within the Creanova repository under .github/scripts:
...
def find_Creanova_repo() -> Optional[Path]:
    """
    Find the Creanova repository.
    ...
    2. Current directory
    3. Environment variable Creanova_REPO
    """
    ...
    if 'Creanova_REPO' in os.environ:
        repo_path = Path(os.environ['Creanova_REPO'])
...
def main():
    if len(sys.argv) < 3:
        ...
        print('  --repo <path>   Path to Creanova repository (default: auto-detect)')
        print()
        print('Example:')
        print(
        ...
        print('  The script will try to find the Creanova repository in this order:')
        print('  1. --repo argument')
        print('  2. Repository root (../../ from script location)')
        print('  3. Current directory')
        print('  4. Creanova_REPO environment variable')
        print()
        print('Environment variables:')
        print(
            '  GITHUB_TOKEN    Optional. If set, will fetch additional PR info from GitHub API'
        )
        print('  Creanova_REPO  Optional. Path to Creanova repository')
        sys.exit(1)
    ...
    if repo_path is None:
        repo_path = find_Creanova_repo()
        if repo_path is None:
            print('Error: Could not find Creanova repository', file=sys.stderr)
            print('Please either:', file=sys.stderr)
            print(
                '  1. Place this script in .github/scripts/ within the Creanova repository',
                file=sys.stderr,
            )
            print('  2. Run from the Creanova repository directory', file=sys.stderr)
            print(
                '  3. Use --repo <path> to specify the repository location',
                file=sys.stderr,
            )
            print('  4. Set Creanova_REPO environment variable', file=sys.stderr)
            sys.exit(1)
