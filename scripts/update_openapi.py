#!/usr/bin/env python3
"""
Update Creanova OpenAPI documentation.
...
Behavior:
- Uses Creanova.app_server.app.app.openapi() to build the spec.
- Preserves existing "servers" from docs/openapi.json if present; otherwise
  writes sensible defaults.
- Sets info.version to Creanova.__version__.
...
os.environ['Creanova_LOG_LEVEL'] = 'CRITICAL'
...
try:
    from Creanova import __version__
    from Creanova.app_server.app import app
except ImportError as e:
    print(f'Error importing Creanova modules: {e}')
    print(
