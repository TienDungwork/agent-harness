...
from Creanova.app_server.app import app
from Creanova.app_server.file_store.memory import InMemoryFileStore
from Creanova.app_server.integrations.provider import ProviderToken, ProviderType
from Creanova.app_server.integrations.service_types import UserGitInfo
from Creanova.app_server.secrets.secrets_models import Secrets
from Creanova.app_server.secrets.secrets_store import SecretsStore
from Creanova.app_server.settings.file_settings_store import FileSettingsStore
...
from Creanova.app_server.user.skills_router import (
    GLOBAL_SKILLS_DIR,
...
def test_client():
    with (
        ...
        patch('Creanova.app_server.utils.dependencies._SESSION_API_KEY', None),
        patch(
            'Creanova.app_server.user_auth.user_auth.UserAuth.get_instance',
            return_value=MockUserAuth(),
        ),
        patch(
            'Creanova.app_server.settings.file_settings_store.FileSettingsStore.get_instance',
...
async def test_skills_search_returns_skills(test_client, tmp_path):
    ...
    with (
        patch('Creanova.app_server.user.skills_router.GLOBAL_SKILLS_DIR', global_dir),
        patch(
            'Creanova.app_server.user.skills_router.USER_SKILLS_DIR',
...
async def test_skills_search_handles_missing_dirs(test_client, tmp_path):
    ...
    with (
        patch(
            'Creanova.app_server.user.skills_router.GLOBAL_SKILLS_DIR',
...
async def test_skills_search_sorted_by_source_then_name(test_client, tmp_path):
    ...
    with (
        patch('Creanova.app_server.user.skills_router.GLOBAL_SKILLS_DIR', global_dir),
        patch('Creanova.app_server.user.skills_router.USER_SKILLS_DIR', user_dir),
    ):
        response = test_client.get('/api/v1/skills/search')
...
class TestMarketplaceSkillsCloneFailures:
    ...
    async def test_clone_invalid_repo_path(self):
        ...
        with patch(
            'Creanova.app_server.user.skills_router._parse_marketplace_source',
            return_value=('github', ''),
        ):
    ...
    async def test_clone_network_timeout(self):
        ...
        with patch('Creanova.app_server.user.skills_router.subprocess.run') as mock_run:
    ...
    async def test_clone_with_ref_checkout_failure(self):
        ...
        with patch('Creanova.app_server.user.skills_router.subprocess.run') as mock_run:
            with patch('Creanova.app_server.user.skills_router._cleanup_clone_dir'):
    ...
    async def test_clone_repo_path_not_found(self):
        ...
        with patch('Creanova.app_server.user.skills_router.subprocess.run') as mock_run:
            with patch(
                'Creanova.app_server.user.skills_router.Path.exists'
            ) as mock_exists:
                with patch('Creanova.app_server.user.skills_router._cleanup_clone_dir'):
                    with patch(
    ...
    async def test_clone_bbdc_personal_url_uses_pinned_authenticated_url(self):
        ...
        with patch(
            'Creanova.app_server.user.skills_router.subprocess.run',
    ...
    async def test_clone_public_github_url_works_with_non_github_provider(self):
        ...
        marketplace = MarketplaceRegistration(
            name='public-gh',
            source='https://github.com/Creanova/skills.git',
        )
        ...
        clone_argv = mock_run.call_args[0][0]
        assert clone_argv[3] == 'https://github.com/Creanova/skills.git'
