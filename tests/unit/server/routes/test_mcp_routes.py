...
from Creanova.app_server.integrations.service_types import GitService
from Creanova.app_server.mcp.mcp_router import get_conversation_link, init_tavily_proxy
...
def test_mcp_server_no_stateless_http_deprecation_warning():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        from Creanova.app_server.mcp.mcp_router import mcp_server
...
async def test_get_conversation_link_non_saas_mode():
    ...
    with patch('Creanova.app_server.mcp.mcp_router.get_global_config') as mock_config:
        mock_config.return_value.app_mode = AppMode.Creanova
...
async def test_get_conversation_link_saas_mode():
    ...
    # Test with SAAS mode
    with (
        patch('Creanova.app_server.mcp.mcp_router.get_global_config') as mock_config,
        patch(
            'Creanova.app_server.mcp.mcp_router.CONVERSATION_URL',
...
class TestInitTavilyProxy:
    ...
    def test_init_tavily_proxy_no_api_key(self):
        ...
        with (
            patch(
                'Creanova.app_server.mcp.mcp_router.get_global_config'
            ) as mock_config,
            patch('Creanova.app_server.mcp.mcp_router.logger') as mock_logger,
            patch('Creanova.app_server.mcp.mcp_router.Client') as mock_client,
            patch('Creanova.app_server.mcp.mcp_router.create_proxy') as mock_proxy,
            patch('Creanova.app_server.mcp.mcp_router.mcp_server') as mock_mcp_server,
        ):
    ...
    def test_init_tavily_proxy_with_api_key(self):
        ...
        with (
            ...
            patch('Creanova.app_server.mcp.mcp_router.Client') as mock_client,
            patch(
                'Creanova.app_server.mcp.mcp_router.StreamableHttpTransport'
            ) as mock_transport,
            patch(
                'Creanova.app_server.mcp.mcp_router.create_proxy'
    ...
    def test_init_tavily_proxy_handles_exception(self):
        ...
        with (
            ...
            patch('Creanova.app_server.mcp.mcp_router.logger') as mock_logger,
            patch('Creanova.app_server.mcp.mcp_router.Client') as mock_client,
            patch('Creanova.app_server.mcp.mcp_router.StreamableHttpTransport'),
            patch('Creanova.app_server.mcp.mcp_router.mcp_server') as mock_mcp_server,
        ):
            # Configure API key
            mock_config.return_value.tavily_api_key = 'test-tavily-key'
