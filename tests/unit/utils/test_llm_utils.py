"""Tests for Creanova.app_server.utils.llm module."""

from Creanova.app_server.utils import llm as llm_utils
from Creanova.app_server.utils.llm import (
    _assign_provider,
...
class TestIsCreanovaModel:
    """Tests for the is_Creanova_model function."""

    def test_Creanova_model_returns_true(self):
        """Test that models with 'Creanova/' prefix return True."""
        assert is_Creanova_model('Creanova/claude-sonnet-4-5-20250929') is True
        assert is_Creanova_model('Creanova/gpt-5-2025-08-07') is True
        assert is_Creanova_model('Creanova/gemini-2.5-pro') is True

    def test_non_Creanova_model_returns_false(self):
        """Test that models without 'Creanova/' prefix return False."""
        assert is_Creanova_model('gpt-4') is False
        assert is_Creanova_model('claude-3-opus-20240229') is False
        assert is_Creanova_model('anthropic/claude-3-opus-20240229') is False
        assert is_Creanova_model('openai/gpt-4') is False
        assert is_Creanova_model('litellm_proxy/gpt-4') is False

    def test_none_model_returns_false(self):
        """Test that None model returns False."""
        assert is_Creanova_model(None) is False

    def test_empty_string_returns_false(self):
        """Test that empty string returns False."""
        assert is_Creanova_model('') is False

    def test_similar_prefix_not_matched(self):
        """Test that similar prefixes don't incorrectly match."""
        assert is_Creanova_model('Creanova') is False  # Missing slash
        assert is_Creanova_model('Creanovax/model') is False  # Extra char
        assert is_Creanova_model('Creanova/model') is False  # Wrong case
...
class TestDeriveVerifiedModels:
    ...
    def test_extracts_Creanova_model_names(self):
        """Test that only Creanova-prefixed models are returned bare."""
        models = [
