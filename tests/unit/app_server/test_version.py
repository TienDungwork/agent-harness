...
class TestGetVersionFromPyproject:
    ...
    def test_reads_first_candidate_path(self, tmp_path):
        ...
        fake_file = tmp_path / 'Creanova' / 'app_server' / 'version.py'
        fake_file.parent.mkdir(parents=True)
        fake_file.touch()

        with patch(
            'Creanova.app_server.version.os.path.abspath', return_value=str(fake_file)
        ):
            assert get_version() == '1.2.3'

    def test_reads_second_candidate_path(self, tmp_path):
        # Only place pyproject.toml under the Creanova/ subdirectory
        Creanova_dir = tmp_path / 'Creanova'
        Creanova_dir.mkdir()
        _write_pyproject(Creanova_dir, '4.5.6')
...
class TestGetVersionFallbacks:
    ...
    def _patch_no_pyproject(self):
        ...
        return patch(
            'Creanova.app_server.version.os.path.abspath',
            return_value='/nonexistent/Creanova/app_server/version.py',
        )
...
class TestModuleLevelVersion:
    ...
    def test_module_version_matches_pattern(self):
        from Creanova.app_server.version import __version__
