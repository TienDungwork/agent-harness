...
``Creanova-agent-server`` package, with auto-correction of stale canonical
...
def test_get_agent_server_image_derived_from_package_version():
    """The URL must be built from the installed Creanova-agent-server version,
    ...
    with patch.object(
        ...
        assert get_agent_server_image() == (
            f'ghcr.io/Creanova/agent-server:{fake_version}-python'
        )
...
def test_get_agent_server_image_auto_corrects_stale_canonical_tag():
    ...
    with patch.object(importlib.metadata, 'version', return_value=fake_version):
        with patch.dict(
            'os.environ',
            {
                'AGENT_SERVER_IMAGE_REPOSITORY': 'ghcr.io/Creanova/agent-server',
...
def test_get_agent_server_image_passes_through_canonical_tag_without_suffix():
    ...
    with patch.object(importlib.metadata, 'version', return_value=fake_version):
        with patch.dict(
            ...
            assert get_agent_server_image() == ('ghcr.io/Creanova/agent-server:nightly')
...
def test_get_agent_server_image_propagates_package_not_found():
    """Creanova-agent-server is a hard runtime dependency; a missing install
    must surface as PackageNotFoundError at first call, not silently degrade."""
    with patch.object(
        importlib.metadata,
        'version',
        side_effect=importlib.metadata.PackageNotFoundError('Creanova-agent-server'),
    ):
