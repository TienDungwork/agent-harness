"""Regression tests for ``Creanova.app_server.services.db_session``.
...
exactly the crashloop seen in Creanova/deploy#4315.
"""
...
from Creanova.app_server.services.db_session import (
    _yield_db_session,
    depends_db_session,
)
...
def test_depends_db_session_does_not_resolve_config_at_construction():
    ...
    We install a sentinel ``Creanova.app_server.config`` module whose
    ...
    """
    original = sys.modules.get('Creanova.app_server.config')
    sentinel = types.ModuleType('Creanova.app_server.config')
    ...
    sys.modules['Creanova.app_server.config'] = sentinel
    try:
        ...
        if original is None:
            sys.modules.pop('Creanova.app_server.config', None)
        else:
            sys.modules['Creanova.app_server.config'] = original
