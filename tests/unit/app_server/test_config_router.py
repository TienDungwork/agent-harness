...
from Creanova.app_server.config_api.config_models import LLMModel, Provider
from Creanova.app_server.config_api.config_router import router
...
from Creanova.app_server.utils.paging_utils import encode_page_id, paginate_results
...
class TestToProviders:
    ...
    def test_Creanova_provider_appears_first(self):
        """The ``Creanova`` managed provider must always be first in the list."""
        providers = _to_providers(get_supported_llm_models())

        assert providers, 'expected at least one provider'
        assert providers[0].name == 'Creanova'
