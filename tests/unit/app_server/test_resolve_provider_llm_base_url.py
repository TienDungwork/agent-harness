"""Tests for resolve_provider_llm_base_url in Creanova.app_server.config."""
...
class TestCreanovaPrefixWithProviderUrl:
    """Creanova/ prefix + SDK default URL + provider URL set → returns provider URL."""

    def test_Creanova_prefix_replaces_sdk_default(self):
        result = resolve_provider_llm_base_url(
            model='Creanova/gpt-4',
            base_url=SDK_DEFAULT,
            provider_base_url=STAGING_URL,
        )
        assert result == STAGING_URL

    def test_Creanova_prefix_sdk_default_no_trailing_slash(self):
        result = resolve_provider_llm_base_url(
            model='Creanova/gpt-4',
...
class TestCreanovaPrefixWithCustomUrl:
    """Creanova/ prefix + custom URL (not SDK default) → returns custom URL."""
...
class TestCreanovaPrefixNoProviderUrl:
    """Creanova/ prefix + SDK default URL + no provider URL → returns SDK default."""
    ...
    def test_sdk_default_returned_when_provider_none_and_env_unset(self, monkeypatch):
        monkeypatch.delenv('Creanova_PROVIDER_BASE_URL', raising=False)
...
class TestNonMatchingModelPrefix:
    """Model without Creanova/ prefix → returns base_url unchanged."""
...
class TestEdgeCases:
    ...
    def test_none_base_url_with_Creanova_model_and_provider(self):
        result = resolve_provider_llm_base_url(
            model='Creanova/gpt-4',
            base_url=None,
            provider_base_url=STAGING_URL,
    ...
    def test_none_base_url_with_Creanova_model_no_provider(self, monkeypatch):
        monkeypatch.delenv('Creanova_PROVIDER_BASE_URL', raising=False)
        monkeypatch.delenv('LLM_BASE_URL', raising=False)
        result = resolve_provider_llm_base_url(
            model='Creanova/gpt-4',
    ...
    def test_env_fallback_when_provider_base_url_is_none(self, monkeypatch):
        monkeypatch.setenv('Creanova_PROVIDER_BASE_URL', STAGING_URL)
        result = resolve_provider_llm_base_url(
            model='Creanova/gpt-4',
            base_url=SDK_DEFAULT,
