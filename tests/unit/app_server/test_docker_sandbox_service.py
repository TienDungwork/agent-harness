...
class TestDockerSandboxService:
    ...
    @patch('Creanova.app_server.sandbox.docker_sandbox_service.base62.encodebytes')
    @patch('os.urandom')
    ...
    async def test_delete_sandbox_success(self, service):
        ...
        service.docker_client.volumes.get.assert_called_once_with(
            'Creanova-workspace-oh-test-abc123'
        )
    ...
    @patch(
        'Creanova.app_server.utils.docker_utils.is_running_in_docker',
        return_value=True,
    )
...
class TestDockerSandboxServiceInjectorFromEnv:
    ...
    def test_config_from_env_with_sandbox_host_port(self):
        ...
        with patch.dict(os.environ, env_vars, clear=False):
            ...
            import Creanova.app_server.config as config_module
            from Creanova.app_server.config import config_from_env
...
class TestDockerSandboxServiceHostNetwork:
    ...
    @patch('Creanova.app_server.sandbox.docker_sandbox_service._logger')
    @patch('Creanova.app_server.sandbox.docker_sandbox_service.base62.encodebytes')
    @patch('os.urandom')
    ...
    @patch('Creanova.app_server.sandbox.docker_sandbox_service.utc_now')
