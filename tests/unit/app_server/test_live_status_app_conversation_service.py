...
from Creanova.agent_server.models import (
    SendMessageRequest,
...
from Creanova.app_server.app_conversation.app_conversation_service import (
    ConversationExportAlreadyRunning,
    ConversationExportLockUnavailable,
    ConversationExportTooLarge,
)
from Creanova.app_server.app_conversation.live_status_app_conversation_service import (
    LiveStatusAppConversationService,
    effective_disabled_skills,
)
from Creanova.app_server.integrations.provider import ProviderToken, ProviderType
from Creanova.app_server.integrations.service_types import SuggestedTask, TaskType
from Creanova.app_server.sandbox.sandbox_models import (
    AGENT_SERVER,
    ExposedUrl,
    SandboxInfo,
    SandboxPage,
...
from Creanova.app_server.sandbox.sandbox_spec_models import SandboxSpecInfo
from Creanova.app_server.settings.llm_profiles import LLMProfiles
from Creanova.app_server.settings.settings_models import (
    SandboxGroupingStrategy,
    Settings,
)
from Creanova.app_server.user.user_context import UserContext
from Creanova.app_server.utils.redis_lock import RedisLockUnavailable
from Creanova.sdk import Agent, AgentContext, Event
from Creanova.sdk.llm import LLM
from Creanova.sdk.secret import LookupSecret, StaticSecret
from Creanova.sdk.settings import ConversationSettings, CreanovaAgentSettings
from Creanova.sdk.workspace.remote.async_remote_workspace import AsyncRemoteWorkspace
...
def _build_test_user_agent_settings(user: SimpleNamespace) -> CreanovaAgentSettings:
    llm_vals: dict = {}
...
class _TestUserInfo(SimpleNamespace):
    @property
    def agent_settings(self) -> CreanovaAgentSettings:
    ...
    def to_agent_settings(self) -> CreanovaAgentSettings:
...
class TestEffectiveDisabledSkills:
    ...
    def _user(self, member, profile_disabled):
        ...
        user.agent_settings = CreanovaAgentSettings(
    ...
    def test_member_only_on_non_profile_launch(self):
        ...
        user.agent_settings = CreanovaAgentSettings(agent_context=AgentContext())
...
# Env var used by Creanova SDK LLM to skip context-window validation (e.g. for gpt-4 in tests)
...
class TestLiveStatusAppConversationService:
    ...
    def setup_method(self):
        ...
        self.service = LiveStatusAppConversationService(
            ...
            Creanova_provider_base_url='https://provider.example.com',
    ...
    @pytest.mark.asyncio
    async def test_maybe_refresh_managed_llm_key_skips_Creanova_custom_base_url(
        self, monkeypatch
    ):
        self.service.app_mode = 'saas'
        self.mock_user.id = 'user-123'
    ...
    async def test_configure_llm_and_mcp_with_custom_model(self):
        ...
        assert default_server.headers[
            'X-Creanova-ServerConversation-ID'
    ...
    @pytest.mark.asyncio
    async def test_configure_llm_and_mcp_Creanova_model_uses_user_base_url(
        self,
    ):
        """Creanova/* model uses user's base_url when set."""
        # Arrange
        self.mock_user.llm_model = 'Creanova/special'
        ...
        # Assert — user base_url takes precedence for Creanova/ models
        assert llm.base_url == 'https://user-llm.example.com'

    @pytest.mark.asyncio
    async def test_configure_llm_and_mcp_Creanova_model_falls_back_to_provider_url(
        self,
    ):
        """Creanova/* model falls back to provider base URL when user has no base_url."""
        # Arrange
        self.mock_user.llm_model = 'Creanova/default'
        ...
        # Assert — falls back to service-level Creanova_provider_base_url
        assert llm.base_url == 'https://provider.example.com'

    @pytest.mark.asyncio
    async def test_configure_llm_and_mcp_Creanova_model_no_base_urls(self):
        """Creanova/* model is kept public when no explicit base URL exists."""
        # Arrange
        self.mock_user.llm_model = 'Creanova/default'
        self.mock_user.llm_base_url = None
        self.service.Creanova_provider_base_url = None
        self.mock_user_context.get_mcp_api_key.return_value = None
        ...
        # Assert
        assert llm.model == 'Creanova/default'
    ...
    async def test_configure_llm_and_mcp_litellm_proxy_model_keeps_empty_base_url(
        ...
        """litellm_proxy/* is not treated as an Creanova provider model."""
        self.mock_user.llm_base_url = None
        self.mock_user_context.get_mcp_api_key.return_value = None
    ...
    @pytest.mark.asyncio
    async def test_configure_llm_and_mcp_non_Creanova_model_ignores_provider(self):
        """Non-Creanova model ignores provider base URL and uses user base URL."""
        # Arrange
        self.mock_user.llm_model = 'gpt-4'
        self.mock_user.llm_base_url = 'https://user-llm.example.com'
        self.service.Creanova_provider_base_url = 'https://provider.example.com'
        self.mock_user_context.get_mcp_api_key.return_value = None
    ...
    def test_build_observability_context_includes_repository(self):
        ...
        metadata, tags = self.service._build_observability_context(
            conversation_id,
            agent_kind='Creanova',
            selected_repository='Creanova/software-agent-sdk',
        ...
        assert metadata == {
            ...
            'agent_kind': 'Creanova',
            'repo_name': 'Creanova/software-agent-sdk',
            'selected_branch': 'main',
            'git_provider': 'github',
        }
        assert 'repo:Creanova/software-agent-sdk' in tags
        assert 'branch:main' in tags
        assert 'git_provider:github' in tags

    def test_apply_server_overrides_adds_repo_metadata(self):
        llm = LLM(model='Creanova/gpt-4', api_key='k', usage_id='agent')
        ...
        assert metadata['repo_name'] == 'Creanova/software-agent-sdk'
        assert metadata['git_provider'] == 'github'
        assert metadata['selected_branch'] == 'main'
        assert 'repo:Creanova/software-agent-sdk' in metadata['tags']
        assert 'branch:main' in metadata['tags']
        assert 'git_provider:github' in metadata['tags']

    @patch(
        'Creanova.app_server.app_conversation.live_status_app_conversation_service.get_default_tools',
        return_value=[],
    )
    @pytest.mark.asyncio
    ...
    async def test_build_request_passes_profile_and_member_disabled_skills(
        ...
        self.mock_user.agent_settings = CreanovaAgentSettings(
    ...
    async def test_build_request_skills_loading_fails_gracefully(self, _mock_tools):
        ...
        with patch(
            'Creanova.app_server.app_conversation.live_status_app_conversation_service._logger'
        ) as mock_logger:
            result = await self.service._build_start_conversation_request_for_user(
                sandbox=self.mock_sandbox,
                conversation_id=conversation_id,
    ...
    def test_apply_server_overrides_sets_condenser_usage_id(self):
        ...
        from Creanova.sdk.context.condenser import LLMSummarizingCondenser
    ...
    def test_apply_server_overrides_condenser_non_Creanova_model(self):
        """Condenser usage_id is set even for non-Creanova models (no metadata)."""
        from Creanova.sdk.context.condenser import LLMSummarizingCondenser
        ...
        # Non-Creanova model: main LLM unchanged, but condenser still gets usage_id
        assert updated.condenser.llm.usage_id == 'condenser'
    ...
    async def test_build_request_routes_acp_user_to_observability_metadata(self):
        ...
        """
        from Creanova.sdk.settings import ACPAgentSettings
    ...
    @patch(
        'Creanova.app_server.app_conversation.live_status_app_conversation_service.get_registered_agent_definitions'
    )
    @patch(
        'Creanova.app_server.app_conversation.live_status_app_conversation_service.register_builtins_agents'
    )
    @patch(
        'Creanova.app_server.app_conversation.live_status_app_conversation_service.get_default_tools',
        return_value=[],
    ...
    async def test_build_request_passes_enable_sub_agents_true(
        ...
        from Creanova.sdk.settings import CreanovaAgentSettings
        from Creanova.sdk.subagent.schema import AgentDefinition
        ...
        agent_settings = CreanovaAgentSettings(
    ...
    async def test_open_conversation_export_streams_with_lock(self):
        ...
        with patch(
            'Creanova.app_server.app_conversation.live_status_app_conversation_service.try_acquire_redis_lock',
    ...
    @patch(
        'Creanova.app_server.app_conversation.live_status_app_conversation_service.AsyncRemoteWorkspace'
    )
    @patch(
        'Creanova.app_server.app_conversation.live_status_app_conversation_service.ConversationInfo'
    )
    ...
    async def test_start_app_conversation_preserves_acp_and_repository_tags(
        ...
        request = AppConversationStartRequest(
            selected_repository='Creanova/Creanova',
            selected_branch='main',
            git_provider=ProviderType.GITHUB,
        )
        ...
        assert saved_info.tags['repo_name'] == 'Creanova/Creanova'
    ...
    async def test_configure_llm_and_mcp_returns_flat_server_map(self):
        ...
        """
        from Creanova.sdk.mcp.config import MCPServer
    ...
    def test_get_project_dir_with_repo(self):
        ...
        from Creanova.app_server.app_conversation.app_conversation_service_base import (
            get_project_dir,
        )

        assert (
            get_project_dir('/workspace/project', 'Creanova/software-agent-sdk')
    ...
    async def test_build_request_workspace_uses_project_dir(self, _mock_tools):
        ...
        .Creanova/hooks/on_stop.sh is not found.
        """
        self.mock_user_context.get_user_info.return_value = self.mock_user
...
class TestPluginHandling:
    ...
    def test_construct_initial_message_with_plugin_params_no_plugins(self):
        ...
        from Creanova.agent_server.models import SendMessageRequest, TextContent
    ...
    def test_construct_initial_message_with_plugin_params_creates_new_message(self):
        ...
        from Creanova.agent_server.models import TextContent
        from Creanova.app_server.app_conversation.app_conversation_models import (
            PluginSpec,
        )
...
class TestLoadHooksFromWorkspace:
    ...
    def test_get_project_dir_for_hooks_with_selected_repository(self):
        ...
        from Creanova.app_server.app_conversation.hook_loader import (
            get_project_dir_for_hooks,
        )

        result = get_project_dir_for_hooks(
            '/workspace/project',
            'Creanova/software-agent-sdk',
        )
...
class TestAgentKindConversationUrl:
    ...
    @pytest.mark.parametrize('agent_kind', ['Creanova', 'acp'])
...
class TestBuildAcpStartConversationRequestSecrets:
    ...
    def service(self):
        ...
        return LiveStatusAppConversationService(
            ...
            httpx_client=Mock(),
            web_url=None,
            Creanova_provider_base_url=None,
            access_token_hard_timeout=None,
            app_mode='test',
        )
    ...
    async def test_observability_metadata_populated(self, service, tmp_path):
        """Repo / branch / provider land on the request's observability_metadata
        for ACP conversations too, matching the Creanova-agent path."""
        user = self._make_acp_user()
    ...
    async def test_observability_metadata_includes_commit(self, service, tmp_path):
        """The post-clone HEAD sha is resolved from the workspace and added,
        mirroring the Creanova-agent path's commit resolution."""
        user = self._make_acp_user()
        remote_workspace = Mock(spec=AsyncRemoteWorkspace)
        remote_workspace.execute_command = AsyncMock(
            return_value=SimpleNamespace(exit_code=0, stdout='abc123sha\n')
