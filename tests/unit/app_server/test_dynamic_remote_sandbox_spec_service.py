...
from Creanova.app_server.sandbox.dynamic_remote_sandbox_spec_service import (
...
_CONFIGS_RESPONSE = {
    'configs': [
        {
            'name': 'v1_current',
            'image': 'ghcr.io/Creanova/agent-server:1.0.0',
            'command': ['--port', '8000'],
            'environment': {'FOO': 'bar'},
        ...
        {
            'name': 'v1_legacy',
            'image': 'ghcr.io/Creanova/agent-server:0.9.0',
            'command': ['--port', '9000'],
            'environment': {},
        ...
        {
            'name': 'nightly',
            'image': 'ghcr.io/Creanova/agent-server:nightly',
            'command': None,
            'environment': {'DEBUG': '1'},
            'working_dir': '/workspace',
        },
...
class TestFetchSpecs:
    ...
    async def test_maps_fields_to_sandbox_spec_info(self):
        ...
        assert len(specs) == 3
        first = specs[0]
        assert first.id == 'ghcr.io/Creanova/agent-server:1.0.0'
    ...
    async def test_populates_name_to_spec_mapping(self):
        ...
        assert (
            ...
            == 'ghcr.io/Creanova/agent-server:nightly'
        )
    ...
    async def test_empty_configs_fallback_uses_bundled_default_not_env_override(
        ...
        expected_image = 'ghcr.io/Creanova/agent-server:9.9.9-python'

        ctx, _ = _make_async_client_mock(_make_http_response({'configs': []}))
        service = _make_service()

        with patch(
            'Creanova.app_server.sandbox.remote_sandbox_spec_service.get_agent_server_image',
...
class TestSearchSandboxSpecs:
    ...
    async def test_respects_limit_and_sets_next_page_id(self):
        ...
        assert page.items[0].id == 'ghcr.io/Creanova/agent-server:1.0.0'
        assert page.items[1].id == 'ghcr.io/Creanova/agent-server:0.9.0'
    ...
    async def test_page_id_offsets_start_index(self):
        ...
        assert page.items[0].id == 'ghcr.io/Creanova/agent-server:nightly'
        assert page.next_page_id is None
...
class TestGetSandboxSpec:
    async def test_returns_spec_by_image_id(self):
        ...
        with patch('httpx.AsyncClient', return_value=ctx):
            spec = await service.get_sandbox_spec('ghcr.io/Creanova/agent-server:0.9.0')

        assert spec is not None
        assert spec.id == 'ghcr.io/Creanova/agent-server:0.9.0'
...
class TestGetDefaultSandboxSpec:
    ...
    async def test_falls_back_to_first_spec_when_name_not_found(self):
        ...
        assert spec.id == 'ghcr.io/Creanova/agent-server:1.0.0'
