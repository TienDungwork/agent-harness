...
The discriminated ``CreanovaAgentSettings | ACPAgentSettings`` union means a
...
from Creanova.sdk.settings.model import AGENT_SETTINGS_SCHEMA_VERSION
...
def test_kind_switch_does_not_raise():
    ...
    """
    s = Settings()
    s.update(_set_Creanova(llm_model='anthropic/claude-sonnet-4-5'))

    s.update(_set_acp())
    assert s.agent_settings.agent_kind == 'acp'

    s.update(_set_Creanova())
    assert s.agent_settings.agent_kind == 'Creanova'
...
def test_kind_switch_with_inline_field_override():
    ...
    s.update(_set_Creanova(llm_model='model-c'))
    assert s.agent_settings.agent_kind == 'Creanova'
...
def test_replace_mcp_config_in_kind_switch():
    ...
    s.update(_set_Creanova(mcp_config={'mcpServers': {'foo': {'command': 'foo-bin'}}}))
...
def test_loader_normalizes_legacy_llm_tag_at_current_schema_version():
    ...
    ``schema_version`` must read back as ``Creanova``.

    The SDK's ``llm -> Creanova`` rename only fires while advancing the
    ...
    every read stays on the canonical ``{Creanova, acp}`` variants — this is
    ...
    assert loaded.agent_kind == 'Creanova'
...
def test_loader_preserves_acp_variant_without_coercion():
    ...
    ACP settings (``ACPAgentSettings.agent_context`` is nullable; the Creanova
    shape rejects ``None``).
    """
    loaded = _load_persisted_agent_settings(
        {
