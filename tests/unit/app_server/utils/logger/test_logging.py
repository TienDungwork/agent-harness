...
from Creanova.app_server.utils.logger import (
    LOG_JSON_LEVEL_KEY,
    CreanovaLoggerAdapter,
    json_log_handler,
)
from Creanova.app_server.utils.logger import Creanova_logger as Creanova_logger
...
def test_handler():
    ...
    Creanova_logger.addHandler(handler)
    yield Creanova_logger, stream
    Creanova_logger.removeHandler(handler)
...
def json_handler():
    ...
    Creanova_logger.addHandler(json_handler)
    yield Creanova_logger, stream
    Creanova_logger.removeHandler(json_handler)
...
def test_sensitive_env_vars_masking(test_handler):
    ...
    with patch.dict('Creanova.app_server.utils.logger.os.environ', environ, clear=True):
...
class TestJsonOutput:
    ...
    def test_extra_fields_from_adapter(self, json_handler):
        logger, string_io = json_handler
        subject = CreanovaLoggerAdapter(logger, extra={'context_field': '..val..'})
    ...
    def test_extra_fields_from_adapter_can_override(self, json_handler):
        logger, string_io = json_handler
        subject = CreanovaLoggerAdapter(logger, extra={'override': 'a'})
