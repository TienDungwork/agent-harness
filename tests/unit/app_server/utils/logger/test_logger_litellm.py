...
def reset_litellm():
    ...
    if 'Creanova.app_server.utils.logger' in sys.modules:
        del sys.modules['Creanova.app_server.utils.logger']
...
def test_litellm_settings_debug_llm_disabled(reset_litellm):
    ...
    with mock.patch.dict(os.environ, {'DEBUG_LLM': 'false'}):
        import Creanova.app_server.utils.logger  # noqa: F401

        importlib.reload(Creanova.app_server.utils.logger)
...
def test_litellm_loggers_suppressed_with_uvicorn_json_config(reset_litellm):
    ...
    # Find the logger.py file path relative to the Creanova package
    # __file__ is tests/unit/app_server/utils/logger/test_logger_litellm.py
    # We need to go up to tests/, then find Creanova/app_server/utils/logger.py
    test_dir = pathlib.Path(__file__).parent  # tests/unit/app_server/utils/logger
    project_root = test_dir.parent.parent.parent.parent.parent  # workspace/Creanova
    logger_file = project_root / 'Creanova' / 'app_server' / 'utils' / 'logger.py'
