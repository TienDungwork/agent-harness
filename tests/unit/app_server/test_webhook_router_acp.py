...
async def test_llm_conversation_stores_llm_model(
    ...
    """LLM path stores the real model in llm_model and sets agent_kind='Creanova'."""
    ...
    assert saved.agent_kind == 'Creanova'
...
async def test_acp_conversation_analytics_llm_model_is_null(
    ...
    with (
        ...
        patch(
            'Creanova.app_server.event_callback.webhook_router.get_analytics_service',
            return_value=analytics,
        ),
        patch(
            'Creanova.app_server.event_callback.webhook_router.resolve_analytics_context',
