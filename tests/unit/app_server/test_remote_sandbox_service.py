...
from Creanova.app_server.errors import SandboxDeleteRetryError, SandboxError
...
from Creanova.app_server.settings.settings_models import SandboxGroupingStrategy
...
def mock_sandbox_spec_service():
    ...
    mock_spec = SandboxSpecInfo(
        ...
        command=['/usr/local/bin/Creanova-agent-server', '--port', '60000'],
...
class TestSandboxLifecycle:
    ...
    async def test_archive_conversation_workspace_archives_without_stopping(
        ...
        with patch(
            'Creanova.app_server.sandbox.remote_sandbox_service.workspace_archive'
        ) as mock_wa:
...
class TestPollAgentServersSessionScoping:
    ...
    def _patches(
        ...
        return [
            patch('Creanova.app_server.config.get_db_session', side_effect=tracker),
            patch(
                'Creanova.app_server.config.get_app_conversation_info_service',
                side_effect=_async_cm_factory(conv_service),
            ),
            patch(
                'Creanova.app_server.config.get_event_service',
                side_effect=_async_cm_factory(event_service),
            ),
            patch(
                'Creanova.app_server.config.get_event_callback_service',
                side_effect=_async_cm_factory(callback_service),
            ),
            patch(
                'Creanova.app_server.config.get_httpx_client',
                side_effect=_async_cm_factory(httpx_client),
            ),
            patch(
                'Creanova.app_server.sandbox.remote_sandbox_service.ConversationInfo',
                mock_conv_info,
            ),
            patch(
                'Creanova.app_server.sandbox.remote_sandbox_service.EventPage',
                mock_event_page,
            ),
            patch('Creanova.app_server.sandbox.remote_sandbox_service.InjectorState'),
            patch('Creanova.app_server.sandbox.remote_sandbox_service.ADMIN'),
            patch(
                'Creanova.app_server.sandbox.remote_sandbox_service.USER_CONTEXT_ATTR',
                'user_context',
            ),
        ]
...
class TestArchiveConversationWorkspace:
    ...
    async def test_required_failure_returns_false(self, remote_sandbox_service):
        ...
        with (
            patch(
                'Creanova.app_server.sandbox.workspace_archive.archive_enabled',
                return_value=True,
            ),
            patch(
                'Creanova.app_server.sandbox.workspace_archive.archive_workspace',
    ...
    async def test_pinned_path_forwarded_to_archive(self, remote_sandbox_service):
        ...
        with (
            ...
            ok = await remote_sandbox_service.archive_conversation_workspace(
                ...
                workspace_path='/home/Creanova/workspace/conv-1',
            )
        assert ok is True
        _, kwargs = mock_archive.call_args
        assert kwargs['archive_path'] == '/home/Creanova/workspace/conv-1'
    ...
    async def test_resolve_archive_path_uses_pinned(self, remote_sandbox_service):
        ...
        path = await remote_sandbox_service._resolve_archive_path(
            stored, 'conv-1', '/home/Creanova/workspace/conv-1'
        )
        assert path == '/home/Creanova/workspace/conv-1'
        remote_sandbox_service.sandbox_spec_service.get_sandbox_spec.assert_not_called()
...
class TestArchiveWorkspaceHelper:
    ...
    async def test_archive_uploads_archive_and_manifest(self, monkeypatch):
        from Creanova.app_server.sandbox import workspace_archive
...
class TestDeleteSandboxKeyHandling:
    ...
    async def async_engine(self):
        ...
        from Creanova.app_server.utils.sql_utils import Base
...
class TestArchiveEnvToggles:
    """Per Creanova/AGENTS.md, env enable-toggles must accept '1' as well as
