...
async def test_saving_settings_with_frozen_secrets_store(test_client):
    ...
    See https://github.com/Creanova/Creanova/issues/13306.
    """
    payload = _dump_update(
        Settings(
            language='en',
            agent_settings=CreanovaAgentSettings(llm=LLM(model='gpt-4')),
        )
    )
...
async def test_search_api_key_explicit_clear(test_client):
    ...
    response = test_client.post(
        ...
        json=_dump_update(
            Settings(
                search_api_key='',
                agent_settings=CreanovaAgentSettings(llm=LLM(model='claude-3-opus')),
            )
        ),
    )
    assert response.status_code == 200
    ...
    response = test_client.post(
        ...
        json=_dump_update(
            Settings(
                ...
                agent_settings=CreanovaAgentSettings(llm=LLM(model='test-model')),
            )
        ),
    )
    assert response.status_code == 200
