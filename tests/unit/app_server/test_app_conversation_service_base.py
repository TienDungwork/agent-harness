...
from Creanova.app_server.app_conversation.app_conversation_models import AgentType
...
from Creanova.app_server.sandbox.sandbox_models import SandboxInfo, SandboxStatus
...
@patch(
    'Creanova.app_server.app_conversation.app_conversation_service_base.LLMSummarizingCondenser'
)
...
def test_create_security_analyzer_returns_llm_analyzer():
    ...
    # Assert
    from Creanova.sdk.security import LLMSecurityAnalyzer
...
def test_create_security_analyzer_logs_warning_for_unknown_value():
    ...
    # Act
    with patch(
        'Creanova.app_server.app_conversation.app_conversation_service_base._logger'
    ) as mock_logger:
...
def test_select_confirmation_policy_when_disabled_returns_never_confirm():
    ...
    # Assert
    from Creanova.sdk.security import NeverConfirm
...
def test_select_confirmation_policy_llm_returns_confirm_risky():
    ...
    # Assert
    from Creanova.sdk.security import ConfirmRisky
...
def test_select_confirmation_policy_non_llm_returns_always_confirm(
    ...
    # Assert
    from Creanova.sdk.security import AlwaysConfirm
...
async def test_clone_or_init_git_repo_configures_dynamic_azure_devops_helper(
    ...
    helper_command = next(
        command
        for command in commands
        if 'Creanova-azure-devops-credential-helper' in command
    )
    assert (
...
async def test_azure_devops_git_credential_helper_logs_without_web_url(
    ...
    with patch(
        'Creanova.app_server.app_conversation.app_conversation_service_base._logger.debug'
    ) as mock_debug:
...
class TestLoadAndMergeAllSkills:
    ...
    @pytest.mark.asyncio
    @patch(
        'Creanova.app_server.app_conversation.app_conversation_service_base.load_skills_from_agent_server'
    )
    @patch(
        'Creanova.app_server.app_conversation.app_conversation_service_base.build_org_configs'
    )
    @patch(
        'Creanova.app_server.app_conversation.app_conversation_service_base.build_sandbox_config'
    )
    async def test_loads_skills_successfully(
        ...
        with patch.object(AppConversationServiceBase, '__abstractmethods__', set()):
            ...
            from Creanova.app_server.sandbox.sandbox_models import ExposedUrl
