...
import Creanova.app_server.settings.settings_models as settings_module
from Creanova.app_server.settings.llm_profiles import ProfileNotFoundError
...
from Creanova.sdk.mcp.config import dump_mcp_config
from Creanova.sdk.settings import (
    AGENT_SETTINGS_SCHEMA_VERSION,
    ConversationSettings,
    CreanovaAgentSettings,
)
from Creanova.sdk.settings.model import CondenserSettings, VerificationSettings
...
def test_settings_loads_persisted_settings_via_sdk_loaders():
    loaded_agent_settings = CreanovaAgentSettings(agent='migrated-agent')
...
def test_settings_update_mcp_config():
    settings = Settings(
        agent_settings=CreanovaAgentSettings(llm=LLM(model='sdk-model'))
    )
...
def test_litellm_proxy_with_Creanova_proxy_keeps_prefix_for_display():
...
def test_litellm_proxy_custom_endpoint_keeps_prefix():
    ...
    # Display should NOT convert to Creanova/ because it's a custom endpoint
    api_data = settings.get_agent_settings_display()
    assert api_data['llm']['model'] == 'litellm_proxy/gpt-5.3-codex'


def test_Creanova_model_display_does_not_reverse_map():
    """Display data reflects the LLM model shape provided by the SDK."""
    settings = Settings(
        agent_settings=CreanovaAgentSettings(
            llm=LLM(model='Creanova/claude-opus-4-5-20251101')
        )
    )
...
class TestMarketplaceRegistration:
    ...
    def test_repo_path_validation_rejects_url(self):
        ...
        with pytest.raises(ValidationError, match='not a URL'):
            MarketplaceRegistration(
                name='test',
                source='github:Creanova/extensions',
                repo_path='https://github.com/Creanova/extensions',
            )
