...
Skills are specialized prompts that enhance Creanova with domain-specific knowledge and task-specific workflows. They help developers by providing expert guidance, automating common tasks, and ensuring consistent practices across projects. Each skill is designed to excel in a specific area, from Git operations to code review processes.

## Terminology Note

**Version 0 (V0)**: The term "microagents" continues to be used for V0 conversations. V0 is the current stable version of Creanova.
...
This directory (`Creanova/skills/`) contains shareable skills that will be used in V1 conversations. For V0 conversations, the system continues to use microagents from the same underlying files.

## Sources of Skills/Microagents

Creanova loads skills (V1) or microagents (V0) from two sources:

### 1. Shareable Skills/Microagents (Public)

This directory (`Creanova/skills/`) contains shareable skills (V1) or microagents (V0) that are:

- Available to all Creanova users
- Maintained in the Creanova repository
...
Each repository can have its own instructions in `.Creanova/microagents/` (V0) or `.Creanova/skills/` (V1). These instructions are:
...
- V1 supports both `.Creanova/skills/` (preferred) and `.Creanova/microagents/` (backward compatibility)
...
When Creanova works with a repository, it:

1. Loads repository-specific instructions from `.Creanova/microagents/repo.md` (V0) or `.Creanova/skills/` (V1) if present
2. Loads relevant knowledge agents based on keywords in conversations

**Note**: V1 also supports loading from `.Creanova/microagents/` for backward compatibility.
...
You can see an example of a knowledge-based agent in [Creanova's github skill](https://github.com/Creanova/Creanova/tree/main/skills/github.md).

### 2. Repository Agents

Repository agents provide repository-specific knowledge and guidelines. They are:

- Loaded from `.Creanova/microagents/repo.md` (V0) or `.Creanova/skills/` directory (V1)
- V1 also supports `.Creanova/microagents/` for backward compatibility
...
You can see an example of a repo agent in [the glossary for the Creanova repo](https://github.com/Creanova/Creanova/blob/main/.Creanova/microagents/glossary.md).
...
2. **For Repository Agents**:
   ...
   - Consider using Creanova to generate a comprehensive repo.md (see [Creating a Repository Agent](#creating-a-repository-agent))
...
1. Create your agent file in the appropriate directory:
   ...
   - Note: Repository-specific agents should remain in their respective repositories' `.Creanova/skills/` (V1) or `.Creanova/microagents/` (V0) directory
2. Test thoroughly
3. Submit a pull request to Creanova

## License

All skills/microagents are subject to the same license as Creanova. See the root LICENSE file for details.
