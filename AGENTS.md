This repository contains the code for Creanova, an automated AI software engineer. It has a Python backend
(in the `Creanova` directory) and React frontend (in the `frontend` directory).

## General Setup:
To set up the entire repo, including frontend and backend, run `make build`.
You don't need to do this unless the user asks you to, or if you're trying to run the entire application.

## Running Creanova with Creanova:
To run the full application to debug issues:
```bash
export INSTALL_DOCKER=0
export RUNTIME=local
make build && make run FRONTEND_PORT=12000 FRONTEND_HOST=0.0.0.0 BACKEND_HOST=0.0.0.0 &> /tmp/Creanova-log.txt &
```
...
### Agent Canvas (local overlay)
- Full stack from `agent-canvas/`: `docker compose up -d` starts agents + **ClickHouse** (`creanova-clickhouse`) + **vms-sync** + local-gateway + path gateway. CH config/init under `deploy/clickhouse/`; data volume `clickhouse_creanova_ch_data` (external). `local-gateway` waits for ClickHouse healthy; `CH_URL=http://creanova-clickhouse:8123`, `CH_PASSWORD` must match `CH_VMS_PASSWORD`.
- Agent Canvas create path **never** injects the public coding-skill catalog (`skills: []`, `load_*_skills: false`) — VMS/SSH agent has no use for ~350KB of SKILL.md and it overflows LiteLLM project message limits.
- Named `agent_profile_id` launches used to re-inject ~63 skills via `discover_profile_skills()` (~480KB). Mitigations: `patch_no_profile_skills.py` (import-module), local-auth UI skips profile launch, gateway strips inline skills.
- Gateway create also rewrites LLM from the per-user settings blob (UI/Ollama) instead of encrypted disk LiteLLM project — avoids `vượt giới hạn project` when disk still points at `:18083`.
- `local-gateway` also strips `agent_context.skills` on `POST /api/conversations` (`_lean_conversation_create_body`) so a cached browser cannot re-inject the catalog. MCP (VMS/SSH) is kept.
- Cross-login LiteLLM `AuthenticationError` / missing `OPENAI_API_KEY`: local-gateway per-user settings blobs cache UI-redacted `api_key` (`**********`). Lean create must not force `secrets_encrypted=False` unless the rewrite installed a plaintext key; Fernet keys need decrypt. Tunnel/UI models are rewritten to `ANALYTICS_LLM_*` (Ollama) with plaintext `ollama` key.
- Create latency today is dominated by MCP stdio handshake (~2s for `creanova_infra`), not skills. Mitigations:
  - Always launch infra MCP via `python3 /opt/infra-mcp/mcp_stdio.py` (never `uv run --with …`); UI `ensureCreanovaInfraMcpConfig` + gateway rewrite both enforce this.
  - App-shell warm-pools one empty local conversation once via `useWarmLocalConversationPrefetch` in `root-layout` (do not also mount that effect from Home — dual mounts discard each other).
  - Home / sidebar "no workspace" claim the warm slot (`claimWarmLocalConversation`); on miss, `createWarmLocalConversationResponse` (same lean create) — never the heavy `useCreateConversation` mutation for plain local text.
  - VMS/local analytics text also uses warm + navigate-then-`sendMessage` (do **not** gate on `needsInfraMcp`); lean gateway already attaches `creanova_infra`.
  - Skip the "Creating…" toast when `peekWarmLocalConversation()` is already READY so Enter feels instant.
  - Warm slot must be discarded when the active LLM / Agent profile changes (`discardWarmLocalConversation`); otherwise Home Enter reuses a conversation baked with the previous model (e.g. selecting `qwen3-8b` still chats as `ntiendung`).
  - Warm create mirrors profile rules: named Agent profiles launch via `agent_profile_id`; seeded `default` uses `agent_settings` (active LLM).
  - Home local text submits create without `initial_message`, then `sendMessage` after navigate.
- After changing `agent-canvas/src/**`, rebuild overlay only (no restart): `docker compose --profile overlay-build run --rm canvas-ui-build` then hard-refresh `/agents`. That command builds into `build.staging` and syncs into `./build` so the live bind-mount is never emptied mid-build. Overlay static serving uses `--live-static` (sirv without startup file-tree cache) so new hashed JS/CSS are visible immediately. Do **not** `docker compose restart agent-canvas` for UI-only changes — restart reloads every conversation and blanks `:18000/agents` for ~30s. Restart only when entrypoint/compose/env changes.
- Do not bypass the agent for VMS Q&A (no `/vms` fast UI). Home creates a normal conversation; the agent must call MCP tools (`vms_count_vehicles`, `vms_plate_flow`, `vms_plate_by_manufacturer`, `vms_trace_plate`, `vms_intrusion_peak`, …). Seat counts (5/7/9…) are not in CH attrs yet — `vms_plate_flow` says so.
- Latency: ClickHouse/API ~0.1s. Slow UX was (1) first LLM tool-call TTFT (fat tool schemas) and (2) second LLM re-typing `reply_vi`. Mitigations: lean create (`tools=[]`, short LOCAL_HARNESS), **single MCP tool `vms_query`** (`MCP_VMS_ONLY=1`, no SSH on lean path), and `patch_vms_reply_vi_finish.py` which finishes when observation has `reply_vi`. **Do not** hardcode/autodispatch tool calls — the agent must choose via the LLM (`vms_query` + `action=`).
- VMS charts: MCP slim payload includes `chart`; short-circuit embeds `<!--CREANOVA_VMS_CHART:…-->` in the assistant MessageEvent and streams via `StreamingDeltaEvent` with **`asyncio.sleep` on the async path** (sync `time.sleep` blocks the event loop → WS only flushes after the full reply). UI strips the marker and renders **ApexCharts** (npm local: line / column / donut — no CDN) under the agent message; CSS bars only as error fallback.
- Residual chat latency vs direct Ollama was mostly (a) ~16KB SDK coding system prompt (OpenHandsAgentSettings drops client `system_prompt`), (b) autotitle second LLM call, (c) many MCP tool schemas. Mitigations: `patch_lean_local_agent.py` (SOUL-only prompt + FinishTool + NoOp condenser when `<LOCAL_HARNESS>`), gateway `autotitle=false` / always lean MCP / force local Ollama + `openai/` model prefix.
- Mid-chat VMS after a greeting used to strip MCP → empty LLM + “function call or a message” nudge loops. Fix: always attach lean `creanova_infra` on create; greet turns temporarily `_tools={}` so Ollama prefill stays small. VMS turns expose `vms_query` only.
- Ollama/Qwen sometimes emits text ``<tool_call>{…}</tool_call>`` instead of native FC → UI freezes on raw XML. `patch_vms_reply_vi_finish` recovers those into real MCP ActionEvents (still the tool the model named — not intent hardcoding).
...
* If you've made changes to the VSCode extension, you should run `cd Creanova/app_server/integrations/vscode && npm run lint:fix && npm run compile ; cd ../../..`
...
- GitHub-authored (`actions/*`, `github/*`) and first-party (`Creanova/*`) actions are currently exempt.
...
Backend:
- Located in the `Creanova` directory
- The current V1 application server lives in `Creanova/app_server/`. `make start-backend` still launches `Creanova.server.listen:app`, which includes the V1 routes by default unless `ENABLE_V1=0`.
...
VSCode Extension:
- Located in the `Creanova/app_server/integrations/vscode` directory
...
The `enterprise/` directory contains additional functionality that extends the open-source Creanova codebase. This includes:
...
**Setup Steps:**
1. First, build the main Creanova project: `make build`
...
- The enterprise server extends the Creanova server through dynamic imports
- Database changes require careful migration planning in `enterprise/migrations/`
- Always test changes in both Creanova and enterprise contexts
- Use the enterprise-specific Makefile commands for development
- When the `Creanova-ai` package (root project) version has been updated, run `poetry lock` in the `enterprise/` folder to update the version in the enterprise poetry lockfile.
...
- This ensures code works in both Creanova and enterprise contexts
...
- For frontend issues, ensure the main Creanova frontend is built: `make build`
...
Microagents are specialized prompts that enhance Creanova with domain-specific knowledge and task-specific workflows. They are Markdown files that can include frontmatter for configuration.

#### Types:
- **Public Microagents**: Located in `microagents/`, available to all users
- **Repository Microagents**: Located in `.Creanova/microagents/`, specific to this repository
...
- To add a new user setting to Creanova, follow these steps:
  ...
  2. Add the setting to the backend:
     - Add the setting to the `Settings` model in `Creanova/app_server/settings/settings_models.py`
     - Update any relevant backend code to apply the setting (e.g., in session creation)

#### Settings UI Patterns:

There are two main patterns for saving settings in the Creanova frontend:
...
To add a new LLM model to Creanova, you need to update multiple files across both frontend and backend:
...
1. **Frontend Model Arrays** (`frontend/src/utils/verified-models.ts`):
   ...
   - Add to provider-specific arrays based on the model's provider:
     ...
     - `VERIFIED_Creanova_MODELS` for models available through Creanova provider

2. **Backend CLI Integration** (`Creanova/cli/utils.py`):
   - Add the model to the appropriate `VERIFIED_*_MODELS` arrays
   - This ensures the model appears in CLI model selection

3. **Backend Model List** (`Creanova/utils/llm.py`):
   - **CRITICAL**: Add the model to the `Creanova_models` list (lines 57-66) if using Creanova provider
   - This is required for the model to appear in the frontend model selector
   - Format: `'Creanova/model-name'` (e.g., `'Creanova/o3'`)

4. **Backend LLM Configuration** (`Creanova/llm/llm.py`):
...
- **VERIFIED_Creanova_MODELS**: Models available through Creanova managed provider
...
#### User Credentials with Exposed Secrets (in `Creanova/app_server/user/user_router.py`):
...
- Bearer token (proves user identity via `Creanova_API_KEY`)
...
#### Sandbox-Scoped Secrets Endpoints (in `Creanova/app_server/sandbox/sandbox_router.py`):
...
- `Creanova/sdk/llm/llm.py`: `LLM.api_key` accepts `SecretSource` (including `LookupSecret`)
- `Creanova/workspace/cloud/workspace.py`: `get_llm()` and `get_secrets()` return LookupSecret-backed objects
...
- The Creanova classifier logic for newcomer suitability lives in `scripts/issue_good_first_issue_check_Creanova.py`, with focused unit coverage in `tests/unit/test_issue_good_first_issue_check_Creanova.py`.
